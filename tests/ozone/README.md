...
The shell harness bounds Docker actions, gateway and bucket readiness, service
stop/start, and cleanup. The Rust contract tests also impose a bounded request
timeout. By default the harness removes the test-owned container, its
anonymous Ozone data volumes, and its temporary fixture directory. Set
`MOUNT_RS_OZONE_KEEP=1` only when an explicit retained container is needed for
diagnosis.
