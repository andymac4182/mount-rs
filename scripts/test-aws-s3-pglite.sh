...
# version-aware cleanup; this script owns only its PGlite child, temporary
# database snapshot, and log file.
umask 077
...
if ! command -v node >/dev/null 2>&1 || ! command -v cargo >/dev/null 2>&1 || ! command -v tar >/dev/null 2>&1; then
  echo "AWS S3 plus PGlite test requires Node.js, cargo, and tar" >&2
  exit 2
fi
...
data_dir=$(mktemp -d "${TMPDIR:-/tmp}/mount-rs-aws-s3-pglite-data.XXXXXX")
backup_file=$(mktemp "${TMPDIR:-/tmp}/mount-rs-aws-s3-pglite-backup.XXXXXX.tar")
scope="aws-s3-pglite-${AWS_S3_TEST_PREFIX##*/}"
server_pid=
...
start_server() {
  ...
  PGLITE_DATA_DIR="$data_dir" \
    PGLITE_PORT="$port" \
    node "$server_dir/server.mjs" >"$log_file" 2>&1 &
  server_pid=$!
  ready=0
...
start_server
run_test live_aws_s3_blocks_with_independent_pglite_metadata
run_test live_aws_s3_pglite_prepare_for_restart
stop_server

# Treat the on-disk PGlite directory as a backup artifact, then restore it to
# a fresh directory before the second server process opens the same volume.
# This is a local metadata backup/restore drill; the parent AWS harness still
# owns cleanup of all objects below the exact test prefix.
tar -C "$data_dir" -cf "$backup_file" .
rm -rf "$data_dir"
mkdir "$data_dir"
tar -C "$data_dir" -xf "$backup_file"

start_server
run_test live_aws_s3_pglite_reopen_after_restore
stop_server

echo "AWS_S3_PGLITE_TEST_PASS prefix=$AWS_S3_TEST_PREFIX scope=$scope"
