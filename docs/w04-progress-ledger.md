...
| Field | Current value |
| --- | --- |
| Snapshot base revision | `1d750ed` (`origin/main` at this ledger refresh; the isolated candidate source `d550805` is in its history and the published W04 EPIPE fix remains `bdfcb11`) |
...
| Latest published repository revision | `1d750ed` (latest `origin/main` tip at this ledger refresh; the W04 source and provider-matrix lockfile fixes are in its history) |
| Qualification evidence revision | Isolated candidate source `d55080550cd4` on `andymac4182/c/w04-production-candidate-lockfile-20260922`; run [35610813385](https://github.com/andymac4182/mount-rs/actions/runs/35610813385) is terminal overall **failure** only because provider jobs failed, while all five Node jobs and `aggregate-native` passed. Shared-main run [35609647646](https://github.com/andymac4182/mount-rs/actions/runs/35609647646) was cancelled and is not evidence |
| Snapshot time | 2026-09-22 00:44 AEST |
...
| Hosted/native/provider acceptance | **W04.2 and isolated production-candidate Node/package qualification complete; provider acceptance remains open**: run [35610813385](https://github.com/andymac4182/mount-rs/actions/runs/35610813385) passed Linux, Linux ARM, macOS-latest, macOS-15-intel, and Windows Node jobs, including every advertised exact PGlite/restart step and Windows package distribution; `aggregate-native` [106379759443](https://github.com/andymac4182/mount-rs/actions/runs/35610813385/job/106379759443) passed artifact aggregation, five-package validation, and clean-consumer smoke. The run is still overall failure because Ozone variants hit a stale provider lockfile and FoundationDB/RustFS failed a config-backed FUSE mount. Shared-main run [35609647646](https://github.com/andymac4182/mount-rs/actions/runs/35609647646) was cancelled; provider credentials/services and launch-scope decisions remain explicit gates |
...
| Current external blocker | The isolated candidate run [35610813385](https://github.com/andymac4182/mount-rs/actions/runs/35610813385) is terminal overall failure despite a green W04 Node/package slice: Ozone [106369155190](https://github.com/andymac4182/mount-rs/actions/runs/35610813385/job/106369155190), [106369155571](https://github.com/andymac4182/mount-rs/actions/runs/35610813385/job/106369155571), [106369155784](https://github.com/andymac4182/mount-rs/actions/runs/35610813385/job/106369155784), and [106369155910](https://github.com/andymac4182/mount-rs/actions/runs/35610813385/job/106369155910) failed before their real contract on `tests/ozone/Cargo.lock` being stale under `--locked`; FoundationDB/RustFS [106369155935](https://github.com/andymac4182/mount-rs/actions/runs/35610813385/job/106369155935) reached real composition checks but failed `cli_foundationdb_rustfs_config_binary_mounts_and_reopens` because config-backed FUSE did not mount. The five Node jobs and aggregate/package gate passed, but production rollout remains NO-GO pending provider scope, persistence/rollback, observability, ownership, and release approval. |
...
| Native package/artifact and consumer validation | Complete — isolated production-candidate qualification; shared-main run cancelled | 100% | Run [35610813385](https://github.com/andymac4182/mount-rs/actions/runs/35610813385) passed Windows Node package distribution [106369156167](https://github.com/andymac4182/mount-rs/actions/runs/35610813385/job/106369156167) and `aggregate-native` [106379759443](https://github.com/andymac4182/mount-rs/actions/runs/35610813385/job/106379759443). The aggregate log records `mount-rs N-API artifact aggregation: PASS`, validation of all five native distribution packages, and `mount-rs clean consumer install/smoke: PASS`. | Preserve the exact candidate artifacts for the release record; production release remains NO-GO until persistence/rollback, provider scope, operations, ownership, and release approval close. | 0h implementation; 0.25h evidence publication | Hosted/package gate |
...
| Provider and durability matrix | Explicitly bounded; W04 PGlite slice green, broader provider run failed | 60% for local/PGlite scope / 0% for unapproved providers | The five Node jobs in [35610813385](https://github.com/andymac4182/mount-rs/actions/runs/35610813385) passed PGlite/restart and provider-matrix rows with explicit R2/TiDB/RustFS skips. Ozone variants failed before their real contract because `tests/ozone/Cargo.lock` was stale under `--locked`; FoundationDB/RustFS reached real composition and failed the config-backed FUSE mount assertion. These failures are not silently promoted to W04 passes. | Decide whether launch scope is PGlite-only. If Ozone or FoundationDB/RustFS is in scope, refresh and verify its standalone lockfile, fix/requalify the config-backed FUSE mount, and run real restart/cleanup/recovery evidence. | 1–4h engineering plus provider wait | External provider gate |
...
| W04 production-candidate hosted qualification | Complete — isolated candidate Node/package slice; overall provider qualification open | 90% | On source `d55080550cd4`, run [35610813385](https://github.com/andymac4182/mount-rs/actions/runs/35610813385) passed Linux Node [106369155618](https://github.com/andymac4182/mount-rs/actions/runs/35610813385/job/106369155618), Linux ARM [106369155597](https://github.com/andymac4182/mount-rs/actions/runs/35610813385/job/106369155597), macOS-latest [106369155745](https://github.com/andymac4182/mount-rs/actions/runs/35610813385/job/106369155745), macOS-15-intel [106369155579](https://github.com/andymac4182/mount-rs/actions/runs/35610813385/job/106369155579), and Windows Node [106369156167](https://github.com/andymac4182/mount-rs/actions/runs/35610813385/job/106369156167). Each Unix Node job's exact `Verify PGlite integration and restart recovery` step passed; the four Unix logs reported Rust SDK `pass=6 skip=3 fail=0`, Node SDK `pass=5 skip=3 fail=0`, CLI `pass=12 skip=2 fail=0`, upstream `1,200 passed / 82 skipped`, and trace `TRACE_EVIDENCE_JSON status PASS` including PGlite/chunked-PGlite. Windows package distribution passed and aggregate-native [106379759443](https://github.com/andymac4182/mount-rs/actions/runs/35610813385/job/106379759443) passed all five packages plus clean consumer smoke. | Close the remaining provider qualification only if those providers are in launch scope; otherwise record the production launch scope as PGlite-only and retain the failed provider jobs as excluded evidence. | 0.5–2h scope/approval plus provider remediation if in scope |
...
10. [ ] Obtain or explicitly exclude the provider scope behind the terminal
    Ozone lockfile failures and FoundationDB/RustFS config-backed FUSE failure
    before a production release decision. A PGlite-only launch may exclude
    those services only after the release owner records that scope.
...
provider-matrix lockfile diagnosis and fix `caa9324`, and isolated candidate
run `35610813385`. It is reviewed with `git diff --check` and the relevant
formatting check, then committed and pushed to `origin/main`. It does **not**
approve production: the isolated run's Ozone/FoundationDB-RustFS provider
failures, launch-scope decision, persistence/rollback policy, observability,
runbook execution, ownership, and release approval remain open.
