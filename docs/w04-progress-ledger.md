...
| Field | Current value |
| --- | --- |
| Snapshot base revision | `c719306` (`origin/main` at this ledger refresh; the published W04 EPIPE fix remains `bdfcb11`) |
| Ledger publication | This current ledger revision is published on `origin/main`; the exact commit is recorded in Git history |
| Current-head local evidence revision | `98243d2` (published N-API reconcile-provider teardown fix plus active-multipart oracle fixture fix; local focused N-API and 40-case HTTP-parity checks passed) |
...
| Latest published repository revision | `c719306` (latest `origin/main` tip at this ledger refresh; W04 N-API and oracle fixes are included in its history) |
| Qualification evidence revision | `98243d2` on `andymac4182/c/w04-production-candidate-20260921`; isolated run `35608472226` is the current published-candidate qualification and remains in progress |
| Snapshot time | 2026-09-21 23:56 AEST |
...
| Hosted/native/provider acceptance | **W04.2 hosted acceptance complete; production-candidate qualification in progress**: isolated run [35588994864](https://github.com/andymac4182/mount-rs/actions/runs/35588994864) passed the exact PGlite recovery step on Linux, macOS-latest, and macOS-15-intel. The follow-up run [35602906005](https://github.com/andymac4182/mount-rs/actions/runs/35602906005) exposed the Windows Node cleanup race and the cross-platform `NoSuchUpload` parity failure; source fixes are now published in `98243d2`, and candidate run [35608472226](https://github.com/andymac4182/mount-rs/actions/runs/35608472226) is executing all four Node jobs. Provider credentials/services remain explicit gates |
...
| Current external blocker | The published candidate `98243d2` contains the N-API fix that clears the reconcile callback after shutdown (preventing SQLite handles from keeping `blocks.sqlite` busy) and the HTTP-oracle fix that preserves active multipart-upload mtimes; local focused tests passed. Candidate run [35608472226](https://github.com/andymac4182/mount-rs/actions/runs/35608472226) has Windows Node job [106361359489](https://github.com/andymac4182/mount-rs/actions/runs/35608472226/job/106361359489), Linux Node job [106361359694](https://github.com/andymac4182/mount-rs/actions/runs/35608472226/job/106361359694), Linux ARM Node job [106361359780](https://github.com/andymac4182/mount-rs/actions/runs/35608472226/job/106361359780), macOS-latest Node job [106361359553](https://github.com/andymac4182/mount-rs/actions/runs/35608472226/job/106361359553), and macOS-15-intel Node job [106361359767](https://github.com/andymac4182/mount-rs/actions/runs/35608472226/job/106361359767) in progress; their exact recovery/package steps are not evidence until terminal logs are available. Production rollout remains NO-GO pending this candidate result, clean published-main package/consumer evidence, provider scope, persistence/rollback, observability, ownership, and release approval. |
...
| W04 N-API reconcile-provider teardown | Complete — implementation and local regression | 100% | Commit `88ecee2` clears the stored `Filesystem.reconcile` callback after successful shutdown, releasing the cloned `ChunkedFs`/SQLite handles before Windows temporary-directory removal. The N-API library suite passed 15 tests; the debug addon rebuilt and `node test/chunked.mjs` passed its non-PGlite integration path locally. | Confirm the hosted Windows Node cleanup and package-distribution steps on candidate run `35608472226`. | 0h remaining; hosted confirmation open |
| W04 HTTP parity oracle fixture | Complete — local qualification fix | 100% | Commit `98243d2` preserves the real mtime only for private active multipart-upload directories in `examples/http_oracle.rs`; local `check-http-parity.mjs` passed all 40 S3/WebDAV paired cases and the full S3 gateway suite passed 15 tests. | Confirm the hosted Node HTTP-parity lane and subsequent PGlite recovery steps on candidate run `35608472226`. | 0h remaining; hosted confirmation open |
| W04 production-candidate hosted qualification | In progress — hosted/native/provider gate | 75% | Candidate run [35608472226](https://github.com/andymac4182/mount-rs/actions/runs/35608472226) was launched from published source `98243d2`. At the ledger snapshot, Windows Node [106361359489](https://github.com/andymac4182/mount-rs/actions/runs/35608472226/job/106361359489), Linux Node [106361359694](https://github.com/andymac4182/mount-rs/actions/runs/35608472226/job/106361359694), Linux ARM Node [106361359780](https://github.com/andymac4182/mount-rs/actions/runs/35608472226/job/106361359780), macOS-latest Node [106361359553](https://github.com/andymac4182/mount-rs/actions/runs/35608472226/job/106361359553), and macOS-15-intel Node [106361359767](https://github.com/andymac4182/mount-rs/actions/runs/35608472226/job/106361359767) were in progress. No exact PGlite/package pass is claimed before terminal logs. | Inspect each completed Node log, require exact `Verify PGlite integration and restart recovery` plus Windows package/consumer evidence, then record provider skips/failures without promoting them to W04 acceptance. | 0.5–2h hosted runner time plus any evidence-backed remediation |
...
8. [ ] Confirm the published N-API teardown fix in candidate run
   `35608472226`: require Windows Node to reach `Verify Windows package
   distribution`, then retain artifact aggregation and clean-consumer results.
9. [ ] Confirm the published HTTP-oracle fix in candidate run `35608472226`:
   require every advertised Node job to complete its HTTP-parity lane and the
   exact PGlite recovery step; record provider skips/failures separately.
...
| Native artifact aggregation and clean consumer smoke | Complete for qualification; 0.25–1h published-candidate enforcement remains | Hosted/package gate |
| Windows Node `EBUSY` cleanup remediation and hosted rerun | 1–3h | Engineering plus hosted/native gate |
| Cross-platform `NoSuchUpload` HTTP-parity lane | 0.25–1h hosted verification; provider fallback if it recurs | Hosted/provider boundary; local fixture fix published and hosted candidate confirmation pending; not converted to a W04 pass |
...
long-symlink pass, the local N-API and HTTP-oracle fixes, and the in-progress
published-candidate qualification run `35608472226`. It is reviewed with
`git diff --check` and the relevant formatting check, then committed and
pushed to `origin/main`. It does **not** approve production: candidate Node
