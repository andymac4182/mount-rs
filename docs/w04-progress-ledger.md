...
| Field | Current value |
| --- | --- |
| Snapshot base revision | `8f0e7413` (current `origin/main` at this ledger update; includes the published W04 test hardening `d955042b`, terminal qualification evidence, and concurrent unrelated mainline work) |
| Ledger publication | This revision records the terminal accepted qualification, the published macOS Node 24 early-rejection remediation, the current requalification run, the explicit split-store lease-TTL control, the current package/provider evidence, and the remaining production blockers; it is published on `origin/main` with the exact commit recorded in Git history |
| Current-head local evidence revision | `27c96424` for the published fragmented-request hardening; the focused regression passed 30 fail-fast repetitions with no uncaught EPIPE, and the full pinned-oracle HTTP differential passed 40 S3/WebDAV cases; PGlite/R2/FDB/native-mount rows remained explicit skips where prerequisites were absent |
...
| Latest published repository revision | `8f0e7413` (current `origin/main`; W04 test hardening `d955042b` and terminal qualification evidence are ancestors, with unrelated mainline commits preserved) |
...
| Snapshot time | 2026-09-22 08:46 AEST |
| Latest current-tip requalification | Run [35664315098](https://github.com/andymac4182/mount-rs/actions/runs/35664315098) was dispatched at published head `27c96424d436de7425ef0a542f88c0afd361a3a9` after the macOS Node 24 early-rejection hardening. At this snapshot, ARM Node job [106546518670](https://github.com/andymac4182/mount-rs/actions/runs/35664315098/job/106546518670), FoundationDB/RustFS [106546518605](https://github.com/andymac4182/mount-rs/actions/runs/35664315098/job/106546518605), Ozone/FoundationDB [106546518617](https://github.com/andymac4182/mount-rs/actions/runs/35664315098/job/106546518617), Windows Rust [106546519187](https://github.com/andymac4182/mount-rs/actions/runs/35664315098/job/106546519187), and Windows Node [106546518899](https://github.com/andymac4182/mount-rs/actions/runs/35664315098/job/106546518899) had started; macOS Node jobs [106546518673](https://github.com/andymac4182/mount-rs/actions/runs/35664315098/job/106546518673) and [106546518714](https://github.com/andymac4182/mount-rs/actions/runs/35664315098/job/106546518714) remained queued. No step or run acceptance is inferred until every required Node job completes and the exact PGlite/restart step passes. |
...
| Current external blocker | The published macOS Node 24 early-rejection remediation `d955042b` has passed local stress and HTTP differential checks, but current-tip hosted confirmation is still running in [35664315098](https://github.com/andymac4182/mount-rs/actions/runs/35664315098); the macOS jobs had not yet completed at this snapshot. Production remains NO-GO pending this hosted evidence plus deployment-specific persistence/backup/rollback, provider scope/performance, observability/runbook/ownership, and release approval. |
| Historical external blocker | The fixed-tip run [35651055621](https://github.com/andymac4182/mount-rs/actions/runs/35651055621) exposed the W01/S3 `s3/multipart-complete-signed-trailer` ETag mismatch before the W04 recovery step; the published multipart allocation-order fix is recorded in the subsequent qualification history. Ubuntu Rust/native-FUSE and Ozone/TiDB/RustFS provider failures, plus the missing `OZONE_IOPS_PASS` marker, remain historical evidence and are not converted into production passes. |
...
differential, and full CI run `35664315098` was dispatched from `27c96424`.
At publication time that run was still active, with the macOS Node jobs not
yet terminal; no queued, skipped, or partial result is promoted to W04 or
