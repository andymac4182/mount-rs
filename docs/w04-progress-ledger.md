...
| Field | Current value |
| --- | --- |
| Snapshot base revision | `113a1fbc` (published split-store lease-TTL configuration and W04 production-policy control; hosted qualification run `35635114595` used the preceding functional tip `d2db74dd`) |
| Ledger publication | This revision records the terminal current-tip qualification, the repaired hosted Ubuntu Rust gate, the explicit split-store lease-TTL control, the current package/provider evidence, and the remaining production blockers; it is published on `origin/main` with the exact commit recorded in Git history |
| Current-head local evidence revision | `113a1fbc`; Rust 1.95 Linux-container `mount::tests` passed 14/14 on the preceding functional tip, and the published TTL chunk passed CLI 46/46, SDK 3/3, provider-matrix CLI 10/3/0 pass/skip/fail, policy fixtures, shared Cargo formatting, and `git diff --check` |
...
| Latest published repository revision | `113a1fbc` (explicit split-store lease-TTL configuration; unrelated mainline commits were rebased and preserved before publication) |
...
| Snapshot time | 2026-09-22 04:54 AEST |
...
| Implementation/local qualification | **Complete for the recorded packet**; the fresh post-fix oracle-enabled W04 gate passed at `6d59d20`, the focused Linux-container FUSE suite passed 14/14 on `d2db74dd`, and the published `113a1fbc` TTL chunk passed its focused CLI/SDK/provider-matrix checks and shared formatting/diff checks. The dedicated W04 PGlite-only launch-config policy and local backup/rollback rehearsal remain green with their explicit shape-only/local boundaries. |
| Hosted/native/provider acceptance | **W04.2, current Node/package gates, and the Ubuntu Rust gate are complete; provider launch scope remains open**: run [35635114595](https://github.com/andymac4182/mount-rs/actions/runs/35635114595) passed all four Node exact PGlite/restart steps, Windows package distribution, five-package aggregation, and clean-consumer smoke. FoundationDB/RustFS [106450522539](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106450522539) passed durable chunk/reopen, service restart, N-API, VFS restart, and cleanup markers. Ozone lifecycle checks completed and cleanup passed, but all three hard IOPS rows failed; TiDB/RustFS also failed a native Node mount assertion. The run is therefore not a release pass. |
| Production rollout decision | **NO-GO**: the demo was successful and the credential-free PGlite-only config policy now includes a bounded, explicit lease-TTL control, but production readiness still requires hosted policy evidence plus artifact, deployment durability, operational, and rollback gates |
| Current external blocker | Current run [35635114595](https://github.com/andymac4182/mount-rs/actions/runs/35635114595) recorded Ozone lifecycle IOPS of 47.48/57.85 in `ozone-compositions`, 15.37 in `ozone-tidb`, and 32.86 in `ozone-foundationdb`, each below the hard 1,000 target despite successful cleanup. TiDB/RustFS [106450522915](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106450522915) failed its native Node mount assertion after a zero-error provider soak, and W26 evidence [106454477027](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106454477027) failed on the missing `OZONE_IOPS_PASS` marker. The prior four Ubuntu FUSE failures are resolved by `d2db74dd`; production remains NO-GO pending the provider launch-scope decision or passing performance evidence, deployment-specific persistence/backup/rollback, observability/runbook/ownership, and release approval. |
...
| --- | --- | ---: | --- | --- | ---: | --- |
| W04 implementation packet and deterministic local qualification | Complete — current-tip Node/package/Rust candidate green; provider/operations follow-up open | 100% implementation / 100% current Node/package/Rust qualification | The current packet includes the socket server, cleanup/fencing fixes, version metadata, mount-free VFS, native hosting tests, the published fixture-shutdown EPIPE fixes `bdfcb11` and `41d5514`, the privileged-FUSE rootmode correction, the published test-framing fix `d2db74dd`, and explicit split-store `lease_ttl_ms` configuration published as `113a1fbc`; oracle-enabled local qualification remained green, the Linux-container FUSE suite passed 14/14, and run `35635114595` passed all four Node exact PGlite/restart steps plus the Ubuntu Rust workspace gate. | Decide launch provider scope and complete deployment-specific persistence/rollback and operations evidence; the terminal provider IOPS and TiDB/RustFS native-mount failures remain external/performance/provider gates. | 0.25–1h engineering; hosted/provider wait external | Engineering plus hosted gate |
| Hosted Linux Node acceptance | Complete — current-tip confirmation | 100% | Run [35635114595](https://github.com/andymac4182/mount-rs/actions/runs/35635114595), job [106450522757](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106450522757), completed successfully; the exact PGlite/restart and fragmented early-rejection steps passed, with PGlite-inclusive trace and explicit provider skips. | None for the hosted Linux W04.2 gate. | 0h | Hosted gate |
| Hosted macOS-latest Node acceptance | Complete — current-tip confirmation | 100% | Run [35635114595](https://github.com/andymac4182/mount-rs/actions/runs/35635114595), job [106450522868](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106450522868), completed successfully at `2026-09-21T18:07:05Z`; the exact PGlite/restart and fragmented early-rejection steps passed, the log recorded `providersFailed: 0`, PGlite/chunked-PGlite trace PASS, `PGLITE_BACKUP_RESTORE_ROLLBACK_PASS`, and storage artifact `10656542212`. | None for the hosted macOS-latest W04.2 gate. | 0h | Hosted/native gate |
| Hosted macOS-15-intel Node acceptance | Complete — current-tip confirmation | 100% | Run [35635114595](https://github.com/andymac4182/mount-rs/actions/runs/35635114595), job [106450522845](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106450522845), completed successfully at `2026-09-21T18:13:03Z`; the exact PGlite/restart step passed after the long Intel trace at `18:12:45Z`, early rejection passed, the log recorded `providersFailed: 0`, PGlite/chunked-PGlite trace PASS, `PGLITE_BACKUP_RESTORE_ROLLBACK_PASS`, and storage artifact `10656562978`; the historical socket EPIPE did not recur. | None for the hosted macOS-15-intel W04.2 gate. | 0h | Hosted/native gate |
...
| W04 production-candidate hosted qualification | Partial — current Node/package/Ubuntu-Rust/FDB-RustFS gates green; Ozone performance and TiDB/RustFS native-mount gates open | 95% | Current-tip run [35635114595](https://github.com/andymac4182/mount-rs/actions/runs/35635114595) passed Linux [106450522757](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106450522757), ARM [106450522980](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106450522980), macOS-latest [106450522868](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106450522868), and macOS-15-intel [106450522845](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106450522845) exact PGlite/restart steps; the macOS logs show PGlite/chunked-PGlite trace PASS, `providersFailed: 0`, backup/restore/rollback PASS, and current storage artifacts. Windows package [106450522875](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106450522875), aggregate/package/clean-consumer [106456609397](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106456609397), Ubuntu Rust [106450522426](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106450522426), and FoundationDB/RustFS [106450522539](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106450522539) passed. The overall run remains failure because Ozone measured 47.48/57.85, 15.37, and 32.86 IOPS against the hard 1,000 target and TiDB/RustFS [106450522915](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106450522915) failed its native Node mount assertion after a zero-error provider soak; W26 [106454477027](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106454477027) correctly failed on the missing Ozone marker. | Decide whether Ozone and TiDB/RustFS are in launch scope; if retained, remediate and requalify their gates; if excluded, obtain release-owner approval and preserve the explicit boundary. Retain NO-GO until production gates close. | 1–4h engineering/scope work plus hosted/provider wait |
| W04 credential-free PGlite-only launch-config policy | Complete — implementation and terminal current-tip dedicated hosted workflow | 100% implementation / 100% current-tip policy workflow | `scripts/verify-w04-pglite-production-config.mjs` validates an absolute normalized mountpoint, splitstore PGlite metadata/blocks, durable providers, external `MOUNT_RS_PGLITE_URL` references, distinct scoped volume keys, bounded chunking, a positive safe-integer `lease_ttl_ms` no greater than 24 hours when supplied, owner metadata, and fail-closed unknown/inline-secret cases. The positive, non-durable, inline-secret, and invalid-TTL fixtures passed locally; the positive fixture passed the CLI `--check` parser without opening a provider. The current-tip dedicated non-cancelling run [35629439034](https://github.com/andymac4182/mount-rs/actions/runs/35629439034), job [106431854955](https://github.com/andymac4182/mount-rs/actions/runs/35629439034/job/106431854955), predates the TTL field and remains the last hosted policy result; the new policy behavior is locally verified on `113a1fbc` and awaits a current-tip hosted policy run. | Bind the actual PGlite server data directory, version policy, backup/restore, rollback, RPO/RTO, and owners to a deployment; this policy is shape-only. Run the dedicated policy workflow on `113a1fbc` before treating the new TTL control as hosted evidence. | 0.25h evidence retention; hosted/deployment review external | Production config gate |
| W04 explicit split-store lease TTL configuration | Complete — implementation and local qualification; hosted confirmation pending | 100% implementation / 100% focused local qualification / 0% current-tip hosted | Commit `113a1fbc` adds optional `driver.storage.lease_ttl_ms` to the shared Rust and Node CLI schema, preserves the 30-second default, maps the value to the public SDK/N-API chunked driver, and documents the recovery tradeoff. Evidence: mount-rs CLI 46/46, SDK 3/3, Node CLI reopen with `120000`, provider-matrix CLI 10 pass / 3 explicit skips / 0 failures, positive policy fixture with `120000`, and invalid zero-TTL fixture rejection; `cargo fmt --check` and `git diff --check` passed. | Run the dedicated hosted policy workflow at the new tip, select and record the deployment-specific TTL against measured provider latency and stale-writer recovery objectives, and include it in the production configuration/recovery drill. | 0.5–1h hosted/documentation; deployment decision external | Engineering plus hosted/deployment gate |
...
| W04 macOS early-rejection socket EPIPE guard | Complete — current hosted confirmation | 100% | Current Intel job [106450522845](https://github.com/andymac4182/mount-rs/actions/runs/35635114595/job/106450522845) passed fragmented early rejection and the exact PGlite/restart step after commit `41d5514`; the long trace and PGlite run completed without the historical underlying-socket EPIPE. | None for the published socket guard; retain the hosted log in the release record. | 0h remaining |
...
| Explicit split-store lease TTL configuration | Published in `113a1fbc`; focused local qualification complete, hosted policy rerun pending | Engineering/configuration gate; deployment owner must select the value and verify provider-latency and stale-writer recovery objectives |
| Credential-free PGlite-only launch-config policy | Complete; current-tip dedicated run `35629439034` passed; 0.25h evidence retention remaining | Hosted policy shape is green, but no provider connection, production data directory, backup, or owner approval was used |
...
This ledger revision records the W04.2 closure and superseding current-tip
qualification run `35635114595` at published revision `d2db74dd`, plus the
published production-config control at `113a1fbc`. All four Node
...
launch-config policy passed its dedicated non-cancelling workflow; the new
positive/invalid-TTL policy fixtures and local CLI/SDK qualification pass on
`113a1fbc`, while a current-tip hosted policy rerun remains pending. The local
