...
| Field | Current value |
| --- | --- |
| Snapshot base revision | `ff7ef998` (current `origin/main` at this ledger update; includes the published W04 peer-fault test hardening `093565d9`, terminal qualification evidence, and concurrent unrelated mainline work) |
...
| Latest published repository revision | `ff7ef998` (current `origin/main`; W04 peer-fault hardening `093565d9` and terminal qualification evidence are ancestors, with unrelated mainline commits preserved) |
...
| Snapshot time | 2026-09-22 06:49 AEST |
...
| W04 production-candidate hosted qualification | Partial — accepted W04 packet is green, but fixed-tip requalification is terminally mixed and the production candidate remains blocked | 90% | Accepted run [35635114595](https://github.com/andymac4182/mount-rs/actions/runs/35635114595) passed all four Node exact PGlite/restart steps, Windows package distribution, aggregate/clean-consumer smoke, Ubuntu Rust, and FoundationDB/RustFS. Later fixed-tip run [35651055621](https://github.com/andymac4182/mount-rs/actions/runs/35651055621) ended terminal failure: all four Unix Node jobs hit the same W01/S3 multipart-complete ETag mismatch, Ubuntu Rust/native-FUSE and Ozone/TiDB/RustFS provider gates failed, aggregate-native was skipped, and W26 failed closed on the missing marker. | Keep the candidate NO-GO; assign the W01/S3 parity regression and native/provider failures to their owning workstreams, decide whether Ozone/TiDB/RustFS are in launch scope, and requalify the exact release revision before any production approval. | 1–4h engineering/scope work plus hosted/provider wait |
