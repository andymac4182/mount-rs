...
| Field | Current value |
| --- | --- |
| Snapshot base revision | `816107f6` (current `origin/main` at this ledger update; includes the published W04 peer-fault test hardening `093565d9` and concurrent unrelated mainline work) |
...
| Current-head local evidence revision | `093565d9` for the W04 N-API test hardening; after a shared-target release rebuild, the full pinned-oracle `pnpm --dir integrations/mount-rs-napi test` passed, including both structural server runs, P9/NFS/WebDAV/S3 parity, distribution and clean-consumer checks; PGlite/R2/FDB/native-mount rows remained explicit skips where prerequisites were absent |
...
| Latest published repository revision | `816107f6` (current `origin/main`; W04 peer-fault hardening `093565d9` is an ancestor and was rebased with unrelated mainline commits preserved) |
...
| Snapshot time | 2026-09-22 06:35 AEST |
...
| Current fixed-tip hosted qualification | Manual run [35651055621](https://github.com/andymac4182/mount-rs/actions/runs/35651055621) was dispatched from head `3589a142964da53e962239fc2cee6480a7fb6e93` (which contains `093565d9`). Windows job [106503290630](https://github.com/andymac4182/mount-rs/actions/runs/35651055621/job/106503290630) completed successfully at `2026-09-21T20:31:35Z`: its exact N-API server integration, S3 restart/multipart recovery, structural lifecycle, package distribution, clean-consumer, and artifact aggregation checks passed; PGlite/R2/FDB/native-mount rows remained explicit skips where prerequisites were absent. The overall run is still in progress and is not a full acceptance: Ubuntu Rust [106503290545](https://github.com/andymac4182/mount-rs/actions/runs/35651055621/job/106503290545) failed the FUSE forced-unmount deadline test at `404.993546ms`, native FUSE [106503290699](https://github.com/andymac4182/mount-rs/actions/runs/35651055621/job/106503290699) has two native-unmount timeouts, ARM Node [106503290868](https://github.com/andymac4182/mount-rs/actions/runs/35651055621/job/106503290868) failed `s3/multipart-complete-signed-trailer` parity, and Linux Node [106503290885](https://github.com/andymac4182/mount-rs/actions/runs/35651055621/job/106503290885) also failed. macOS Node and provider jobs remain in progress; no full-run or release acceptance is claimed. |
...
14. [x] Diagnose and harden the Windows structural N-API peer-fault test. The
    ...
    locally; fixed-tip hosted confirmation is run `35651055621`, job
    `106503290630`, and remains pending at this snapshot.
...
`35651055621` / Windows job `106503290630` now passes its exact server,
restart, structural, package, clean-consumer, and artifact checks. The overall
run remains in progress with separate Ubuntu Rust/native-FUSE and Linux/ARM
Node failures, and macOS/provider jobs are not yet terminal; no full-run or
