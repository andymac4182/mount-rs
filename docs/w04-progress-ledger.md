...
| Field | Current value |
| --- | --- |
| Snapshot base revision | `d2db74dd1c647ef5b6eebcb0790be4bdd87b3c3f` (published W04 FUSE test-framing fix; qualification run `35635114595` used this exact shared tip) |
| Ledger publication | This revision records the terminal current-tip qualification, the repaired hosted Ubuntu Rust gate, the current package/provider evidence, and the remaining production blockers; it is published on `origin/main` with the exact commit recorded in Git history |
| Current-head local evidence revision | `d2db74dd`; Rust 1.95 Linux-container `mount::tests` passed 14/14, shared Cargo formatting and `git diff --check` passed, and the same revision passed the hosted Ubuntu Rust gate |
...
| Latest published repository revision | `d2db74dd` (W04 FUSE test-framing fix; unrelated mainline commits were rebased and preserved before publication) |
...
| Snapshot time | 2026-09-22 04:17 AEST |
...
| Implementation/local qualification | **Complete for the recorded packet**; the fresh post-fix oracle-enabled W04 gate passed at `6d59d20`, the focused Linux-container FUSE suite passed 14/14 on `d2db74dd`, and the shared formatting/diff checks passed. The dedicated W04 PGlite-only launch-config policy and local backup/rollback rehearsal remain green with their explicit shape-only/local boundaries. |
...
qualification run `35635114595` at published revision `d2db74dd`. All four Node
...
launch-config policy passed its dedicated non-cancelling workflow; the local
