...
| Field | Current value |
| --- | --- |
| Snapshot base revision | `00e7169` (`origin/main` at this ledger refresh; the latest published W04 FUSE fix is `a55bd64` plus concurrent privileged-metadata correction `aa3dae3`, and the isolated candidate source `a2fda11` is retained as historical evidence) |
| Ledger publication | This current ledger revision is published on `origin/main`; the exact commit is recorded in Git history |
| Current-head local evidence revision | `a55bd64` (published privileged-FUSE `rootmode` mask; focused locked `mount-rs-fuse` tests and formatting passed locally) |
...
| Latest published repository revision | `00e7169` (latest `origin/main` tip at this ledger refresh; W04 fixes and provider lockfile updates are in its history, with unrelated concurrent commits preserved) |
| Qualification evidence revision | Corrected isolated candidate source `a2fda1167d2dd26e57924f3c75d122255b323422` on `andymac4182/c/w04-provider-requalification-latest-20260922`; run [35615617217](https://github.com/andymac4182/mount-rs/actions/runs/35615617217) is terminal overall **failure**: Linux, ARM, and macOS-latest Node gates passed, Intel failed before PGlite on an underlying-socket EPIPE, Windows packaging passed, aggregate-native was skipped, and provider gates include one performance threshold failure plus one pre-fix configuration failure. The subsequent current-head run [35617849558](https://github.com/andymac4182/mount-rs/actions/runs/35617849558) remains queued at run level with partial jobs and is not evidence. Rootmode run [35619576365](https://github.com/andymac4182/mount-rs/actions/runs/35619576365) reached Node distribution on Linux, ARM, macOS, and Windows but failed the pre-PGlite export assertion because it predates concurrent package/export commits; it is excluded from acceptance. The published combined tip has fresh qualification pair CI [35620669973](https://github.com/andymac4182/mount-rs/actions/runs/35620669973) and Fault Injection [35620669906](https://github.com/andymac4182/mount-rs/actions/runs/35620669906), currently queued and supplying no acceptance evidence yet |
| Snapshot time | 2026-09-22 01:41 AEST |
...
| W04 production-candidate hosted qualification | Partial — corrected requalification blocked before Intel PGlite; provider/performance gates open | 90% | Prior run [35610813385](https://github.com/andymac4182/mount-rs/actions/runs/35610813385) passed all five Node jobs plus aggregate/package/clean-consumer validation. Corrected run [35615617217](https://github.com/andymac4182/mount-rs/actions/runs/35615617217) passed Linux [106385420039](https://github.com/andymac4182/mount-rs/actions/runs/35615617217/job/106385420039), ARM [106385419798](https://github.com/andymac4182/mount-rs/actions/runs/35615617217/job/106385419798), and macOS-latest [106385419751](https://github.com/andymac4182/mount-rs/actions/runs/35615617217/job/106385419751) exact PGlite/restart steps, but Intel [106385419824](https://github.com/andymac4182/mount-rs/actions/runs/35615617217/job/106385419824) failed with `Error: write EPIPE` in the early-rejection lane before PGlite; aggregate-native [106391712172](https://github.com/andymac4182/mount-rs/actions/runs/35615617217/job/106391712172) was skipped. The socket-level guard is published as `41d5514`; current-head run [35617849558](https://github.com/andymac4182/mount-rs/actions/runs/35617849558) is still queued at run level with partial jobs, and rootmode-fix run [35619576365](https://github.com/andymac4182/mount-rs/actions/runs/35619576365) failed Node distribution before PGlite because it predates current package/export commits. Current-head run [35620669973](https://github.com/andymac4182/mount-rs/actions/runs/35620669973) is queued and is the next valid candidate. | Require the current-head run's Intel PGlite step, aggregate/package validation, corrected FoundationDB/RustFS provider outcome, and remaining provider-scope decisions; retain NO-GO until the production matrix closes. | 0.5–2h scope/approval plus hosted/provider wait |
...
| W04 privileged Linux FUSE `rootmode` boundary | Implementation complete; hosted requalification pending | 100% implementation / 0% current hosted qualification | FoundationDB/RustFS job [106393065909](https://github.com/andymac4182/mount-rs/actions/runs/35617849558/job/106393065909) reached the real configuration-driven mount and failed `mount(2)` with `EINVAL`; inspection of the driver stat and Linux/libfuse contract showed the full mode (permissions plus `S_IFMT`) was serialized as `rootmode`. Commit `a55bd64` masks `rootmode` to `S_IFMT`; concurrent commit `aa3dae3` keeps helper metadata out of privileged mount data, and `9ef982d` corrects its Linux-only expectation. macOS formatting/focused tests and Linux-target `cargo check --tests` passed; Linux runtime mount evidence remains hosted. Rootmode run [35619576365](https://github.com/andymac4182/mount-rs/actions/runs/35619576365) is excluded at the Node distribution gate because it predates current package/export commits. | Inspect current-head run [35620669973](https://github.com/andymac4182/mount-rs/actions/runs/35620669973)'s FoundationDB/RustFS job and require its config mount, reopen, restart, and cleanup markers to pass before closing this gate. | 0.25–0.75h engineering complete; hosted native/provider wait external |
...
11. [ ] Complete current-head run `35620669973` from the published combined
    tip `00e7169`; require Linux, ARM, macOS-latest, and macOS-15-intel exact
    PGlite/restart steps, Windows packaging, aggregate-native, clean-consumer,
    and corrected provider evidence before treating the current candidate as
    qualified. Runs `35617849558` and `35619576365` are partial/stale and are
    retained only as diagnostic evidence. Queued, skipped, cancelled, partial,
    or pre-fix evidence does not count.
...
| Privileged Linux FUSE `rootmode` correction and hosted confirmation | Implementation complete; 0.25–0.75h engineering completed; run `35620669973` queued | Engineering fix plus external native/provider gate; `a55bd64` masks `rootmode` to `S_IFMT`, `aa3dae3` separates privileged helper metadata, and local focused/Linux-target compile checks pass |
...
evidence from run `35615617217`, the socket-level EPIPE fix `41d5514`, the
privileged-FUSE `rootmode` correction `a55bd64`, the concurrent privileged
metadata correction `aa3dae3`, and the assertion alignment `9ef982d`. It
records the stale pre-PGlite distribution failure in `35619576365` and the
current-head qualification `35620669973`. It is reviewed with
`git diff --check` and the relevant formatting check, then committed and
pushed to `origin/main`. It does **not** approve production: the fresh Intel
requalification, aggregate/package evidence, Ozone performance threshold,
