...
| Field | Current value |
| --- | --- |
| Snapshot base revision | `113724487e9efc172ab69254d995377cfcfab296` (origin/main immediately before this ledger update) |
...
| Latest full W04 gate revision | `9a86c00194438029437cbc94342f3c5258d08494` (published `origin/main` tree; full `scripts/test-pglite.sh` exited 0) |
| Snapshot time | 2026-09-21 18:18 AEST / 2026-09-21 08:18 UTC |
...
| Implementation/local qualification | **Complete for the recorded packet**; the fresh W04 gate passed at `9a86c00`, and synced workspace tests plus scoped W04 Clippy passed at `1137244` |
...
| W04.1 full `scripts/test-pglite.sh` local gate | Complete — local qualification | 100% | The latest run at `9a86c00194438029437cbc94342f3c5258d08494` exited 0 and passed slot cleanup, injected failure, provider parity, reconnect, fencing, cancellation, disk restart, mixed stores, Node factories, chunked mounts, and userspace FUSE. R2 and TiDB/RustFS rows retained explicit skips. The synced workspace test gate also exited 0 at `1137244`; scoped PGlite/N-API Clippy passed with `-D warnings`. | No further local W04.1 action unless W04.2 exposes a regression. | 0h remaining |
| Fresh current-tree rerun, SDK/CLI users, upstream, and trace lanes | Complete — local qualification | 100% | The latest current-tree `scripts/test-pglite.sh` run exited 0 with Rust SDK `pass=6 skip=3 fail=0`, Node SDK `pass=5 skip=3 fail=0`, and CLI `pass=12 skip=2 fail=0`; N-API PGlite, factory, chunked, and FUSE integration all passed. The run did not execute upstream/trace lanes because `MOUNTX_SOURCE` was unset, and credential/service-gated rows remained explicit skips. Historical upstream/trace passes remain recorded separately; hosted/provider claims are not inferred from this local result. | None for the current local result; hosted W04.2 remains separate. | 0h remaining |
