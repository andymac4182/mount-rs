...
| Field | Current value |
| --- | --- |
| Snapshot base revision | `41e9ff3` (`origin/main` before this CI-run provenance update; W04 code fix remains `bdfcb11`) |
...
| Latest published repository revision | `41e9ff3` (published W04 production-readiness ledger; W04 code fix remains `bdfcb11`) |
| Snapshot time | 2026-09-21 20:19 AEST / 2026-09-21 10:19 UTC |
...
| Hosted/native/provider acceptance | **In progress**: latest published-revision run [35587575994](https://github.com/andymac4182/mount-rs/actions/runs/35587575994) has Linux, macOS-latest, and macOS-15-intel Node jobs running; provider credentials/services remain explicit gates |
...
| Current external blocker | Hosted execution is in progress for Node jobs [106294483397](https://github.com/andymac4182/mount-rs/actions/runs/35587575994/job/106294483397), [106294483486](https://github.com/andymac4182/mount-rs/actions/runs/35587575994/job/106294483486), and [106294483435](https://github.com/andymac4182/mount-rs/actions/runs/35587575994/job/106294483435). The prior run's Intel failure was an EPIPE test-harness race fixed and published as `bdfcb11`; the latest published-revision run is the active closure source. |
...
| Latest hosted Linux Node acceptance | In progress | 25% | Run [35587575994](https://github.com/andymac4182/mount-rs/actions/runs/35587575994), job [106294483397](https://github.com/andymac4182/mount-rs/actions/runs/35587575994/job/106294483397), has started on the published documentation/requalification revision. | Inspect the completed log and require the exact `Verify PGlite integration and restart recovery` step to pass; record benchmark/trace and artifact results. | 0.25–0.5h after completion | Hosted gate |
| Latest hosted macOS-latest Node acceptance | In progress | 25% | Run [35587575994](https://github.com/andymac4182/mount-rs/actions/runs/35587575994), job [106294483486](https://github.com/andymac4182/mount-rs/actions/runs/35587575994/job/106294483486), has started on the published documentation/requalification revision. | Inspect the completed log and require the exact PGlite recovery step to pass; retain the platform-specific native mount and consumer evidence. | 0.25–0.5h after completion | Hosted/native gate |
| Latest hosted macOS-15-intel Node acceptance | In progress | 25% | Run [35587575994](https://github.com/andymac4182/mount-rs/actions/runs/35587575994), job [106294483435](https://github.com/andymac4182/mount-rs/actions/runs/35587575994/job/106294483435), has started on the published EPIPE fix. The historical Intel failure in run `35560240894` stopped before the PGlite step and therefore remains diagnostic only. | Inspect the completed log for the exact PGlite recovery step and confirm the previous EPIPE race is absent. | 0.25–0.5h after completion | Hosted/native gate |
...
| W04.2 hosted macOS/Linux reconnect acceptance | **Open — latest external hosted gate in progress** | **25% provisionally** | Latest published-revision run [35587575994](https://github.com/andymac4182/mount-rs/actions/runs/35587575994) is running on the documentation/requalification revision: Linux Node job [106294483397](https://github.com/andymac4182/mount-rs/actions/runs/35587575994/job/106294483397), macOS-latest Node job [106294483486](https://github.com/andymac4182/mount-rs/actions/runs/35587575994/job/106294483486), and macOS-15-intel Node job [106294483435](https://github.com/andymac4182/mount-rs/actions/runs/35587575994/job/106294483435) have started. Historical run [35560240894](https://github.com/andymac4182/mount-rs/actions/runs/35560240894) is not closure evidence: macOS-latest passed the PGlite step, but macOS-15-intel failed earlier in `test-http-early-rejection.mjs` with `EPIPE`, before the required PGlite step. | Inspect all three latest logs after completion and require the exact `Verify PGlite integration and restart recovery` step to pass. If any fails, isolate the failure, patch the smallest evidence-backed root cause, run focused and relevant W04 gates, and publish the next fix before requalifying. If all pass, update W04.2 and the production ledger, run formatting/diff checks, commit, rebase if needed, and push. | 0.25–0.5h log audit after completion; 0.5–1h tracker/publish work if green; 2–6h if a code regression requires remediation. Hosted runner time is external and is not engineering time. |
...
- In the historical run `35560240894`, macOS-latest eventually passed the
  ...
  Intel. The EPIPE listener fix is published as `bdfcb11`, and latest run
  `35587575994` is the requalification source for all three Node platforms.
...
1. Monitor latest run `35587575994`, jobs `106294483397`, `106294483486`, and
   `106294483435`, for status transitions.
