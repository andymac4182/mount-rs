...
| Field | Current value |
| --- | --- |
| Snapshot base revision | `68c087d1` (published split-store lease-TTL configuration and explicit production-policy enforcement; hosted qualification run `35635114595` used the preceding functional tip `d2db74dd`) |
...
| Current-head local evidence revision | `68c087d1`; Rust 1.95 Linux-container `mount::tests` passed 14/14 on the preceding functional tip, and the published TTL/policy chunk passed CLI 46/46, SDK 3/3, provider-matrix CLI 10/3/0 pass/skip/fail, positive plus four negative policy fixtures, shared Cargo formatting, and `git diff --check` |
...
| Latest published repository revision | `68c087d1` (explicit production lease-TTL enforcement on top of `113a1fbc`; unrelated mainline commits were rebased and preserved before publication) |
...
| Snapshot time | 2026-09-22 05:08 AEST |
