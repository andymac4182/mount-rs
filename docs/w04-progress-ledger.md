...
| Field | Current value |
| --- | --- |
| Snapshot base revision | `87db9fd` (current `origin/main` at this evidence refresh; it includes the published W04 policy and recovery chunks plus concurrent mainline commits; qualification run `35620669973` used the earlier isolated source `f54590c`) |
| Ledger publication | This current ledger revision records the W04 PGlite-only launch-config policy and its terminal hosted policy-step evidence; it is published on `origin/main`, with the exact commit recorded in Git history |
...
| Latest published repository revision | `87db9fd` (current `origin/main` tip before this evidence publication; W04 chunks and unrelated concurrent commits are preserved) |
...
| Snapshot time | 2026-09-22 02:45 AEST |
...
| Production-like persistence, restart, and version recovery | Hosted candidate qualification complete; hosted config policy and local backup/rollback rehearsal green; deployment evidence pending | 90% hosted qualification / 50% config-and-local rehearsal / 0% production drill | Current run [35620669973](https://github.com/andymac4182/mount-rs/actions/runs/35620669973) passed all four exact PGlite/restart steps and PGlite-inclusive five-seed/eight-backend traces; FoundationDB/RustFS [106402628095](https://github.com/andymac4182/mount-rs/actions/runs/35620669973/job/106402628095) passed durable chunk/reopen, service restart, configured FUSE, and cleanup markers. The policy step in [35626892113](https://github.com/andymac4182/mount-rs/actions/runs/35626892113), job [106423435077](https://github.com/andymac4182/mount-rs/actions/runs/35626892113/job/106423435077), emitted `W04_PGLITE_PRODUCTION_CONFIG_POLICY_PASS` plus the expected non-durable and inline-secret rejection markers; the enclosing run was cancelled and is not a full CI pass. The focused lifecycle file passed both graceful restart and `PGLITE_BACKUP_RESTORE_ROLLBACK_PASS`; these checks demonstrate configuration shape and local control flow, not the actual production deployment. | Bind the actual PGlite server data directory/configuration and version policy, and execute an approved encrypted backup/restore and rollback drill with measured RPO/RTO and owner approval. | 1–2h engineering plus deployment-owner wait | Engineering plus deployment decision |
...
| W04 credential-free PGlite-only launch-config policy | Complete — implementation and exact hosted policy step; dedicated non-cancelling workflow added | 100% implementation / 100% policy step; dedicated workflow pending | `scripts/verify-w04-pglite-production-config.mjs` validates an absolute normalized mountpoint, splitstore PGlite metadata/blocks, durable providers, external `MOUNT_RS_PGLITE_URL` references, distinct scoped volume keys, bounded chunking, owner metadata, and fail-closed unknown/inline-secret cases. The positive and two negative fixtures passed locally, the positive fixture passed the CLI `--check` parser without opening a provider, and CI job [106423435077](https://github.com/andymac4182/mount-rs/actions/runs/35626892113/job/106423435077) emitted `W04_PGLITE_PRODUCTION_CONFIG_POLICY_PASS`, `metadata.durable-must-be-true`, and `connection.password-must-not-be-inline`. The overall run was cancelled during unrelated Ozone work and is not a full CI/release pass. The dedicated `.github/workflows/w04-production-policy.yml` now repeats this static gate with `cancel-in-progress: false`; its terminal hosted run is still pending. | Retain a terminal dedicated policy-workflow result, then bind the actual PGlite server data directory, version policy, backup/restore, rollback, RPO/RTO, and owners to a deployment; this policy is shape-only. | 0.25h workflow/evidence retention; deployment review external | Production config gate |
...
| Credential-free PGlite-only launch-config policy | Complete implementation; dedicated non-cancelling workflow pending; 0.25h evidence retention | Hosted step is bounded evidence only because enclosing CI run `35626892113` was cancelled; no provider connection or secret was used |
...
unrelated Ozone work and is not a full CI/release pass. It also records the local disk-backed
