...
| Field | Current value |
| --- | --- |
| Snapshot base revision | `85cd4ff` (`origin/main` at this ledger refresh; the latest published W04 FUSE fix is `a55bd64`, and the isolated candidate source `a2fda11` is retained as historical evidence) |
...
11. [ ] Complete current-head run `35617849558` from published `41d5514` and
    rootmode-fix run `35619576365` from published `a55bd64`; require Linux,
    ARM, macOS-latest, and macOS-15-intel exact PGlite/restart steps, Windows
    packaging, aggregate-native, clean-consumer, and corrected provider
    evidence before treating the current candidate as qualified. Queued,
    skipped, cancelled, partial, or pre-fix evidence does not count.
...
| Privileged Linux FUSE `rootmode` correction and hosted confirmation | Implementation complete; 0.25–0.75h engineering completed; run `35619576365` queued | Engineering fix plus external native/provider gate; `a55bd64` masks `rootmode` to `S_IFMT`, and local focused tests pass |
...
evidence from run `35615617217`, the socket-level EPIPE fix `41d5514`, and the
privileged-FUSE `rootmode` correction `a55bd64`. It is reviewed with
