...
| Field | Current value |
| --- | --- |
| Snapshot base revision | `2aaae52` (`origin/main` at this ledger refresh; it includes the published W04 FUSE/rootmode packet, the concurrent transport-hook work, and later mainline commits; qualification run `35620669973` used the earlier isolated source `f54590c`) |
...
| Latest published repository revision | `2aaae52` (latest `origin/main` tip at this ledger refresh; W04 fixes and provider lockfile updates are in its history, with unrelated concurrent commits preserved) |
...
| Snapshot time | 2026-09-22 02:13 AEST |
...
| Production-like persistence, restart, and version recovery | Hosted candidate qualification complete; deployment policy pending | 90% hosted qualification / 0% deployment policy | Current run [35620669973](https://github.com/andymac4182/mount-rs/actions/runs/35620669973) passed all four exact PGlite/restart steps and PGlite-inclusive five-seed/eight-backend traces; FoundationDB/RustFS [106402628095](https://github.com/andymac4182/mount-rs/actions/runs/35620669973/job/106402628095) passed durable chunk/reopen, service restart, configured FUSE, and cleanup markers. These demonstrate the candidate harness and configured provider paths, not the actual production deployment. | Bind the actual production data directory/configuration, version compatibility, backup/restore expectation, rollback behavior, RPO/RTO, and owner approval to the deployment shape. | 1–2h plus deployment-owner wait | Engineering plus deployment decision |
...
W04 code failure. It is reviewed with `git diff --check` and the relevant
formatting check, then committed and pushed to `origin/main`. It does
