...
| Field | Current value |
| --- | --- |
| Snapshot base revision | `f03595e` (current `origin/main` immediately before this evidence publication; it includes the published W04 policy/recovery chunks and dedicated policy workflow plus concurrent mainline commits; qualification run `35620669973` used the earlier isolated source `f54590c`) |
| Ledger publication | This current ledger revision records the terminal dedicated W04 PGlite policy workflow evidence; it is published on `origin/main`, with the exact commit recorded in Git history |
...
| Latest published repository revision | `f03595e` (current `origin/main` tip before this evidence publication; W04 chunks and unrelated concurrent commits are preserved) |
...
| Snapshot time | 2026-09-22 02:50 AEST |
...
| Production-like persistence, restart, and version recovery | Hosted candidate qualification complete; dedicated hosted config policy and local backup/rollback rehearsal green; deployment evidence pending | 90% hosted qualification / 60% config-and-local rehearsal / 0% production drill | Current run [35620669973](https://github.com/andymac4182/mount-rs/actions/runs/35620669973) passed all four exact PGlite/restart steps and PGlite-inclusive five-seed/eight-backend traces; FoundationDB/RustFS [106402628095](https://github.com/andymac4182/mount-rs/actions/runs/35620669973/job/106402628095) passed durable chunk/reopen, service restart, configured FUSE, and cleanup markers. The dedicated non-cancelling policy run [35627650073](https://github.com/andymac4182/mount-rs/actions/runs/35627650073), job [106425838566](https://github.com/andymac4182/mount-rs/actions/runs/35627650073/job/106425838566), completed successfully and emitted `W04_PGLITE_PRODUCTION_CONFIG_POLICY_PASS` plus the expected non-durable and inline-secret rejection markers. The focused lifecycle file passed both graceful restart and `PGLITE_BACKUP_RESTORE_ROLLBACK_PASS`; these checks demonstrate configuration shape and local control flow, not the actual production deployment. | Bind the actual PGlite server data directory/configuration and version policy, and execute an approved encrypted backup/restore and rollback drill with measured RPO/RTO and owner approval. | 1–2h engineering plus deployment-owner wait | Engineering plus deployment decision |
...
| W04 credential-free PGlite-only launch-config policy | Complete — implementation and terminal dedicated hosted workflow | 100% implementation / 100% dedicated policy workflow | `scripts/verify-w04-pglite-production-config.mjs` validates an absolute normalized mountpoint, splitstore PGlite metadata/blocks, durable providers, external `MOUNT_RS_PGLITE_URL` references, distinct scoped volume keys, bounded chunking, owner metadata, and fail-closed unknown/inline-secret cases. The positive and two negative fixtures passed locally, the positive fixture passed the CLI `--check` parser without opening a provider, and the dedicated non-cancelling run [35627650073](https://github.com/andymac4182/mount-rs/actions/runs/35627650073), job [106425838566](https://github.com/andymac4182/mount-rs/actions/runs/35627650073/job/106425838566), completed successfully with `W04_PGLITE_PRODUCTION_CONFIG_POLICY_PASS`, `metadata.durable-must-be-true`, and `connection.password-must-not-be-inline`. The earlier embedded CI step in [106423435077](https://github.com/andymac4182/mount-rs/actions/runs/35626892113/job/106423435077) is retained as bounded corroboration; its enclosing run was cancelled and is not a full CI/release pass. | Bind the actual PGlite server data directory, version policy, backup/restore, rollback, RPO/RTO, and owners to a deployment; this policy is shape-only. | 0.25h evidence retention; deployment review external | Production config gate |
...
| Credential-free PGlite-only launch-config policy | Complete; dedicated run `35627650073` passed; 0.25h evidence retention remaining | Hosted policy shape is green, but no provider connection, production data directory, backup, or owner approval was used |
...
step `106423435077` passed the positive fixture and both expected rejection
fixtures in run `35626892113`, while the enclosing run was cancelled during
unrelated Ozone work and is not a full CI/release pass. The dedicated
non-cancelling workflow then passed in run `35627650073`, job `106425838566`,
with the same exact markers. It also records the local disk-backed
