...
- Qualify cross-process crash, concurrency, and durability behavior; bounded
  ...
  durability remain outside the supported local claim. The restart gate
  explicitly rejects a pre-crash NFSv3 file handle with `NFS3ERR_STALE` before
  recovering the same bytes through a replacement-server LOOKUP.
...
- `./scripts/cargo-shared test -p mount-rs-nfs --all-targets --locked` — PASS:
  38 unit tests, process restart 1, rootless wire 1, transport concurrency 1,
