...
| 2026-09-22 | NFSv4 channel/state limit parity | Rust `Nfs4StateOptions` and nested N-API `Nfs4StateKnobs` now cover lease seconds, per-client sessions, fore slots, COMPOUND operations, request/replay-cache ceilings, per-file opens/locks, and reclaim policy; the v4 wire suite passes 5/5, including `maxLocksPerFile` enforcement for an existing lock state, `NFS4ERR_TOOSMALL` for an undersized fore response offer, and `NFS4ERR_NOSPC` for an exhausted session cap | Upstream ID-map callbacks, deterministic `now`/`seed`, and session `onError` remain explicit parity gaps; native Linux/hosted lifecycle and crash/durability remain external gates |
