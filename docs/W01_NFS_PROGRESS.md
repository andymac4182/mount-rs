...
- `./scripts/cargo-shared test -p mount-rs-nfs --all-targets --locked` — PASS:
  39 unit tests, process restart 2, rootless wire 1, transport concurrency 2,
  ...
  acceptance.
- `./scripts/cargo-shared test -p mount-rs-nfs --test transport_concurrency --locked` — PASS: 2/2 rootless real-TCP concurrency tests, including a fast NFSv3 `GETATTR` reply completing before a blocked earlier `GETATTR` on the same connection, with both replies matched exactly once.
