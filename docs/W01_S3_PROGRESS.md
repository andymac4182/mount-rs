...
| Faults, cancellation, concurrency and durability | Open | Reset/close, iterator cancellation, crash/power-loss restart, ordering, and provider failure evidence; local multipart Complete/Abort terminal races, failed-Complete retry release, injected assembly-read failure recovery, and native-filesystem process-restart handoff are covered |
...
| 2026-09-22 | Added multipart assembly-read fault recovery | The complete `mount-rs-s3 --all-targets` matrix passed 4 unit, 6 chunked, 23 gateway, and 5 public-API tests; `failed_multipart_assembly_releases_finalization_claim_for_retry` injects one `EIO` while reading a staged part, proves the `.finalizing` claim is released, and completes the same upload successfully on retry | This is bounded local fault-injection evidence, not power-loss/torn-write durability, provider-backed failure, broader ordering/concurrency, live AWS/R2, or native/hosted acceptance; production decision stays NO-GO |
