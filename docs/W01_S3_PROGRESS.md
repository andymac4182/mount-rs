...
| Faults, cancellation, concurrency and durability | Open | Reset/close, iterator cancellation, crash/power-loss restart, ordering, and provider failure evidence; local multipart Complete/Abort terminal races, failed-Complete retry release, injected assembly-read failure recovery, cancellation-safe staged PUT/part replacement, bounded concurrent multipart publication, and native-filesystem process-restart handoff are covered |
