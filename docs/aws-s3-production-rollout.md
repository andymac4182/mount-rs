...
| Release | Locked artifact provenance, security review, load/soak/fault evidence, staged canary, rollback, and post-deploy smoke pass | Open; the follow-up Standard scan at pushed head `da4d36c497e39c5796c571c6df34905b6a6e5340` reports zero reportable findings in the 20 directly reviewed W25 surfaces, with partial coverage of the 590-file repository. The AWS transport override and mutable workflow references from baseline `89992ce3406f3f7586e5b072af488df4b565ea90` are remediated, but hosted OIDC/deployment evidence, repository-coverage follow-up, load/soak/fault/recovery drills, canary, rollback, and post-deploy smoke remain open. The latest hosted run `35597712935` failed before acceptance because `aws-s3-ci` has no variables or secrets and the existing test-role trust allows only the SSO administrator, not GitHub OIDC; an approved IAM trust/environment change is required. Bounded publication, listing, quota/TTL, and backing-file-aware staging cleanup remediations have landed |
