...
| Release | Locked artifact provenance, security review, load/soak/fault evidence, staged canary, rollback, and post-deploy smoke pass | Open; the sealed Standard scan `bb69ddae-798a-4387-bb87-f3e7acd496cb` covers pushed head `227f81932b48fa1fe8b4999ed619add812a88e42` with zero reportable findings in 22 directly reviewed W25 surfaces, while the remaining repository inventory was explicitly deferred. The fresh scan `c6992ddb-3762-4638-b37e-f1399bd77e42` targets `8e271cd24de1458519c9dca61363c427784a3dd3`, not the current pushed head `428ce6db9504223483feee7784824b4804820d00`; its result cannot be used as current-head release evidence. Hosted OIDC/deployment evidence, repository-coverage follow-up, load/soak/fault/recovery drills, canary, rollback, and post-deploy smoke remain open. The latest hosted run [`35610661014`](https://github.com/andymac4182/mount-rs/actions/runs/35610661014) at `428ce6d` fails safely at `AWS_S3_CI_CONFIG_BLOCKED missing_bucket` before AWS authentication; the existing test-role trust still allows only the SSO administrator, not GitHub OIDC, so an approved IAM trust/environment change is required. Bounded publication, listing, quota/TTL, backing-file-aware staging cleanup, and the explicit AWS client retry budget have landed |
