...
| P05 — observability, SLOs and alerting | Open — 15% gate weight; local probe slice passed | The HTTP transport's unauthenticated `/healthz` and `/readyz` contract is locally tested, but exit still requires provider-aware checks where applicable, production collector, dashboards, SLO/error-budget thresholds, paging, retention/redaction and an exercised alert route |
