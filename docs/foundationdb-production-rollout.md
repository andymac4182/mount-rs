...
| P8 — load, capacity, soak and cost envelope | Harness + terminal ten-round cross-platform qualification; production evidence open | The opt-in harness supports bounded repeated real FoundationDB/RustFS composition rounds with unique prefixes and cleanup, and emits p50/p95/p99 operation-latency and throughput markers; latest hosted run `35698630720` at exact revision `a19d37b61fdbb769102df15714cc88a3ff5eea91` recorded base `operations=15 p50_us=2469 p95_us=25063 p99_us=25063 total_ms=71 throughput_ops_per_sec=208.39`; ten isolated durable composition rounds passed with p95/p99 from 13,399µs to 13,906µs and throughput from 225.29 to 258.64 ops/s. The same run validated the 400-iteration, 64-concurrency, 4 KiB profile: 400 successful writes, reads and deletes, 1,200 successful lifecycle operations, measured 467.15 lifecycle IOPS, zero timeouts and zero cleanup failures. Linux artifact `foundationdb-production-qualification-35698630720-1` (ID `10682020359`, SHA-256 `0b8ead4a8cf548322f71cf2a391b036f65ebf8412e52fc7720042487f61fcb11`) and the aggregate packet were retained; the measured rate is bounded qualification, not a capacity target. Production-shaped duration, retry/error budget, resource growth, safe capacity, cost and scaling triggers are still required |
