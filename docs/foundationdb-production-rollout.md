...
| P8 — load, capacity, soak and cost envelope | Harness + five-round local and hosted qualification; fixed workload profile added; measured latency marker verified; production evidence open | The opt-in harness supports bounded repeated real FoundationDB/RustFS composition rounds with unique prefixes and cleanup, and emits p50/p95/p99 operation-latency and throughput markers; latest hosted run `35671492720` recorded five isolated durable composition rounds at revision `9460a62` with base `operations=15 p50_us=3846 p95_us=38409 p99_us=38409 total_ms=96 throughput_ops_per_sec=155.90`; soak p95/p99 ranged from 13,005µs to 14,243µs and throughput from 199.39 to 214.47 ops/s. The dedicated W07 lane now additionally runs and retains a validated 400-iteration, 64-concurrency, 4 KiB profile with measured throughput and a structural non-zero completion floor. Attempt `35673343510` did not qualify because its initial 1,000-IOPS floor measured 29.13 IOPS; the corrected profile requires a fresh terminal run. Production-shaped duration, retry/error budget, resource growth, safe capacity, cost and scaling triggers are still required |
...
| P11 — native client, mount and platform support | Qualification only; hosted Linux Node/CLI/native FUSE evidence is green at `35671492720` for revision `9460a62` | An explicit advertised platform matrix; clean-install, native FDB client, Node/CLI, FUSE/NFS/FSKit lifecycle, concurrent access, restart/recovery and packaging/signing evidence for every advertised platform |
