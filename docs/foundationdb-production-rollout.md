...
| P8 — load, capacity, soak and cost envelope | Harness + five-round local and hosted qualification; corrected fixed workload profile now has terminal published-tip evidence; production evidence open | The opt-in harness supports bounded repeated real FoundationDB/RustFS composition rounds with unique prefixes and cleanup, and emits p50/p95/p99 operation-latency and throughput markers; latest hosted run `35686583792` at exact revision `7ba3998a6f6a88b7eedefd98cba9262980d24ce9` recorded base `operations=15 p50_us=3133 p95_us=32028 p99_us=32028 total_ms=79 throughput_ops_per_sec=187.57`; five isolated durable composition rounds passed with p95/p99 from 12,020µs to 12,490µs and throughput from 227.19 to 235.58 ops/s. The same run validated the 400-iteration, 64-concurrency, 4 KiB profile: 400 successful writes, reads and deletes, 1,200 successful lifecycle operations, measured 166.29 lifecycle IOPS, zero timeouts and zero cleanup failures. Artifact `foundationdb-production-qualification-35686583792-1` (ID `10676804103`, SHA-256 `afccc878e46a096c6153f04fac017f3df19d37d147a2e83629fa14616f850e30`) was retained; the measured rate is bounded qualification, not a capacity target. The prior `35683503910` checkpoint remains retained terminal evidence, and production-shaped duration, retry/error budget, resource growth, safe capacity, cost and scaling triggers are still required |
...
| P11 — native client, mount and platform support | Qualification only; hosted Linux Node/CLI/native FUSE evidence is green at `35686583792` for exact revision `7ba3998a6f6a88b7eedefd98cba9262980d24ce9` | An explicit advertised platform matrix; clean-install, native FDB client, Node/CLI, FUSE/NFS/FSKit lifecycle, concurrent access, restart/recovery and packaging/signing evidence for every advertised platform |
...
at exact revision `7ba3998a6f6a88b7eedefd98cba9262980d24ce9` (job
`106614608451`). It supersedes the earlier `35683503910` checkpoint for
current-tip implementation qualification: the repaired standalone lockfile
passed the seven-case qualification-log verifier, the independent 30-second
authority heartbeat stayed live across staged clients and service restart, the
shared authority stats marker reconciled three successful publications and one
fail-closed reader, the 400-iteration workload measured 166.29 IOPS with
1,200 successful lifecycle operations and zero timeouts/cleanup failures, and
the native Linux Node/CLI/FUSE and durable restart paths passed. This does not
move P8 or P11 to production acceptance: the heartbeat and process-local stats
are bounded qualification instrumentation, not production monitoring or
deployment credentials, and the advertised platform matrix, capacity
envelope, failover, observability, packaging and owner evidence remain open.
...
case, so a fresh current-tip hosted execution remains required to record this
CI result. This is evidence-integrity hardening only; the production packet
