...
| P8 — load, capacity, soak and cost envelope | Harness + five-round local and hosted qualification; corrected fixed workload profile now has terminal published-tip evidence; production evidence open | The opt-in harness supports bounded repeated real FoundationDB/RustFS composition rounds with unique prefixes and cleanup, and emits p50/p95/p99 operation-latency and throughput markers; latest hosted run `35691106828` at exact revision `6b8639aa618f301e8424b5619bc1dda559a75948` recorded base `operations=15 p50_us=2680 p95_us=13759 p99_us=13759 total_ms=58 throughput_ops_per_sec=254.79`; five isolated durable composition rounds passed with p95/p99 from 12,701µs to 13,784µs and throughput from 241.60 to 248.00 ops/s. The same run validated the 400-iteration, 64-concurrency, 4 KiB profile: 400 successful writes, reads and deletes, 1,200 successful lifecycle operations, measured 259.35 lifecycle IOPS, zero timeouts and zero cleanup failures. Artifact `foundationdb-production-qualification-35691106828-1` (ID `10678891553`, SHA-256 `f26ae7cb81dd298cc78153f27e573bdb1096a6ae038cc8f42a40d7cbeeab3c17`) was retained; the measured rate is bounded qualification, not a capacity target. The prior `35688516329` checkpoint remains retained terminal evidence, and production-shaped duration, retry/error budget, resource growth, safe capacity, cost and scaling triggers are still required |
