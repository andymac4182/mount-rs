...
| P8 — load, capacity, soak and cost envelope | Harness + exact-tip terminal ten-round cross-platform qualification; production evidence open | The opt-in harness supports bounded repeated real FoundationDB/RustFS composition rounds with unique prefixes and cleanup, and emits p50/p95/p99 operation-latency and throughput markers; latest hosted run `35700746198` at exact revision `8520e362710a4b3fe00fd567cf00fcc13e64c222` recorded base `operations=15 p50_us=1950 p95_us=24689 p99_us=24689 total_ms=61 throughput_ops_per_sec=242.82`; ten isolated durable composition rounds passed with p95/p99 from 9,727µs to 266,600µs and throughput from 47.11 to 346.34 ops/s, retaining the high-variance sample rather than treating it as capacity. The same run validated the 400-iteration, 64-concurrency, 4 KiB profile: 400 successful writes, reads and deletes, 1,200 successful lifecycle operations, measured 323.68 lifecycle IOPS, zero timeouts and zero cleanup failures. Linux artifact `foundationdb-production-qualification-35700746198-1` (ID `10682882571`, SHA-256 `88f5e05ceb926917e251cfb5d8a949ec2ec6448233e94b9496cbd7d07aafe807`) and the aggregate packet were retained; the measured rate is bounded qualification, not a capacity target. Production-shaped duration, retry/error budget, resource growth, safe capacity, cost and scaling triggers are still required |
...
| P11 — native client, mount and platform support | Qualification only; exact-tip hosted Linux/macOS packet is green at `35700746198` for exact revision `8520e362710a4b3fe00fd567cf00fcc13e64c222` | An explicit advertised platform matrix; clean-install, native FDB client, Node/CLI, FUSE/NFS/FSKit lifecycle, concurrent access, restart/recovery and packaging/signing evidence for every advertised platform. The current macOS result is feature compilation only; live macOS service/cluster/mount, clean-install and signed-package evidence remain open |
