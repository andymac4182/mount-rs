...
| P8 — load, capacity, soak and cost envelope | Harness + five-round local and hosted qualification; corrected fixed workload profile now has terminal hosted evidence; production evidence open | The opt-in harness supports bounded repeated real FoundationDB/RustFS composition rounds with unique prefixes and cleanup, and emits p50/p95/p99 operation-latency and throughput markers; terminal hosted run `35674506961` at exact revision `a7eb3be47e3e5edfd4b3e8a516d1b8cbceaab199` recorded base `operations=15 p50_us=2617 p95_us=88362 p99_us=88362 total_ms=134 throughput_ops_per_sec=111.17`; five isolated durable composition rounds had p95/p99 from 13,388µs to 15,041µs and throughput from 206.94 to 236.23 ops/s. The same run validated the 400-iteration, 64-concurrency, 4 KiB profile: 400 successful writes, reads and deletes, 1,200 successful lifecycle operations, measured 70.83 lifecycle IOPS, zero timeouts and zero cleanup failures. Artifact `foundationdb-production-qualification-35674506961-1` (ID `10672253304`, SHA-256 `e558dea65ac63d87873d36e6bff09daa70f824310309c1c1e6dd87c109b7b5b1`) was retained; the non-zero floor is structural, not a capacity target. Attempt `35673343510` remains recorded as the rejected 1,000-IOPS profile correction signal. Production-shaped duration, retry/error budget, resource growth, safe capacity, cost and scaling triggers are still required |
...
| P11 — native client, mount and platform support | Qualification only; hosted Linux Node/CLI/native FUSE evidence is green at `35674506961` for exact revision `a7eb3be47e3e5edfd4b3e8a516d1b8cbceaab199` | An explicit advertised platform matrix; clean-install, native FDB client, Node/CLI, FUSE/NFS/FSKit lifecycle, concurrent access, restart/recovery and packaging/signing evidence for every advertised platform |
