...
| P11 — native client, mount and platform support | Qualification only; hosted Linux Node/CLI/native FUSE evidence is green at `35681642584` for exact revision `113b13751acc4885ca5916a7f70fe109e1dbadd4` | An explicit advertised platform matrix; clean-install, native FDB client, Node/CLI, FUSE/NFS/FSKit lifecycle, concurrent access, restart/recovery and packaging/signing evidence for every advertised platform |
...
at exact revision `71972b28ca7ae561325342ddf466c8353546547a` (job
`106594973866`). It supersedes the earlier `35675987457` checkpoint for
current-tip implementation qualification: the independent 30-second
authority heartbeat stayed live across staged clients and service restart,
the 400-iteration workload measured 202.31 IOPS with 1,200 successful
lifecycle operations and zero timeouts/cleanup failures, and the native Linux
Node/CLI/FUSE and durable restart paths passed. This does not move P8 or P11
to production acceptance: the heartbeat is a bounded test publisher, not
production clock/cadence telemetry, and the advertised platform matrix,
