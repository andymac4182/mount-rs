...
Native `mount-rs.storage-diagnostics.v2` exports exact decimal strings for live registered split R2 block-store instances. Its scope excludes `Mount.r2`, direct Rust factories, all backing-marker prepare/verify calls, concurrent-prefix probes, qualification/preflight traffic (including calls through the same configured store), reconciliation list streams, and client-internal retries. API invocations are not HTTP attempts, billed requests, wire bytes or physical IOPS. Get-result acquisition and body materialization have separate outcomes; successful body bytes count before integrity validation, including a later digest mismatch. Submitted upload bytes are attempted bytes; confirmed bytes require `put_opts` returning `Ok`. An ambiguous error confirms zero bytes and triggers no hidden replay.
