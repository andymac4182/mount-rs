...
For the current instrumentation controls and measurement boundaries, see
[Finding remote-drive bottlenecks](bottleneck-metrics.md). The measurements below
retain their original workload, format and source scope.
...
## Opt-in object-store API observations

`MOUNT_RS_PROFILE_IO=1` at store construction enables one boxed fixed bank shared by clones of each object-store block store. Existing logical puts, bytes written and cache counters retain their meaning: a coalesced follower or matching conditional conflict can succeed without a new accepted upload. `stats().raw_api` is absent when disabled. The bank records ten fixed method/reason rows, exact leader/follower claim outcomes, method/body in-flight spans and pending claims. Reconciliation listing is unavailable; reconciliation delete `NotFound` is an API error even when logical reconciliation succeeds.

Native `mount-rs.storage-diagnostics.v3` exports exact decimal strings for live registered split R2 block-store instances. The raw API scope excludes `Mount.r2`, direct Rust factories, all backing-marker prepare/verify calls, concurrent-prefix probes, qualification/preflight traffic (including calls through the same configured store), reconciliation list streams, and client-internal retries. API invocations are not HTTP attempts, billed requests, wire bytes or physical IOPS. Get-result acquisition and body materialization have separate outcomes; successful body bytes count before integrity validation, including a later digest mismatch. Submitted upload bytes are attempted bytes; confirmed bytes require `put_opts` returning `Ok`. An ambiguous error confirms zero bytes and triggers no hidden replay.

Raw latency is inclusive wall time at the invoked adapter await after path/payload preparation. Counts and elapsed time overlap across concurrent calls. Each row has 32 logarithmic microsecond buckets and a cumulative lifetime `latency_max_ns`; phase artifacts retain start/end maxima and declare exact phase maximum unavailable. Maxima are never subtracted. Terminal outcomes, bytes and histogram updates precede in-flight decrement; consumers must provide quiescent boundaries because independent atomics are not a coherent transaction. Saturated endpoints and overflow make evidence incomplete.

The disabled path adds no timer, heap update allocation or forwarding box. The local allocation control observed one enabled construction allocation for a 3,368-byte fixed bank, one positive control allocation, and zero enabled/disabled recorder update or fixed raw snapshot allocations. The 32-byte raw span and 24-byte claim span can enlarge existing async-trait future frames, including default-disabled operations; before/after future frame growth was not measured and is unavailable. Existing async-trait/object-store future boxes, payload/path preparation, in-flight/cache work and logical timers still have their existing costs. Raw snapshots copy fixed arrays; existing logical error-map snapshots and native/Node JSON observations can allocate outside the workload interval. Recorder allocation controls cover counter updates, not a claim that the full provider allocates nothing.

Node validates native v3 metadata, fixed rows, exact counters, outcome/histogram sums, quiescence, saturation and stable instance identities. Setup can open instances; workload phases require stable observed identities. Missing or malformed enabled fields retain bounded sanitized before/after observations and explicit incompleteness. The opt-in W26 gate requires raw evidence for every expected workload size/result of each known split R2 provider. Non-R2 registries may remain empty, and the default gate and 1000 ops/s floor retain their existing behavior. These local controls establish instrumentation semantics; full capacity, durability/formal, cache and merge qualification remain separate open gates.
