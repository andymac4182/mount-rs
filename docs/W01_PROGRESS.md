...
| W01-WebDAV | WebDAV protocol, locks, session, streaming and lifecycle | Local protocol/session evidence exists; the N-API `now`, `onAssertion`, and live-lock-table controls are explicitly outside supported scope with deterministic Rust clock evidence; hosted macOS/Linux native I/O now passes for the recorded packet; provider/session lifecycle remains | Applicable API ledger, auth/lock durability and restart evidence |
