...
| 2026-09-22 | W01-WebDAV | Hardened absolute-authority matching for WebDAV `Destination` and tagged `If` headers: the shared parser now normalizes bracketed IPv6 Host authorities and default HTTP(S) ports; fixtures cover `[::1]:8080`, `[::1]`, and a local tagged `If` resource, while the full WebDAV target passed 28/28, strict warning-denied Clippy passed, and formatting passed | — | 77% W01.1 planning view | This closes local authority parsing only; live-provider qualification, hosted lifecycle/concurrency, power-loss ordering, crash/power-loss filesystem durability, and the explicit same-resource ordering boundary remain open; W01 stays NO-GO |
| 2026-09-22 | W01-WebDAV | Rechecked the pinned TypeScript/Rust HTTP differential after authority normalization: `CARGO=./scripts/cargo-shared MOUNTX_SOURCE=/private/tmp/mountx-source-w01-20260921 node scripts/check-http-parity.mjs` passed all 40 paired S3+WebDAV cases, including 16 WebDAV cases | — | 77% W01.1 planning view | This is local pinned-oracle loopback evidence only; hosted lifecycle/provider qualification, live remote-provider behavior, power-loss ordering, crash/power-loss filesystem durability, and the explicit same-resource ordering boundary remain open; W01 stays NO-GO |
