...
with the existing destroyed-session error boundary. It also adds an ignored
Linux-native harness for eight concurrent
...
| Cross-process/crash, cancellation/close, exact append ordering, durability/restart | Open/classified | Package-level NFS signal teardown now passes in a child process; 9P session destruction now wakes and drains direct `Tflush` waiters, but cross-process I/O/crash, native cancellation/close races, exact append ordering, and failure durability remain |
