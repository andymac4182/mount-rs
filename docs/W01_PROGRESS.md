...
The latest W01-9P remote-admission opt-in packet was published at exact SHA
`2f0e23a4bc5ba79fef61426137da365cbcd55f42` and passed [Native 9P run
`35688865494`](https://github.com/andymac4182/mount-rs/actions/runs/35688865494):
N-API job `106621392719` passed the interface-qualified `allowRemote: true`
version/attach/`Tgetattr` path, default remote refusal, wire-framing, full
server/attach, shared-lock, and teardown phases, and automatic/direct/
structural mounted-I/O cleanup. Rust job `106621392818` passed the Linux probe
plus all four ignored native lifecycle tests. Local elevated N-API execution
passed against the actual external IPv4 interface; crash/reset, broader parity,
and W01 gates remain open, so production remains NO-GO.

The latest W01-9P direct-session packet was published at exact SHA
`496ed42b3cfaca4a379f7e061d24bd27b5e23372` and passed [Native 9P run
`35691732267`](https://github.com/andymac4182/mount-rs/actions/runs/35691732267):
N-API job `106629973640` passed the Linux probe, addon build, isolated
server/attach selector, direct native/structural `P9Session` lifecycle, and
automatic/direct/structural mounted-I/O cleanup; Rust job `106629973787`
passed the Linux probe plus all four ignored native lifecycle tests. Direct
construction now accepts native `Filesystem` or structural `FsDriver`; the
structural adapter is retained until `destroy()` and released without
shutting down the caller-owned source. Crash/reset recovery, broader parity,
and W01 gates remain open, so production remains NO-GO.

The latest W01-9P direct-session state-machine packet was published at exact
SHA `c627761721b982f78fd33942f7287753fad972f8` and passed [Native 9P run
...
server/attach selector, direct `Tflush`/version-reset/destroy invalidation
coverage, and automatic/direct/structural mounted-I/O cleanup; Rust job
`106635344863` passed the Linux probe plus all four ignored native lifecycle
tests. The direct regression proves `Tflush` waits for and wakes an in-flight
request, version reset clears fids/users and returns stale `EIO`, and destroy
clears state and returns stale `ENODEV` even when the provider reports a late
error; the caller-owned structural source remains alive. Local focused checks,
full `mount-rs-9p`, strict Clippy, formatting, and diff checks passed. Crash/
...
The forced-crash v4.1 lane also verifies wire `RENAME` of a seeded host file.
After server replacement, wire `LOOKUP` reports the old name absent and the
...
The refreshed protected-provider admission remains blocked: [AWS S3 run
`35712627727`](https://github.com/andymac4182/mount-rs/actions/runs/35712627727)
...
| 2026-09-22 | W01-S3 | Closed the oracle-derived `Expect: 100-continue` upload framing gap: the Rust gateway regression proves it emits the interim `100 Continue`, accepts the delayed content-length body, returns `200`, and persists exact bytes; the focused test and full Rust 5/6/35/5 packet plus strict Clippy, formatting, and diff checks passed | — | 78% W01.1 planning view | This is bounded local HTTP/1.1 framing evidence only; live AWS/R2, physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Refreshed live provider admission on published `a861032f`: AWS run [`35691926924`](https://github.com/andymac4182/mount-rs/actions/runs/35691926924) stopped at `AWS_S3_CI_CONFIG_BLOCKED missing_bucket`; R2 run [`35691926938`](https://github.com/andymac4182/mount-rs/actions/runs/35691926938) stopped at `R2 CI monthly run cap already exceeded: count=323` before its live job | — | 78% W01.1 planning view | No live AWS/R2 service PASS is claimable; protected AWS configuration, the R2 budget reset, physical power-loss durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Qualified the current mainline rejected-request-body drain: a body-bearing rejected PUT followed immediately by a GET now has a regression proving the second `200` response remains ordered and readable; the complete current Rust 5/6/38/5 packet plus strict Clippy, formatting, and diff checks passed | — | 78% W01.1 planning view | This is bounded local rejected-body/keep-alive evidence only; live AWS/R2, physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Refreshed live provider admission on published `d4f43b28`: AWS run [`35692509801`](https://github.com/andymac4182/mount-rs/actions/runs/35692509801) stopped at `AWS_S3_CI_CONFIG_BLOCKED missing_bucket`; R2 run [`35692509869`](https://github.com/andymac4182/mount-rs/actions/runs/35692509869) stopped at `R2 CI monthly run cap already exceeded: count=325` before its live job | — | 78% W01.1 planning view | No live AWS/R2 service PASS is claimable; protected AWS configuration, the R2 budget reset, physical power-loss durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Closed the oracle-derived abandoned-download handle lifecycle gap: after the first response bytes, the client disconnects while the driver read is parked, the opened handle closes exactly once, and a fresh ranged GET returns the expected `206` bytes; the complete current Rust 5/6/39/5 packet plus strict Clippy, formatting, and diff checks passed | — | 78% W01.1 planning view | This is bounded local cancellation cleanup evidence only; live AWS/R2, physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Refreshed live provider admission on published `09eca17b`: AWS run [`35693316456`](https://github.com/andymac4182/mount-rs/actions/runs/35693316456) stopped at `AWS_S3_CI_CONFIG_BLOCKED missing_bucket`; R2 run [`35693316400`](https://github.com/andymac4182/mount-rs/actions/runs/35693316400) stopped at `R2 CI monthly run cap already exceeded: count=326` before its live job | — | 78% W01.1 planning view | No live AWS/R2 service PASS is claimable; protected AWS configuration, the R2 budget reset, physical power-loss durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Closed the oracle-derived positive in-flight `S3Server.close()` drain path: a parked 2 MiB response resumes and returns every byte while bounded close completes; the timeout-abort case remains separately covered, and the complete current Rust 5/6/40/5 packet plus strict Clippy, formatting, and diff checks passed | — | 78% W01.1 planning view | This is bounded local graceful-close evidence only; live AWS/R2, physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Qualified in-flight streamed PUT crash/restart recovery: `node test/s3-inflight-crash.mjs` forced a child process down after it wrote a private streaming prefix, then reopened NodeFs and SQLite in replacement sessions; both kept `/photos/in-flight.txt` unpublished, accepted a fresh streamed PUT with exact bytes, and reaped the orphan `.mountx-put-*` entry after the effective session TTL through the existing `now` hook | — | 78% W01.1 planning view | This is local provider/process-crash and staging-TTL evidence only; physical power-loss/torn-write ordering, live AWS/R2 behavior, hosted/native lifecycle, and broader workload bounds remain open, so W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Qualified the bounded provider-failure regression `streaming_publish_rename_failure_removes_staging_and_preserves_object`: one injected `rename(EIO)` during streamed replacement returns 500, removes private `.mountx-put-*` staging, and preserves the prior object bytes; the focused shared-target gateway target passed 45/45, with strict Clippy, formatting, and diff checks passing | — | 78% W01.1 planning view | This is bounded local provider-failure evidence only; live AWS/R2 failure handling, physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01-S3 stays NO-GO |
| 2026-09-22 | W01-S3 | Qualified provider-backed S3 network concurrency and staging-scan race: the new NodeFs/SQLite matrix initially reproduced transient `NoSuchKey` responses when concurrent streaming PUTs renamed private staging files between `readdir` and `stat`; S3 quota/reaper scans now ignore only vanished `ENOENT`/`ENOTDIR` entries while preserving other errors; the final matrix passed 32 concurrent buffered PUT/GET pairs plus content-length streamed PUT/GET for both providers with zero session errors; the current Rust 5/6/44/5 packet, strict Clippy, formatting/diff checks, release N-API build, pinned S3 session differential, exact barrel scope, 64-way/CAS concurrency, process restart, callback observability, typecheck/distribution, and isolated S3 N-API server integration passed | — | 78% W01.1 planning view | This closes bounded local NodeFs/SQLite provider concurrency and transient staging-scan race evidence only; live AWS/R2, physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Qualified real HTTP aborted multipart-part replacement cleanup: after an existing multipart part is committed, a replacement part is partially staged and the client sends a TCP reset; the private `.part-*` staging entry is reaped within the bounded wait and the original part remains byte-for-byte intact; the complete Rust 5/6/44/5 packet, strict Clippy, formatting/diff checks, and isolated S3 N-API server integration passed | — | 78% W01.1 planning view | This is bounded local multipart request-cancellation replacement evidence only; live AWS/R2, physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Qualified real HTTP aborted-upload cleanup: after a partial staged PUT is visible, the client sends a TCP reset; the private `.mountx-put-*` entry is reaped within the bounded wait and the destination is not published; the complete Rust 5/6/43/5 packet, strict Clippy, formatting/diff checks, and isolated S3 N-API server integration passed | — | 78% W01.1 planning view | This is bounded local request-cancellation staging evidence only; live AWS/R2, physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Qualified streamed driver-read fault recovery: an injected `EIO` on the first streamed GET read terminates the damaged response, emits exactly one `S3 response body stream failed` transport report, and leaves a fresh GET healthy with exact bytes; the complete Rust 5/6/42/5 packet, strict Clippy, formatting/diff checks, and isolated S3 N-API server integration passed | — | 78% W01.1 planning view | This is bounded local streamed driver-read fault evidence only; live AWS/R2, physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Closed the pre-body disconnect download-handle lifecycle gap: a delayed driver open is parked after opening the object, the client disconnects before any response body chunk, and the released handle closes exactly once; the complete Rust 5/6/41/5 packet, strict Clippy, formatting/diff checks, and isolated S3 N-API server integration passed, with the initial sandbox loopback `PermissionDenied` classified and the host-enabled rerun passing | — | 78% W01.1 planning view | This is bounded local pre-body cancellation cleanup evidence only; live AWS/R2, physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Re-qualified the current release N-API S3 runtime on published mainline `76eb2914`: fresh addon build, callback observability, 64-way/CAS concurrency, session differential, exact 155/258/153 barrel scope, process restart, typecheck, distribution, and isolated S3 server integration all passed | — | 78% W01.1 planning view | This refreshes current local Rust/N-API evidence only; live AWS/R2, physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Refreshed live provider admission on published `fcf1d547`: AWS run [`35693941024`](https://github.com/andymac4182/mount-rs/actions/runs/35693941024) stopped at `AWS_S3_CI_CONFIG_BLOCKED missing_bucket`; R2 run [`35693941037`](https://github.com/andymac4182/mount-rs/actions/runs/35693941037) stopped at `R2 CI monthly run cap already exceeded: count=329` before its live job | — | 78% W01.1 planning view | No live AWS/R2 service PASS is claimable; protected AWS configuration, the R2 budget reset, physical power-loss durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Closed the oracle-derived HEAD and transfer-encoding wire-framing gaps: HEAD preserves `Content-Length` with no body, and `Transfer-Encoding: chunked` PUT returns the documented `411 MissingContentLength`; the focused HTTP subset and full Rust 5/6/37/5 packet plus strict Clippy, formatting, and diff checks passed | — | 78% W01.1 planning view | This is bounded local HTTP/1.1 framing evidence only; live AWS/R2, physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Closed the oracle-derived HTTP/1.1 pipelined-response ordering gap: two complete GET requests sent in one TCP write now have a Rust gateway regression proving two `200` replies remain ordered on one connection; the focused test and full Rust 5/6/34/5 packet plus strict Clippy, formatting, and diff checks passed | — | 78% W01.1 planning view | This is bounded local HTTP/1.1 wire evidence only; live AWS/R2, physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Closed short streamed-response framing fault: Rust HTTP responses now count streamed bytes against `Content-Length`, report exactly one bounded `out of frame` transport error, and terminate the damaged keep-alive connection with `UnexpectedEof`; the oracle-derived regression and full Rust 5/6/33/5 packet, strict Clippy, release N-API build, isolated `MOUNT_RS_SERVER_PHASE=s3 node test/servers.mjs`, callback observability, session/member parity, barrel scope, 64-way/CAS concurrency, process restart, typecheck, and distribution checks passed | — | 78% W01.1 planning view | This closes the local response-framing/connection-fault boundary only; live AWS/R2, physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Refreshed live provider admission on published `4f9120a2`: AWS run [`35690334807`](https://github.com/andymac4182/mount-rs/actions/runs/35690334807) reached the protected-config validator and stopped with `AWS_S3_CI_CONFIG_BLOCKED missing_bucket`; R2 run [`35690334795`](https://github.com/andymac4182/mount-rs/actions/runs/35690334795) stopped at `R2 CI monthly run cap already exceeded: count=317 limit=20` before its live integration job was admitted | — | 78% W01.1 planning view | No live AWS/R2 service PASS is claimable; protected AWS configuration and the R2 CI budget reset remain external prerequisites, and physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-S3 | Refreshed live provider admission on published `f607d445`: AWS run [`35688895917`](https://github.com/andymac4182/mount-rs/actions/runs/35688895917) reached the protected-config validator and stopped with `AWS_S3_CI_CONFIG_BLOCKED missing_bucket`; R2 run [`35688895958`](https://github.com/andymac4182/mount-rs/actions/runs/35688895958) stopped at `R2 CI monthly run cap already exceeded: count=313 limit=20` before its live integration job was admitted | — | 78% W01.1 planning view | No live AWS/R2 service PASS is claimable; protected AWS configuration and the R2 CI budget reset remain external prerequisites, and physical power-loss/torn-write durability, broader workload bounds, and native/hosted acceptance remain open; W01 stays NO-GO |
...
| 2026-09-22 | W01-WebDAV | Classified durable-lock persistence as outside supported scope: `node test/webdav-node-fs-crash.mjs` and `node test/webdav-sqlite-crash.mjs` passed after each killed child had created an exclusive lock, and each replacement session reopened the provider with the bytes intact and `lockCount === 0` | — | 77% W01.1 planning view | Durable lock persistence is not claimed; live-provider qualification, physical power-loss ordering, crash/power-loss filesystem durability, hosted/native acceptance, and the explicit same-resource ordering boundary remain open; W01 stays NO-GO |
| 2026-09-22 | W01-WebDAV | Current exact-tip hosted/provider audit after mainline advanced: `origin/main` is `a7ff32f00e597dbdc37e0baefe3f37321354bb58`, containing provider-admission tip `44e69a27f13a6113753443558996b9d3e417d496`; CI `35690677984` was cancelled after Ubuntu native-WebDAV job `106626916000` passed and macOS native-WebDAV job `106626916081` was cancelled; protected AWS run `35690334807` stopped at `AWS_S3_CI_CONFIG_BLOCKED missing_bucket`, and protected R2 run `35690334795` stopped at `count=317 limit=20`, skipping their live acceptance paths | — | 77% W01.1 planning view | No current exact-tip hosted WebDAV PASS is claimable; the Ubuntu native slice passed but macOS/aggregate CI was cancelled, while protected AWS configuration, the R2 usage envelope, live remote-provider behavior, physical power-loss ordering, and crash/power-loss filesystem durability remain external gates; W01 stays NO-GO |
| 2026-09-22 | W01-WebDAV | Audited absolute-authority matching for WebDAV `Destination` and tagged `If` headers against the pinned oracle: Rust `Url::host_str()` already retains bracketed IPv6, so the existing literal host/explicit-port comparison was retained; fixtures cover `[::1]:8080`, `[::1]`, and a local tagged `If` resource, while the full WebDAV target passed 28/28, strict warning-denied Clippy passed, and formatting passed | — | 77% W01.1 planning view | This closes the local authority-parser audit only; live-provider qualification, hosted lifecycle/concurrency, power-loss ordering, crash/power-loss filesystem durability, and the explicit same-resource ordering boundary remain open; W01 stays NO-GO |
| 2026-09-22 | W01-WebDAV | Rechecked the pinned TypeScript/Rust HTTP differential after the authority audit: `CARGO=./scripts/cargo-shared MOUNTX_SOURCE=/private/tmp/mountx-source-w01-20260921 node scripts/check-http-parity.mjs` passed all 40 paired S3+WebDAV cases, including 16 WebDAV cases | — | 77% W01.1 planning view | This is local pinned-oracle loopback evidence only; hosted lifecycle/provider qualification, live remote-provider behavior, power-loss ordering, crash/power-loss filesystem durability, and the explicit same-resource ordering boundary remain open; W01 stays NO-GO |
| 2026-09-22 | W01-WebDAV | Hardened the public `DavLockTableOptions` finite-timeout boundary: the cap is applied before the one-second minimum, so `max_timeout_seconds: 0` cannot trigger Rust's invalid `clamp(1, 0)` panic; the deterministic regression and full WebDAV target passed 29/29, strict warning-denied Clippy, formatting, and `git diff --check` passed | — | 77% W01.1 planning view | This closes a local public-option panic boundary only; hosted/provider qualification, live remote-provider behavior, power-loss ordering, crash/power-loss filesystem durability, and the explicit same-resource ordering boundary remain open; W01 stays NO-GO |
| 2026-09-22 | W01-WebDAV | Made public WebDAV lock snapshots deterministic by preserving grant order in `DavLockTable::all()` like the pinned oracle's insertion-ordered lock map, including cleanup of expired/deleted entries and duplicate-token replacement; the focused ordering regression and full WebDAV target passed 30/30, warning-denied Clippy, `cargo fmt --check`, and `git diff --check` passed after `cargo fmt --all` normalized the new `tests/webdav.rs` import block | — | 77% W01.1 planning view | This closes a local public lock-snapshot determinism boundary only; hosted/provider qualification, live remote-provider behavior, power-loss ordering, crash/power-loss filesystem durability, durable lock persistence, and the explicit same-resource ordering boundary remain open; W01 stays NO-GO |
| 2026-09-22 | W01-WebDAV | Hardened provider-error lock-root cleanup: DELETE/MOVE now remove a lock only after `stat` confirms `ENOENT`, retaining it when an I/O error leaves namespace state unknown; the fault-injected regression and full WebDAV target passed 31/31, warning-denied Clippy, `cargo fmt --check`, and `git diff --check` passed | — | 77% W01.1 planning view | This closes a local provider-error/lock-lifecycle boundary only; live-provider qualification, power-loss ordering, crash/power-loss filesystem durability, durable lock persistence, and the explicit same-resource ordering boundary remain open; W01 stays NO-GO |
| 2026-09-22 | W01-WebDAV | Routed declared Content-Length size-limit rejections through session observability before the adapter closes the connection: request/reply/error counters now include the preflight 413 and request-level onError receives the original head exactly once; the real-loopback regression verifies status, callback shape, and counters, and the shared helper preserves bodyless HEAD responses | — | 77% W01.1 planning view | This closes a local HTTP preflight observability boundary only; full WebDAV 36/36, warning-denied workspace Clippy, formatting, git diff --check, and the pinned 40-case differential pass, while authentication-ordering policy, live-provider/hosted qualification, power-loss ordering, crash/power-loss filesystem durability, durable lock persistence, and same-resource ordering remain open; W01 stays NO-GO |
| 2026-09-22 | W01-WebDAV | Made public lock coverage deterministic by iterating the grant-order index for covering and within lookups; lock-discovery lists, locked-member results, and first-conflict selection now match the pinned insertion-ordered Map, with a regression covering ancestor/direct coverage, subtree roots, and the selected 423 conflict | — | 77% W01.1 planning view | This closes a local public/session lock-order boundary only; full WebDAV 37/37, warning-denied workspace Clippy, formatting, git diff --check, and the pinned 40-case differential pass, while hosted/provider qualification, power-loss ordering, crash/power-loss filesystem durability, durable lock persistence, and same-resource ordering remain open; W01 stays NO-GO |
| 2026-09-22 | W01-WebDAV | Hardened public `Lock-Token` parsing against UTF-8 byte-boundary panics while preserving the pinned oracle's Unicode token acceptance; `<urn:uuid:é>` now parses safely and nested angle brackets remain rejected | — | 77% W01.1 planning view | This closes a local public parser safety/parity boundary only; full WebDAV 37/37, warning-denied workspace Clippy, formatting, git diff --check, and the pinned 40-case differential pass, while hosted/provider qualification, power-loss ordering, crash/power-loss filesystem durability, durable lock persistence, and same-resource ordering remain open; W01 stays NO-GO |
...
| 2026-09-22 | W01-NFS | A HostFs-backed real-TCP v4.1 regression exposed `NFS4ERR_IO` after a successful same-owner read-only to read/write OPEN upgrade because the server kept a read-only backend handle. OPEN state now retains per-access backend handles and closes each distinct handle during CLOSE/expiry/destroy. The fixed test passes both read-to-write and write-to-read upgrades with exact data and preserves cross-client share denial; full locked NFS (42 unit, 21 v4 wire), strict NFS/N-API Clippy, pinned parity (266 pass/18 explicit skips), and local native macOS NFSv3 1/1 pass | — | Local host-backed NFSv4.1 access-upgrade gate passed | Native v4.1 client ordering, cross-process state recovery, power-loss durability, exact-tip hosted acceptance, and W01 production readiness remain open; W01 stays NO-GO |

| 2026-09-22 | W01-NFS | A controlled v4.1 OPEN/v3 REMOVE race reproduced a second opaque handle for one unlinked inode (ID 3 instead of ID 2). Destructive namespace operations now take the shared exclusive path gate across versions; the v3 REMOVE waits for OPEN, v3 LOOKUP sees `NOENT`, and the original v4 handle/stateid reads exact bytes. A second real-TCP test caught the overly broad first gate; unrelated v4 OPEN/WRITE/READ now completes while another OPEN is blocked. Focused cases 2/2, full locked NFS (42 unit, 23 v4 wire), strict NFS/N-API Clippy, pinned parity (266 pass/18 explicit skips), and native macOS NFSv3 1/1 pass locally | — | Same-process cross-version handle ordering and independent OPEN progress passed | Cross-process arbitration, native v4 ordering, crash/power-loss durability, exact-tip hosted acceptance, and W01 production readiness remain open; W01 stays NO-GO |
...
| 2026-09-22 | W01-NFS | Expanded the actual kernel-mounted namespace workload with rename-over-open held-read/replacement-read and same-inode hard-link rename/alias-removal checks. macOS native NFSv3 passed 1/1 plus five reruns with no leaked mountpoints; full locked NFS 44 unit/27 v4 wire, strict NFS Clippy, and formatting passed. The Linux native v4.1 case includes the workload but has not run at this patch | — | Local macOS native NFSv3 lifetime/ordering evidence passed | Linux native v4.1, cross-process recovery, crash/power-loss durability, exact-tip hosted acceptance, and W01 production readiness remain open; W01 stays NO-GO |
| 2026-09-22 | W01-NFS | Fixed v4.1 slot consumption on rejected `SEQUENCE`: over-limit COMPOUND and invalid highest-slot requests now reject before incrementing the slot, with the latter returning `NFS4ERR_BAD_HIGH_SLOT`. The real-TCP same-sequence correction regression failed before the fix and passed after it; full locked NFS 44 unit/27 v4 wire, strict NFS/N-API Clippy, and local macOS native NFSv3 1/1 passed | — | Local v4.1 session-limit retry correctness passed | Manual hosted run `35723855631` remains queued; exact-tip native v4.1 ordering, cross-process state, power-loss durability, and W01 production acceptance remain open; W01 stays NO-GO |
| 2026-09-22 | W01-NFS | The existing Lima Ubuntu 26.04 arm64 VM compiled the published `a31f072c` native test through `scripts/cargo-shared` after guest-only `nfs-common` installation. The exact ignored Linux kernel NFSv4.1 mount/workload test passed 1/1 plus three timed reruns; post-test NFS mount and mountpoint checks were empty, and the VM was stopped again | — | Local Lima Linux native NFSv4.1 gate passed | This is not GitHub-hosted Ubuntu acceptance, another platform, durable v4 state, power-loss durability, or W01 production readiness; W01 stays NO-GO |
...
| 2026-09-22 | W01-FUSE | Published-tip CI run `35716364566` at `4e9260ead4925d3140a758374da886f985242f47` was cancelled before any job was created by a newer mainline push; the next run `35716566393` at `924009af061d119404b2ee1f59e86506f7b1cbd2` is pending | No hosted `/dev/fuse` result is claimable; the moving-mainline cancellation/queue condition still requires a terminal native-FUSE job at the current published tip, so W01 stays NO-GO |
| 2026-09-22 | W01-FUSE | Lima Ubuntu 26.04 arm64 at published `32b85965` passed the strict real shipped CLI smoke: a fixed payload was read back before and after rename through the memory-driver FUSE mount, alpha was absent, beta was present, SIGINT exited the process, and the mount was absent afterward (`CLI_FUSE_STRICT_SMOKE=PASS`) | Local Linux CLI usability is green; hosted terminal native-FUSE evidence and the wider W01 release gates remain required, so W01 stays NO-GO |
| 2026-09-22 | W01-FUSE | Local-scope release decision: the verified Lima Linux native-FUSE matrix, PGlite/restart/fault gates, and strict real CLI smoke are accepted as sufficient for the requested local Linux/FUSE deployment scope; the implementation and tracker evidence are merged into current `origin/main` through `0687c3dc` | FUSE is ready for the verified local Linux scope; hosted CI remains explicitly pending/non-blocking by user acceptance, while the broader W01 roll-up remains subject to unrelated transport/provider gates |
| 2026-09-22 | W01-FUSE | Current `origin/main` reproduced and then fixed the real blocked-read teardown failure: the forced `EBUSY` path now drains the serving task before lazy detach, and concurrent native-test mountpoints use atomic unique suffixes. Lima Ubuntu 26.04 arm64 at `65776be` passed native FUSE 3/3, automatic FUSE 2/2 concurrently, CLI lifecycle 3/3, SQLite crash/restart 2/2, fault injection 1/1, native PGlite reopen 1/1, and PGlite SQLite compositions 1/1; workspace tests/Clippy passed and cleanup found no stale mounts/processes | FUSE local Linux scope is requalified; hosted CI remains external/non-blocking by user acceptance and the broader W01 roll-up remains separate |
