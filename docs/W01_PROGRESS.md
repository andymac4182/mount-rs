...
oracle's injectable `now`, `onError`, `onAssertion`, or live `DavLockTable`;
`stats.methods` now has the oracle's `Map<string, number>` shape. Direct
listener lifecycle, network/native/hosted concurrency, and the external
provider/native/restart gates also remain open.
