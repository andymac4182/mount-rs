...
`1e9fffe0f2765e0be17c1a2e394b70c05dea112e` and passed [Native 9P run
`35690887775`](https://github.com/andymac4182/mount-rs/actions/runs/35690887775):
N-API job `106627448717` passed the Linux probe, addon build, isolated
server/attach selector, direct `P9Session` lifecycle, and
automatic/direct/structural mounted-I/O cleanup; Rust job `106627448526`
passed the Linux probe plus all four ignored native lifecycle tests. The
direct constructor is scoped to native `Filesystem` input; structural-driver
adaptation remains on the factory/mount seams. Crash/reset recovery, broader
parity, and W01 gates remain open, so production remains NO-GO.
