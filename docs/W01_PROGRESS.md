...
| W01-9P | 9P protocol, session, connection, attach and mount lifecycle | Rust/N-API attached Node Duplex with typed per-attachment peer/ownership/frame/in-flight bounds, direct session, ownership, duplicate-attach, bounded backpressure, write-fault, server-teardown, broadcast-shutdown, active-connection close-race, bounded task-reaping, and session-destroy/Tflush-wakeup evidence passes; ignored native harnesses now cover eight concurrent mounted file write/read/rename/read workers plus server-close, kernel-connection-close, and bounded unmount; native Tokio listener connections intentionally expose no Node stream and use the supported `attach` seam; hosted run `35616832528` passed the prior native Linux lifecycle packet | Fresh hosted rerun for the shutdown/reaping, concurrent-I/O, and server-close packet, native reset/half-close/process-crash evidence, and complete applicable stream/attach/connection decision |
...
`WebdavSession.handleRequest` and materializes file-backed response bodies for
the N-API boundary; a direct PUT plus the existing loopback PUT/GET lane pass.
Streaming bodies/direct method coverage beyond that packet remains open. The
`@mount-rs/core/webdav` subpath now also exposes the WebDAV constants/status
barrel, with oracle-matched literals, limits, status/errno tables and server
defaults; protocol/XML/lock helper exports remain open.
