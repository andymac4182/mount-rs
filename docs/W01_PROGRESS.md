...
| S3 and WebDAV public Node surfaces | Partial | Native server facades exist; S3/WebDAV `drainTimeout` and `onTransportError` option shapes now map, both expose buffered direct `handleRequest`, WebDAV malformed-connection and direct peer-reset evidence passes, both server objects expose shared session/statistics views, S3/WebDAV expose live connection views, S3 peer-aware connection-error reporting passes at the Rust transport boundary, and the WebDAV subpath now exposes pinned constants/status tables, protocol/XML/lock helpers, active lock-record views, and streamed request/response body bindings with direct class 1/2/3 method, LOCK/UNLOCK, cancellation, and error coverage; S3 also exposes its incremental stream boundary; complete option/member parity and external lifecycle gates remain | 72% | — |
...
transport callback. Complete session/member parity, direct listener lifecycle,
and the external provider/native/restart gates remain open.
