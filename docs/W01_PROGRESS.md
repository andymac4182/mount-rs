...
18 explicit capability/root skips at this worktree tip. This NFS branch and
`origin/main` have diverged, so the requested fast-forward-only main push
remains unavailable without a separate integration decision.
...
| 2026-09-22 | W01-NFS | The pinned NFSv3/MOUNT and NFSv4.1 upstream TCP parity gate passed 266 tests with 18 explicit capability/root skips, 0 failures, using the shared Cargo target and installed Command Line Tools | — | 75% W01.1 planning view | This is local userspace parity, not native/hosted, crash-durable, or production acceptance. The NFS branch and `origin/main` diverge, preventing fast-forward-only main publication; W01 stays NO-GO |
