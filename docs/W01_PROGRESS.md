...
provider/restart gates also remain open. The explicit ignored macOS native
WebDAV harness now passes its mount, I/O, and cleanup round-trip locally;
hosted macOS/Linux results remain a separate gate.
...
differential remains open. A read-only status check for the published tip
