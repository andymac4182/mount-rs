...
Last updated: 2026-09-21 19:43 AEST (2026-09-21 09:43 UTC)
...
just the W05 provider pass. At this update, `origin/main` is `d8bc1bd`; its
CI and fault-injection workflows are still queued, so the release revision is
not yet fully re-qualified. The latest release-gate fix is `2e66f94`; CI run
`35584839843` and Live Cloudflare R2 run `35584839810` are pending, while
fault-injection run `35584839781` is queued for that revision. No credential value is stored in the repository or
in this document. Native/platform gates remain explicitly bounded: macOS
native NFS qualification is local evidence; Linux FUSE, Windows, and
signed/activated FSKit acceptance are separate platform workstreams.
...
| PR-04 / W10, W12, W13, W15, W27 | Native and cross-platform acceptance | Open platform gate | 35% | Local macOS NFS read/write/teardown and mount-free FUSE/NFS/9P/WebDAV/S3 codec/session packets pass; FSKit is unsigned, Windows hosted runtime/mount is pending, and SQLite-hosting/VFS hosted/Windows gates remain open. | Execute revision-matched Linux FUSE/NFS/9P/WebDAV, macOS native/FSKit signing and activation, Windows Rust/Node runtime/mount, SQLite VFS multi-process/recovery, and crash/unmount gates. Ship criterion: each supported platform has a passing native gate, or the platform is explicitly excluded from the release support matrix. | 10–24 h active engineering, plus platform queue | `/dev/fuse`, macOS entitlements/signing, FSKit activation, Windows runners, kernel behavior, and physical host access cannot be replaced by local structural tests. |
...
| PR-06 / W18–W20 | Performance, compression, dependency budget, CI, packaging and release controls | Local code-quality gate passed; hosted/release gate open | 40% | A bounded R2 benchmark and local benchmark packets pass; isolated current-head workspace tests and formatting pass; strict locked workspace Clippy now passes after `2e66f94` fixed the TiDB `single_match` lint. W18 remains partial, W19 is design-only, and W20.1–W20.6 remain unchecked. Current release head `2e66f94` has CI run `35584839843` and Live R2 run `35584839810` pending, with fault-injection run `35584839781` queued. | Run the pending hosted workflows to completion, then complete benchmark/dependency/license audits, package install/provenance checks, required hosted macOS/Linux/Windows jobs, and the final acceptance audit. Ship criterion: no required workflow is queued, cancelled, skipped, or failed on the release SHA. | 8–18 h active engineering, plus hosted queue | Hosted runners, package registries, signing/provenance, and release automation are external gates; pending/queued runs are not evidence of release readiness. |
...
| --- | --- | --- |
| Repository implementation | `2e66f94` pushed to `origin/main` | Production-readiness ledger, CI workflow, budget guard, hosted orchestration, shared Cargo wrapper use, provider-matrix R2 CLI coverage, lease-refresh fix, and strict-Clippy TiDB fix are committed. |
| Isolated current-head Rust gates | Passed on `2e66f94` | `CARGO_TARGET_DIR=/private/tmp/mount-rs-prod-gate-9585d3e ./scripts/cargo-shared test --workspace --all-targets --locked`, `cargo fmt --all -- --check`, and strict locked workspace Clippy passed without cross-worktree artifact contamination. Live-provider/native opt-ins remain separately classified. |
...
| Release revision `2e66f94` hosted workflows | CI `35584839843` pending; Live R2 `35584839810` pending; fault injection `35584839781` queued | Current-release qualification is not complete until these jobs conclude and any required follow-up is pushed. |
...
| 2026-09-21 09:43–current | Recording current-head hosted IDs and continuing the production dependency gates | Hosted wait / active production work | Goal remains active until the release exit criteria are green or an explicit external blocker is recorded. |
