...
Last updated: 2026-09-21 19:31 AEST (2026-09-21 09:31 UTC)
...
not yet fully re-qualified. No credential value is stored in the repository or
...
| PR-06 / W18–W20 | Performance, compression, dependency budget, CI, packaging and release controls | Open release gate | 25% | A bounded R2 benchmark and local benchmark packets pass; W18 remains partial, W19 is design-only, and W20.1–W20.6 remain unchecked. Current `main` head `d8bc1bd` has CI run `35583560988` and fault-injection run `35583561049` queued. | Run locked final tests, strict formatting/Clippy, benchmark/dependency/license audits, package install/provenance checks, required hosted macOS/Linux/Windows jobs, and the complete final acceptance audit. Ship criterion: no required workflow is queued, cancelled, skipped, or failed on the release SHA. | 8–18 h active engineering, plus hosted queue | Hosted runners, package registries, signing/provenance, and release automation are external gates; current queued runs are not evidence of release readiness. |
...
| 2026-09-21 09:31–current | Updating this ledger with the production plan; next step is the first implementation/hosted gate chunk | Documentation / active production work | Goal remains active until the release exit criteria are green or an explicit external blocker is recorded. |
