...
| Repository | `mount-rs` |
| Snapshot base | `229a9cd5d1c79c6f13c7144f66d648e5eead0dd9` (`origin/main`, including the published R2 content-addressing/cache, metadata mutation batching, queue-cancellation hardening, corrected Ozone content-addressed block-contract test and deduplicated content-addressed cleanup; the exact W26 commits and concurrent-mainline merges are recorded below) |
...
| Provisional execution completion | W26 implementation scope: 100% for the 11 shipped tracker rows; current terminal qualification packet: **FAILED / NO-GO** because W26.15's hard performance gate is open; production-rollout readiness: 52% (scope, benchmark matrix, lifecycle safety, bounded remote enumeration, provider-bounded KV/N-API contract, durable-provider bounded-listing tests, strict provider-specific hard-threshold IOPS wiring, fail-closed artifact/profile/metric verification and retention, all-provider credential-free production-config policy, one-revision CI evidence-packet aggregation, complete end-to-end surface marker enforcement, the credential-free customer rollout contract, content-addressed R2 caching, metadata mutation batching and queue cancellation safety are implemented and locally tested; no terminal production gates yet). Customer deployment, native, provider-durability, capacity/SLO, backup/DR and release-stream gates remain separately bounded |
| Current acceptance state | The retained W26 packet on `9c098e5` remains the last accepted hosted result within its documented provider/platform boundaries. The published W26 code now includes optimistic read/write paths, atomic whole-file `FsDriver::write_file`, lazy atime/EOF handling, content-addressed R2 blocks with a bounded process-local cache, fenced metadata mutation batching, a bounded pending queue, cancellation-safe runner cleanup, a corrected real-Ozone block-contract assertion for content-addressed IDs and cleanup that deletes identical content-addressed IDs only once. Local full locked-workspace tests and strict Clippy pass; the focused ChunkedFs suite has 18 passing tests including canceled-runner recovery and one-publication concurrent mutation coverage. The last terminal W26 producer packet, run `35655276021` on `4efec58c93a0cd6e6027676f069edf0c4cde3132`, completed all four provider lifecycle matrices with zero timeouts/cleanup failures but missed the hard 1,000-IOPS target; its producer jobs failed and the aggregate failed closed. Security scans `6d5a7665-d8cd-46b7-a9bd-73fec54f5431`, `46d4cf32-9d57-4f7e-975f-2616ddc53dc5` and cleanup-fix scan `85c31e86-1fc8-4a6d-bb95-ef2d956b9ed8` completed with zero reportable findings within their local scopes; hosted/provider/customer controls remain explicit external boundaries. No Live Cloudflare R2 or Live AWS S3 result is recorded. |
| Latest hosted workflow | Fresh manual run `35664474430` on exact tested revision `229a9cd5d1c79c6f13c7144f66d648e5eead0dd9` is currently queued. W26 producer jobs are `ozone-compositions` `106547013256`, `ozone-foundationdb` `106547013296`, `ozone` `106547013193` and `ozone-tidb` `106547013137`; the aggregate job is not yet present. This run includes the corrected content-addressed Ozone test and deduplicated cleanup plus all published mutation batching/cache/queue hardening and is the required current-tip qualification. |
...
| Next hosted qualification | Manual run `35664474430` against `229a9cd5` | Retain all producer logs/JSON artifacts and aggregate output. Accept W26.15 only if the current revision produces terminal pass markers for every configured provider and the complete end-to-end packet; otherwise continue correctness-preserving performance work. The strict 1,000-IOPS target is unchanged. | GitHub-hosted runners, Ozone/provider startup, provider latency and artifact retention are external gates; queued, canceled, failed or skipped jobs are not evidence |
...
| Release/acceptance decision | W26 implementation and local qualification controls remain accepted within their documented scope; production rollout remains **NO-GO** because terminal run `35655276021` failed every hard 1,000-IOPS provider row and aggregate job `106522172018` failed closed. The corrected cleanup fix is published at `229a9cd5` and security-scanned with zero reportable findings, but fresh hosted run `35664474430` is still queued. Earlier standard, policy, negative-path, strict-IOPS, evidence-packet and rollout-contract scans remain recorded above; no failed or queued run is promoted, and no broader native, customer secure-runtime, Ozone backup/DR or release claim is made |
...
The next W26-owned performance/safety chunks are now published on
`origin/main`. The R2 block path uses content-addressed object IDs and a
...
reply channel has already closed. These safeguards prevent unbounded queue
growth, stale runner state and post-cancellation namespace publication; they do
not weaken the durable publication acknowledgement boundary.
...
committed as `d152fa1a` and is included in current `origin/main`
`5d53f08a289c0ba0662c29e6070f9519b47e19a3` after concurrent mainline
reconciliation. The focused ChunkedFs suite passed 18/18, including
`canceled_mutation_runner_releases_queued_requests_without_publishing` and the
concurrent whole-file one-publication regression. The full locked workspace
...
| W26.3a — SQLite and disk-backed PGlite composition over real Ozone | Functional composition complete; latest terminal packet failed hard IOPS; corrected cleanup rerun queued | 100% functional; 88% production qualification | Terminal packet `35655276021` completed 1,200/1,200 lifecycle operations with zero timeout/cleanup failures but measured SQLite/R2 20.35 and PGlite/R2 205.48 IOPS, both below 1,000; the producer and aggregate failed closed. Published R2 content-addressing/cache, metadata batching/queue hardening, corrected Ozone content-addressed-ID contract and deduplicated cleanup are included in current revision `229a9cd5`; manual rerun `35664474430` is queued on that exact SHA. | Inspect the retained current-tip composition artifacts and close only on strict provider pass markers plus aggregate end-to-end success; otherwise continue correctness-preserving publication/performance work. | ~1–4 d follow-up performance/design; 0.5–1.5 d hosted review | Current Ozone fixture capacity/topology and customer target capacity remain external, but the remaining publication/provider latency cost is still a W26 gate. |
...
| W26.3c — independent durable three-node FoundationDB composition over Ozone | Durable provider lifecycle and restart markers passed; latest terminal packet failed hard IOPS; corrected cleanup rerun queued | 90% base; 84% extension | The arm64 durable FoundationDB proof and provider-backed bounded-listing contract remain green. Terminal hosted job `106517158383` in run `35655276021` completed all 1,200 lifecycle operations and measured 40.50 IOPS, with zero timeout/cleanup failures but `IOPS_TARGET_NOT_MET`; the artifact is diagnostic only. Current revision `229a9cd5` adds mutation batching, cancellation safety, the corrected block-contract assertion and deduplicated content-addressed cleanup; manual rerun `35664474430` job `106547013296` is queued. Local FoundationDB binary linking remains blocked by missing `libfdb_c`. | Inspect the retained rerun while preserving durable restart, fencing, bounded-listing and cleanup assertions; do not promote until the provider and aggregate markers pass. | ~1–4 d shared concurrency follow-up; 0.75–1.5 h hosted review | FoundationDB client image/runtime, Ozone capacity/topology and customer secure durability remain external; local provider-native linking remains blocked. |
| W26.3d — durable multi-node TiDB composition over Ozone | Durable TiDB lifecycle and N-API bounded markers passed; latest terminal packet failed hard IOPS; corrected cleanup rerun queued | 100% base; 84% extension | Terminal hosted job `106517158249` in run `35655276021` completed all 1,200 lifecycle operations and measured 18.09 IOPS, with zero timeout/cleanup failures but `IOPS_TARGET_NOT_MET`; the artifact is diagnostic only. Current revision `229a9cd5` adds mutation batching, cancellation safety, the corrected block-contract assertion and deduplicated content-addressed cleanup; manual rerun `35664474430` job `106547013137` is queued. The atomic path retains fencing/CAS and serialized conflict fallback, with local full-workspace tests and Clippy green. | Inspect the retained rerun while preserving restart/ambiguous-commit/fencing/bounded-listing/cleanup assertions; do not promote until the provider and aggregate markers pass. | ~1–4 d shared concurrency follow-up; 0.75–1.5 h hosted review | GitHub-hosted TiDB/PD/TiKV, N-API runtime, Ozone capacity/topology and customer secure durability remain external. |
...
| W26.4b — CI wiring, pinned images, cleanup, and documentation | Implementation complete; latest terminal packet failed the hard provider gate; corrected cleanup rerun queued | 100% implementation; 63% hosted packet qualification | CI installs NAPI/PGlite with locked Rust dependencies, runs the Ozone composition matrix, Node/CLI coverage, the shipped HTTP server/client reopen path, and durable Ozone/TiDB/FoundationDB jobs. Terminal run `35655276021` retained the base/provider artifacts; base Ozone passed, every configured provider missed 1,000 IOPS and aggregate job `106522172018` failed closed. Its FoundationDB retry also exposed the stale content-addressed-ID assertion before the benchmark; cleanup then required deduplicating the now-equal IDs. Fresh manual run `35664474430` is queued on corrected `229a9cd5` with W26 producer jobs `106547013193`, `106547013256`, `106547013137` and `106547013296`; the aggregate job is not yet present. | Retain exact current-tip artifacts, policy markers, end-to-end markers, performance markers and cleanup; promote only after all producer jobs and `w26-ozone-evidence` are terminally successful on one revision. | ~1–4 d performance follow-up; 0.75–1.5 d hosted review per retained run | Workflow concurrency, hosted provider startup, Ozone capacity/topology, artifact service and customer TLS/IAM remain external; no failed/queued/skipped artifact is acceptance evidence. |
| W26.14 — complete end-to-end Ozone evidence-surface gate | Implementation complete; latest terminal packet failed performance; corrected cleanup revision rerun queued | 100% implementation; 44% production qualification | Expanded `scripts/verify-w26-ozone-evidence-packet.mjs` to require gateway health/ready/restart, SQLite/PGlite composition and bounded listing, Rust CLI, Node provider matrix/CLI, remote HTTP CLI, TiDB Rust/N-API seed/reopen and FoundationDB Rust/N-API seed/reopen markers. Local benchmark/evidence tests, Node/shell/YAML checks, diff checks, full locked workspace tests and strict Clippy pass. Terminal run `35655276021` retained complete provider logs/artifacts, but all four hard IOPS rows failed and aggregate `106522172018` failed closed; its FoundationDB lane additionally found a stale test assertion before benchmarking and then exposed duplicate cleanup of an identical content-addressed ID. Cleanup was corrected and security-scanned with zero findings. Manual run `35664474430` is queued on `229a9cd5`. No packet result is promoted. | Close the packet only after a post-remediation run has terminal pass evidence from every producer and aggregate; retain native/mount and customer secure-runtime evidence separately. | ~1–4 d W26 performance follow-up; ~0.75–1.5 d hosted review; ~0.75–1.25 h security review | Ozone fixture capacity/topology, hosted provider/N-API execution, artifact authorization, native mount runners, customer TLS/IAM/rotation and measured SLO/RPO/RTO remain external. |
| W26.15 — per-drive 1,000-IOPS qualification and safe performance remediation | Metadata batching, content-addressed cache, queue-cancellation safety and Ozone test-contract/cleanup corrections implemented/published; hard target still open; **NO-GO** | 80% W26-owned implementation / 31% hosted qualification | Published W26 performance work includes atomic whole-file write `96a25f17`, lazy atime/EOF `d1bc8fb9`, R2 content addressing/cache, mutation batching `c4e9a253` (published through `a1cb4ca9`) and queue hardening `d152fa1a`. The Ozone integration test asserts identical content reuses its ID and different content does not (`edb6a43e`); cleanup now deletes the repeated content-addressed ID once (`0282df64`, published through `229a9cd5`). The batcher publishes eligible whole-file/unlink mutations once under the lease, bounds pending requests at 1,024, drains canceled-runner requests with `EIO` and skips closed replies before applying a queued mutation. Focused ChunkedFs tests pass 18/18; Ozone test compile-check, full locked workspace tests, strict Clippy and diff checks pass; cleanup security scan `85c31e86-1fc8-4a6d-bb95-ef2d956b9ed8` completed with zero reportable findings within its local scope. Terminal run `35655276021` remains diagnostic at SQLite/R2 20.35, PGlite/R2 205.48, TiDB/R2 18.09 and FoundationDB/R2 40.50 IOPS; every row completed 1,200/1,200 operations with zero timeout/cleanup failures but failed `IOPS_TARGET_NOT_MET`, and aggregate `106522172018` failed closed. Fresh manual rerun `35664474430` is queued on exact revision `229a9cd5`. | Inspect current-tip provider metrics, artifacts and aggregate markers. If the hard target remains open, continue correctness-preserving provider/publication work or obtain an Ozone/customer capacity qualification; do not lower the target or convert failed rows to skips. | ~1.5–4 d implementation/design; ~0.75–1.5 d hosted review per rerun; external queue time excluded | GitHub runner/provider startup, Ozone fixture capacity/topology, provider-native performance, artifact retention, customer secure topology and capacity are external gates; no staging environment exists. |
...
| P8 — load, capacity, soak and cost envelope | Hosted/provider CI plus W26.15 remediation | Hard-threshold and one-revision packet gates implemented; current packet failed performance; cleanup-corrected rerun is queued | 52% | The benchmark records successful write+read+delete lifecycle IOPS, supports a hard `--min-iops` threshold, strict `--require-configured` provider qualification and redacted JSON. The fixed 4 KiB/400-iteration/concurrency-64 profile is enforced. Terminal run `35655276021` completed all 1,200 lifecycle operations per row with zero timeouts/cleanup failures but measured only 20.35/205.48/18.09/40.50 IOPS; aggregate job `106522172018` failed closed. Published `96a25f17` optimized whole-file publication, `d1bc8fb9` avoids per-read/EOF metadata publication, later chunks add R2 content addressing/cache, mutation batching and queue cancellation safety, and `0282df64` fixes duplicate cleanup for identical content-addressed IDs. Fresh manual run `35664474430` on `229a9cd5` is queued. | Inspect the current-tip all-provider packet, then retain p95/p99 latency, errors, CPU/memory and topology; add soak/capacity variants only after every hard provider row closes | 1.5–4 d W26 performance follow-up; 1–2 d hosted review; soak effort provisional | Ozone fixture capacity/topology, stable hosted runners, artifact service, provider quotas and customer 99.99% capacity remain external; do not lower the target |
...
| --- | --- | --- |
| Hosted result inspection | 0.5–1.5 d engineering plus external queue time | Historical green packet `35585066458` remains the last accepted hosted result. Current terminal run `35655276021` is diagnostic: base Ozone passed, all four provider rows completed 1,200/1,200 operations with zero timeouts/cleanup failures but missed 1,000 IOPS, and aggregate job `106522172018` failed closed. Cleanup correction `0282df64` is published through `229a9cd5`; fresh run `35664474430` is queued and requires terminal producer/aggregate review. |
...
| 2026-09-22 — Cleanup-corrected hosted qualification dispatch | Dispatched the next non-canceling CI matrix from the cleanup-corrected published tip. | ~0.25 h | Hosted run `35664474430` targets exact SHA `229a9cd5d1c79c6f13c7144f66d648e5eead0dd9`; W26 producer jobs are `106547013193` (`ozone`), `106547013256` (`ozone-compositions`), `106547013137` (`ozone-tidb`) and `106547013296` (`ozone-foundationdb`), all queued at dispatch; aggregate job not yet present. | Await terminal producer and aggregate output. Production remains **NO-GO** unless every configured provider, all end-to-end markers and the hard IOPS threshold pass on this one revision. |

## Publication record

Authoritative current update (2026-09-22): `origin/main` is
`229a9cd5d1c79c6f13c7144f66d648e5eead0dd9`, and this checkout matches it.
...
`0282df64` are included through the current concurrent-mainline tip. The 18-test focused ChunkedFs suite,
...
`85c31e86-1fc8-4a6d-bb95-ef2d956b9ed8` are complete with zero reportable
...
manual run `35664474430` is queued on `229a9cd5`. No queued, in-progress,
...
and fresh run `35664474430` is queued on `229a9cd5`. No queued, in-progress,
...
workflow jobs were still running at an earlier poll. Published tip `229a9cd5`
contains the lazy-atime/EOF, batching/cache, queue-safety, corrected
content-addressed contract and deduplicated-cleanup work with local test,
