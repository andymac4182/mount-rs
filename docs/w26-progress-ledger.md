...
| Latest hosted workflow | Fresh manual run `35676336466` selected exact hosted revision `503f3f757c8b599e11dbd0c5f99f8cfcc67aee5`, which includes W26 implementation commit `d05548e8`. At the initial live poll, W26 producer jobs `ozone` `106583542086`, `ozone-compositions` `106583542156`, `ozone-tidb` `106583542085` and `ozone-foundationdb` `106583542280` were queued; the aggregate was not yet present. This packet is pending and is not acceptance evidence. |
...
| Release/acceptance decision | W26 implementation and local qualification controls remain accepted within their documented scope; production rollout remains **NO-GO** because terminal run `35674425514` failed every hard 1,000-IOPS provider row and aggregate job `106579811902` failed closed. The single-flight R2 upload optimization is included in current `origin/main` `412c422e` via implementation SHA `d05548e8`; the hosted packet that precedes it is tied to exact SHA `a81a0827`, and fresh run `35676336466` is pending on exact SHA `503f3f75`. Security scan `739f4f5b-8cc4-4154-93f7-9e486745eab1` is sealed with zero reportable findings and complete changed-file coverage. No failed or pending packet is promoted, and no broader native, customer secure-runtime, Ozone backup/DR or release claim is made |
