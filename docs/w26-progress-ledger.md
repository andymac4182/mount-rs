...
| Repository | `mount-rs` |
| Snapshot base | `c719306` (provider-bounded key enumeration is published over concurrent mainline changes; the current CI run is still non-terminal and is not evidence yet) |
...
| Current acceptance state | The retained W26 packet on `9c098e5` remains accepted within its documented provider/platform boundaries. The newer provider-bounded KV/N-API implementation is locally green but its current-revision hosted CI and provider-parity evidence are not terminal |
...
| Current-head CI attempt | Implementation tip `c719306` is running in GitHub Actions CI run `35608567514`: `ozone` job `106361680847` is in progress, `ozone-tidb` job `106361681459` is in progress, `ozone-compositions` job `106361681255` is queued, and `ozone-foundationdb` job `106361681537` is queued. Fault-injection run `35608567519` is pending. No queued, in-progress, canceled or failed run is promoted to evidence; the retained W26 packet above remains the last accepted hosted result. |
