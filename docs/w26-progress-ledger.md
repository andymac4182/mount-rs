...
| Repository | `mount-rs` |
| Snapshot base | `92f900be` (current `origin/main` when this ledger chunk started; it includes the reconciled atomic whole-file write implementation commit `96a25f17` plus concurrent mainline work. The final remote tip after this documentation publication is recorded in the publication/session rows below) |
...
| Current acceptance state | The retained W26 packet on `9c098e5` remains the last accepted hosted result within its documented provider/platform boundaries. The W26-owned optimistic read/write paths remain published and the new atomic whole-file `FsDriver::write_file` contract plus specialized `ChunkedFs` implementation are now published in `96a25f17` and reconciled into `origin/main` at `92f900be`. The atomic path stages immutable blocks outside the metadata gate, flushes them, then performs one fenced namespace publication; optimistic conflicts retain the existing serialized open/write/close fallback. The full locked workspace test and strict Clippy commands both exited 0, including 15 ChunkedFs unit tests and 16 N-API tests; environment-gated native/provider rows remain skips, not passes. Replacement run `35641941218` is terminal for every W26 producer and aggregate: all four configured provider rows completed 1,200/1,200 lifecycle operations with zero timeouts/cleanup failures but missed the hard 1,000-IOPS target, so the producer and aggregate jobs failed closed and no pass marker was emitted. Focused security diff scan `11fa7e54-bb66-488d-a369-c1a7ac46da81` completed with zero reportable findings for all three changed files; its hosted/provider/customer coverage is partial and explicitly deferred. No current-tip hosted qualification has yet been run for `96a25f17`. No Live Cloudflare R2 or Live AWS S3 result is recorded. |
| Latest hosted workflow | Run `35641941218` on exact W26 code head `88b707ba0e8f91949eec47bca04af16313f56deb`: `ozone` `106473112763` passed; `ozone-compositions` `106473112920`, `ozone-tidb` `106473112915`, `ozone-foundationdb` `106473112409` and aggregate `w26-ozone-evidence` `106477164582` failed. The parent workflow remains `in_progress` because unrelated `native-fuse` and `node (macos-15-intel)` jobs are still running; that state cannot change the terminal W26 failure. |
| Current-head CI attempt | The last terminal packet improved measured lifecycle IOPS to SQLite/R2 85.70, PGlite/R2 94.04, TiDB/R2 14.50 and FoundationDB/R2 30.46, but all remain far below 1,000. Each row recorded 400 writes, 400 reads, 400 verified reads and 400 deletes; 1,200/1,200 operations succeeded with zero timeouts and cleanup failures. The new atomic whole-file path is intended to remove a second W26-owned metadata publication from the public `writeFile` benchmark lifecycle, but its hosted effect is not yet known. W26.15 remains open and the threshold is unchanged; a full current-tip packet is required after this implementation chunk. |
| Next hosted qualification | Pending dispatch against the published atomic-write code | The next retained CI packet must run `ozone`, `ozone-compositions`, `ozone-tidb`, `ozone-foundationdb` and `w26-ozone-evidence` on one revision containing `96a25f17`, retain all four provider JSON/log artifacts, and report whether the strict 1,000-IOPS gate closes. Until that terminal result exists, this chunk is local implementation evidence only | GitHub-hosted runners, Ozone/provider startup and artifact retention are external gates; queued, canceled, failed or skipped jobs are not evidence |
...
| Release/acceptance decision | W26 implementation and local qualification controls remain accepted within their documented scope; production rollout remains **NO-GO** because replacement run `35641941218` terminally failed every hard 1,000-IOPS provider row and no current-tip rerun exists yet. Latest focused security diff scan `11fa7e54-bb66-488d-a369-c1a7ac46da81` reports zero local reportable findings for the atomic-write implementation, with hosted/provider/customer coverage explicitly deferred. Earlier standard, policy, negative-path, strict-IOPS, evidence-packet and rollout-contract scans remain recorded above; no failed run is promoted, and no broader native, customer secure-runtime, Ozone backup/DR or release claim is made |
...
| W26.3a — SQLite and disk-backed PGlite composition over real Ozone | Functional composition complete; first concurrency remediation published; hard IOPS qualification failed again | 100% functional; 86% production qualification | Replacement `ozone-compositions` job `106473112920` on exact code head `88b707ba` completed all SQLite/R2 and PGlite/R2 lifecycle operations with zero timeout/cleanup failures but measured 85.70 and 94.04 IOPS against 1,000; both artifacts contain `IOPS_TARGET_NOT_MET` and no pass marker. The W26.15 implementation overlaps immutable block reads/writes outside the volume-wide metadata gate while preserving revision/base checks and serialized conflict fallback; local 8-test concurrency, 14 chunked-unit, full workspace test and strict Clippy gates pass. | Continue W26-owned metadata publication/concurrency work; retain the hard target and rerun the complete packet only after a substantive implementation change. | ~1–4 d follow-up performance/design; 0.5–1.5 d hosted review | Current Ozone fixture capacity/topology and customer target capacity remain external, but the first volume-wide serialization remediation is W26-owned and remains insufficient. |
...
| W26.3c — independent durable three-node FoundationDB composition over Ozone | Durable provider lifecycle and restart markers passed; first concurrency remediation failed the hard IOPS gate | 90% base; 82% extension | The arm64 durable FoundationDB proof and provider-backed bounded-listing contract remain green. Replacement hosted job `106473112409` completed all 1,200 lifecycle operations and measured 30.46 IOPS, with zero timeout/cleanup failures but `IOPS_TARGET_NOT_MET`; its artifact is diagnostic only. The new read/write overlap path and lifecycle shutdown barrier are covered by local concurrency tests and full workspace gates. Local FoundationDB binary linking remains blocked by missing `libfdb_c`. | Continue performance remediation while preserving durable restart, fencing, bounded-listing and cleanup assertions; rerun the provider packet only after a substantive change. | ~1–4 d shared concurrency follow-up; 0.75–1.5 h hosted review | FoundationDB client image/runtime, Ozone capacity/topology and customer secure durability remain external; local provider-native linking remains blocked. |
| W26.3d — durable multi-node TiDB composition over Ozone | Durable TiDB lifecycle and N-API bounded markers passed; first concurrency remediation failed the hard IOPS gate | 100% base; 82% extension | Replacement hosted job `106473112915` completed all 1,200 lifecycle operations and measured 14.50 IOPS, with zero timeout/cleanup failures but `IOPS_TARGET_NOT_MET`; its artifact is diagnostic only. The W26.15 path overlaps remote block I/O while retaining fencing/CAS and serial conflict fallback, with local full-workspace tests and Clippy green. | Continue performance remediation while preserving restart/ambiguous-commit/fencing/bounded-listing/cleanup assertions; rerun the provider packet only after a substantive change. | ~1–4 d shared concurrency follow-up; 0.75–1.5 h hosted review | GitHub-hosted TiDB/PD/TiKV, N-API runtime, Ozone capacity/topology and customer secure durability remain external. |
...
| W26.4b — CI wiring, pinned images, cleanup, and documentation | Implementation complete; replacement one-revision packet terminally failed the hard provider gate | 100% implementation; 58% hosted packet qualification | CI installs NAPI/PGlite with locked Rust dependencies, runs the Ozone composition matrix, Node/CLI coverage, the shipped HTTP server/client reopen path, and durable Ozone/TiDB/FoundationDB jobs. Run `35641941218` on exact code head `88b707ba` retained all four producer artifacts and aggregate job `106477164582` failed closed after each provider missed 1,000 IOPS; the overall workflow remains active only for unrelated native jobs. | Require a new substantive W26.15 implementation result, then rerun all producer jobs and `w26-ozone-evidence` on one revision; retain exact artifacts, policy markers, end-to-end markers, performance markers and cleanup before promotion. | ~1–4 d performance follow-up; 0.75–1.5 d hosted review per retained run | Workflow concurrency, hosted provider startup, Ozone capacity/topology, artifact service and customer TLS/IAM remain external; no failed/queued/skipped artifact is acceptance evidence. |
| W26.14 — complete end-to-end Ozone evidence-surface gate | Implementation complete; terminal replacement packet failed performance; customer admission open | 100% implementation; 40% production qualification | Expanded `scripts/verify-w26-ozone-evidence-packet.mjs` to require gateway health/ready/restart, SQLite/PGlite composition and bounded listing, Rust CLI, Node provider matrix/CLI, remote HTTP CLI, TiDB Rust/N-API seed/reopen and FoundationDB Rust/N-API seed/reopen markers. Local benchmark/evidence tests, Node/shell/YAML checks, diff checks, full locked workspace tests and strict Clippy pass; focused ChunkedFs security scan `e7c03690-856b-42ae-8d47-8af0dc788a54` found zero reportable findings with hosted/customer coverage deferred. Replacement run `35641941218` bound to `88b707ba` completed W26 producer jobs and aggregate `106477164582`, but all four hard IOPS rows failed, so no packet result is promoted. | Close the packet only after a post-remediation run has terminal pass evidence from every producer and aggregate; retain native/mount and customer secure-runtime evidence separately. | ~1–4 d W26 performance follow-up; ~0.75–1.5 d hosted review; ~0.75–1.25 h security review | Ozone fixture capacity/topology, hosted provider/N-API execution, artifact authorization, native mount runners, customer TLS/IAM/rotation and measured SLO/RPO/RTO remain external. |
| W26.15 — per-drive 1,000-IOPS qualification and safe performance remediation | Atomic whole-file write implementation published; hosted qualification pending; **NO-GO** | 58% W26-owned implementation / 24% hosted qualification | Implementation commit `96a25f17`, reconciled/pushed at `92f900be`, adds the public `FsDriver::write_file` contract, N-API adoption and a `ChunkedFs` optimistic block-stage/flush/one-publication path with serialized conflict fallback. Local 15-test ChunkedFs suite, full locked workspace tests, strict workspace Clippy, N-API check, storage benchmark unit tests, formatting/diff checks and security scan `11fa7e54-bb66-488d-a369-c1a7ac46da81` all pass/return zero findings. Hosted run `35641941218` remains the last terminal performance result and failed all four provider rows at 85.70/94.04/14.50/30.46 IOPS; no hosted result exists yet for `96a25f17`. | Dispatch and inspect one-revision Ozone packet on the published code; retain exact provider metrics, latency, resource envelope, artifacts, cleanup and complete-surface markers. If the target remains open, continue metadata-publication/provider-latency work without weakening the threshold. | ~1.5–4 d implementation/design; ~0.75–1.5 d hosted review per rerun; external queue time excluded | GitHub runner/provider startup, Ozone fixture capacity/topology, provider-native performance, artifact retention, customer secure topology and capacity are external gates; no staging environment exists. |
...
| --- | --- | --- | --- |
| W26 integration readiness | **NO-GO** | The retained green packet is still `35585066458`, but replacement run `35641941218` on exact W26 code head `88b707ba` also failed closed: all four configured provider rows completed 1,200/1,200 lifecycle operations with zero timeouts/cleanup failures yet measured only 85.70/94.04/14.50/30.46 IOPS against 1,000, and aggregate job `106477164582` rejected the missing pass markers. W26-owned packet, policy, metric, retention and end-to-end verifier controls are implemented and locally tested; W26.15 is open for further metadata-publication/performance remediation | A current-tip retained revision must close W26.15 and produce terminal pass markers for every configured provider plus the full end-to-end packet; customer Ozone deployment, DR and release remain explicit external dependencies |
...
| Qualification baseline | Historical green packet only | Revision `9c098e5` and hosted run `35585066458` are the last accepted packet within their documented boundaries. Replacement run `35641941218` on exact W26 code head `88b707ba` is diagnostic and failed the hard provider-performance gate; current `origin/main` needs another retained qualification after further W26.15 work | Any performance/concurrency change gets a fresh full packet on the tested revision; no failed, queued, canceled or skipped result is promoted |
...
| P8 — load, capacity, soak and cost envelope | Hosted/provider CI plus W26.15 remediation | Hard-threshold and one-revision packet gates implemented; replacement packet failed performance; atomic write optimization pending hosted proof | 44% | The benchmark records successful write+read+delete lifecycle IOPS, supports a hard `--min-iops` threshold, strict `--require-configured` provider qualification and redacted JSON. The fixed 4 KiB/400-iteration/concurrency-64 profile is enforced. Replacement run `35641941218` completed all 1,200 lifecycle operations per row with zero timeouts/cleanup failures but measured only 85.70/94.04/14.50/30.46 IOPS; aggregate job `106477164582` failed closed. Published commit `96a25f17` now removes an avoidable public `writeFile` metadata publication through a fenced `ChunkedFs` whole-file path, but no hosted result exists for it yet. | Dispatch and retain a current-tip all-provider packet; if the target remains open, continue correctness-preserving metadata-publication/provider-latency work. Then retain p95/p99 latency, errors, CPU/memory and topology, and add soak/capacity variants before treating the target as qualified | 1.5–4 d W26 performance follow-up; 1–2 d hosted review; soak effort provisional | Ozone fixture capacity/topology, stable hosted runners, artifact service, provider quotas and customer 99.99% capacity remain external; do not lower the target |
...
| --- | --- | --- |
| Hosted result inspection | 0.5–1.5 d engineering plus external queue time | Historical green packet `35585066458` remains the last accepted hosted result. Replacement run `35641941218` on exact W26 code head `88b707ba` is terminal for all W26 jobs but diagnostic: base Ozone passed, all four provider rows completed 1,200/1,200 operations with zero timeouts/cleanup failures but missed 1,000 IOPS, and aggregate job `106477164582` failed closed. Atomic whole-file write commit `96a25f17` is published but has no hosted result; a new current-tip retained packet is required. |
...
| Performance and end-to-end matrix (P8, P11, P14, W26.15) | 12–32 d engineering | The workload and packet are implemented, but both terminal diagnostic packets failed the hard provider target. The read/write-overlap design improved IOPS but remained far below target; atomic whole-file write commit `96a25f17` is the next safe metadata-publication optimization and must be measured before further design decisions. Requires a current-tip packet, possible additional provider-latency work or production-like Ozone capacity qualification, stable hosted CI runners, native runners and a one-revision audit. |
...
and terminal diagnoses above: run `35635486040` and replacement run
`35641941218` are failed diagnostic packets, W26.15 is open, and no queued,
in-progress, canceled or failed job is promoted to acceptance. The parent
workflow for `35641941218` remains active only because unrelated native jobs
are still running; that does not make its terminal W26 producer or aggregate
failures pending or successful.
