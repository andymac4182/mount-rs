...
partially completed targeted packet remain diagnostic; neither can be promoted
to production acceptance.

| Field | Current value |
| --- | --- |
| Shared build-on tip | `origin/main` = `9c44d2a4210bcc98d752319aae9b06f65d370337`, verified with `git ls-remote`; the worktree is clean. The current W26 implementation code is the ancestor `5e3680117c26f5b7bb1b9280eec65a350b3dd2f7`; other threads should build on the full `origin/main` tip. |
...
| Latest hosted evidence | Targeted run `35693778762` selected earlier SHA `b3fb7988bce1edaafbd44f2adf22bb217ca99671`: base job `106636116163` passed, compositions `106636116105` failed, TiDB `106636116197` failed, and FoundationDB `106636116201` was still in progress at capture; the run-level status was incorrectly still queued and no aggregate existed. The new push run `35694753908` selected current SHA `5e368011` and was pending at capture. |
| Production decision | **NO-GO**. The current code is not hosted-qualified; the latest terminal packet had only PGlite above the 1,000-IOPS-per-drive threshold, while SQLite, TiDB and FoundationDB missed it. Customer deployment, secure Ozone topology, 99.99% availability, five-minute RPO/RTO, backup/DR and release ownership remain explicit external boundaries. |
| Next action | Publish this ledger/tracker chunk, let CI run on exact `5e368011`, retrieve terminal W26 producer and aggregate evidence, and update acceptance only from one-revision all-provider/end-to-end results. Preserve the native `fdb_c` gate as external until hosted FoundationDB qualification is terminal. |
...
| --- | --- | --- | --- | ---: | --- |
| W26.1 — pinned Ozone gateway and health/readiness harness | **PASS / 100% implementation; current exact-SHA recheck pending** | Targeted run `35693778762`, base job `106636116163` passed on the preceding ledger SHA; prior terminal packet also passed gateway, readiness, restart/reopen and cleanup markers | Reconfirm markers on current `5e368011` packet | 0–1 h review | Hosted image/runner and customer Ozone topology |
| W26.2 — immutable block contract through Ozone S3 gateway | **IMPLEMENTED / 100% code; current exact-SHA acceptance pending** | Prior composition/base logs passed create/CAS, binary/range read, stopped-gateway fault, restart/reopen and owned-cleanup markers; targeted compositions job failed before an all-surface acceptance boundary | Reconfirm all markers on current exact revision; retain failures as non-acceptance evidence | 0.5–1 d review | Ozone gateway capacity, customer TLS/IAM and tenant isolation |
| W26.3a — SQLite/R2 composition | **FAIL / 100% implementation; hosted performance unresolved** | Latest terminal row: `213.947686` IOPS, 1,200/1,200, timeout `0`, cleanup `0`; prior packet reached `1,325.636778`, proving high hosted variance | Diagnose write variance without changing the hard target; requalify exact current SHA | 0.5–1.5 d per review/remediation cycle | Hosted runner variance, Ozone/provider capacity |
| W26.3b — PGlite/R2 composition | **PASS row / 100% implementation; all-provider acceptance open** | Latest row: `2,065.669446` IOPS, 1,200/1,200, timeout `0`, cleanup `0`; Node/Rust/CLI/HTTP/reopen markers passed | Retain the pass marker beside all other providers in a one-revision aggregate | 0.5–1 d review | Hosted PGlite runtime, Ozone latency and runner capacity |
| W26.3c — FoundationDB/R2 composition | **IMPLEMENTED / 100% code; hosted performance unresolved** | `5e368011` shares the protected authority read with the metadata transaction; latest terminal pre-chunk row was `363.254472` IOPS with 1,200/1,200, timeout `0`, cleanup `0`; targeted FDB job `106636116201` was still in progress on the preceding SHA | Complete current exact-SHA FoundationDB qualification and compare transaction/read latency without weakening fencing | 0.5–1.5 d per review/remediation cycle | Native `fdb_c`, hosted FoundationDB image/client, Ozone topology and durability |
| W26.3d — TiDB/R2 composition | **IMPLEMENTED / 100% code; hosted performance unresolved** | `4098c7df` and the current mainline include the autocommit CAS chunk; latest pre-chunk row was `333.356025` IOPS with all 1,200 lifecycle operations and TiDB/reopen markers passing; targeted TiDB job `106636116197` failed on the preceding SHA | Requalify current exact SHA and compare IOPS/p95; preserve fencing and ambiguous-commit markers | 1–3 d per review/remediation cycle | Hosted TiDB/PD/TiKV, Ozone topology and provider-native latency |
...
| W26.11 — one-revision evidence aggregation | **IMPLEMENTED / 100% local verifier; current packet pending** | Aggregate `106631474430` rejected missing `OZONE_IOPS_PASS` for SQLite/R2 and PGlite/R2; targeted run `35693778762` had no aggregate at capture; no provider failure was promoted | Run aggregate against exact `5e368011` artifacts and require all producer markers | 0.5–1.5 d review | Workflow scheduling and artifact availability |
...
| W26.14 — complete end-to-end packet surface enforcement | **IMPLEMENTED / 100% local control; hosted acceptance open** | Aggregate requires gateway, provider compositions, Rust/Node/CLI/HTTP, bounded listing, TiDB/FDB restart/N-API and cleanup markers; latest aggregate failed closed on performance marker absence and the targeted packet had no aggregate at capture | Produce one terminal all-surface packet on exact `5e368011` | 0.75–1.5 d review | Native runners, provider fixtures and CI orchestration |
| W26.15 — P8 per-drive 1,000-IOPS gate | **OPEN / 100% verifier implementation; 1 of 4 latest provider rows passed** | Latest rows: SQLite `213.947686` FAIL, PGlite `2,065.669446` PASS, TiDB `333.356025` FAIL, FoundationDB `363.254472` FAIL; all 1,200/1,200 with zero timeout/cleanup failures | Close all four providers and aggregate on one exact SHA; never lower, average or skip the hard target | 1.5–4 d per remediation cycle plus external queue | Hosted runner/provider capacity, Ozone topology and artifact retention |
...
| Ozone FoundationDB | `106636116201` | in progress at capture | Must pass strict lockfile/preflight, durable restart, bounded-listing, cleanup and >=1,000 IOPS |
...
| Run | [GitHub Actions run `35693778762`](https://github.com/andymac4182/mount-rs/actions/runs/35693778762) — the run API still reported `queued` at capture even though base/compositions/TiDB producer jobs had become terminal; producer results are not acceptance evidence until the complete packet is terminal. |
| Concurrent shared tip | `origin/main` is now `5e3680117c26f5b7bb1b9280eec65a350b3dd2f7`; push-triggered run `35694753908` selected that exact code and was `pending` at capture. This is a separate current-code build boundary; the older targeted run remains useful only for its exact-SHA diagnostic results. |
| Acceptance state | **NO-GO / partial non-acceptance**. The targeted compositions and TiDB jobs failed, FoundationDB was still running, and no aggregate existed; any failed, skipped, canceled, incomplete, missing-artifact or missing-marker producer/aggregate result remains fail-closed. |
| Next action | Finish polling run `35693778762`, retrieve every retained W26 artifact/log, then poll `35694753908` and update the ledger only after current-code provider jobs and its aggregate are terminal. |

### Session time log — hosted dispatch

| Date / phase | Activity | Engineering time | External wait / gate time | Result |
| --- | --- | ---: | ---: | --- |
| 2026-09-22 — Exact-SHA W26 dispatch | Dispatched `ci.yml` from the published ledger SHA, recorded the four W26 producer job IDs, and reconciled the subsequent current-code push run. | ~0.25–0.5 h evidence coordination | ~1–3 h provisional CI queue/provider startup | Run `35693778762` is a partial diagnostic packet for the TiDB chunk; current-code run `35694753908` is the qualification boundary for `5e368011`. Production remains **NO-GO** until terminal all-provider/end-to-end evidence passes. |
