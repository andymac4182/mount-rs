...
| Repository | `mount-rs` |
| Snapshot base | `88b707ba` (current `origin/main`; it carries the W26 evidence-packet verifier, complete-surface marker checks, metric-integrity checks, credential-free rollout contract, optimistic write/read concurrency path and concurrent mainline changes; hosted run `35641941218` is active and no pending result is promoted) |
...
| Current acceptance state | The retained W26 packet on `9c098e5` remains the last accepted hosted result within its documented provider/platform boundaries. The W26-owned `ChunkedFs` optimistic write path (published implementation commit `a2075af0`, merged tip `cd6dea28`) now overlaps immutable block I/O while preserving revision/base checks, lease fencing and serialized publication; the follow-up read path and blocked-read/shutdown regression are published in implementation commit `c4378dc1` at merged tip `88b707ba`. The full locked workspace test and strict Clippy commands both exited 0; the new 8-test concurrency suite, 14 chunked unit tests and full workspace tests are green, while environment-gated native/provider rows remain skips, not passes. Manual run `35635486040` is terminally failed on head `cfe7e29e001dc01f2fa430a54bc8981b44db0b05`: all four configured provider rows failed the hard 1,000 target and the aggregate verifier failed closed. Fresh hosted run `35641941218` is dispatched on `88b707ba` but remains non-terminal, so it is not acceptance evidence. Focused security scan `e7c03690-856b-42ae-8d47-8af0dc788a54` completed with zero reportable findings for the changed ChunkedFs file; its coverage is partial and explicitly defers hosted/provider/customer controls. No Live Cloudflare R2 or Live AWS S3 result is recorded. |
| Latest hosted workflow | Historical run `35635486040` remains the last terminal W26 diagnostic and failed closed. Fresh manual run `35641941218` is active on head `88b707ba0e8f91949eec47bca04af16313f56deb`: `ozone` job `106473112763` and `ozone-compositions` `106473112920` are in progress, `ozone-foundationdb` `106473112409` is in progress and `ozone-tidb` `106473112915` is queued at the latest inspection; no producer or aggregate result is promoted until every job is terminal and the aggregate passes. |
| Current-head CI attempt | The prior terminal run measured SQLite/R2 61.97, PGlite/R2 63.56, TiDB/R2 14.14 and FoundationDB/R2 23.07 IOPS against the hard target of 1,000, with 1,200/1,200 lifecycle operations and zero timeouts/cleanup failures. The W26-owned remediation now overlaps remote immutable reads and writes outside the volume-wide metadata gate, reacquires the gate for fenced revision-checked publication, retains serialized conflict fallback, and blocks shutdown until optimistic operations finish. Local correctness evidence is green; hosted performance result for the exact published tip is still pending in run `35641941218`. |
...
| Release/acceptance decision | W26 implementation and local qualification controls remain accepted within their documented scope; production rollout remains **NO-GO** because the last terminal packet failed all four hard 1,000-IOPS rows and the replacement run `35641941218` is not yet terminal. Latest focused security diff scan `e7c03690-856b-42ae-8d47-8af0dc788a54` reports zero local reportable findings for the concurrency change, with hosted/provider/customer coverage explicitly deferred. Earlier standard, policy, negative-path, strict-IOPS, evidence-packet and rollout-contract scans remain recorded above; no queued, in-progress or failed run is promoted, and no broader native, customer secure-runtime, Ozone backup/DR or release claim is made |
...
All provider jobs completed the requested lifecycle calls successfully, so this is a performance qualification failure rather than a lifecycle-correctness or cleanup failure. The current implementation path serializes the full operation—including remote block calls and whole-namespace publication—under `ChunkedFs`'s volume-wide async gate. W26 must either implement and verify a safe concurrency/publication design that preserves fencing, revision CAS, immutable-block ordering and POSIX semantics, or obtain an Ozone/customer capacity qualification showing that the CI fixture is not representative. Until then, the 1,000-IOPS gate is open and production readiness remains **NO-GO**.

The first W26.15 remediation chunk is now published. `ChunkedFs::write_at` overlaps immutable block rewrite/flush work outside the metadata gate, then reacquires the gate and commits only when the inode, file size and immutable layout still match the captured base; conflicts fall back to the serialized path. `ChunkedFs::read_at` applies the same lifecycle/read-overlap pattern, revision/base check and safe orphan-atime handling. A lifecycle read/write barrier ensures shutdown waits for these optimistic operations before fencing and releasing the writer lease. Local evidence for implementation commit `c4378dc1` is green: 8 concurrency tests including blocked-read gate release and shutdown ordering, 14 chunked unit tests, full locked workspace tests and strict workspace Clippy. Fresh hosted run `35641941218` on merged tip `88b707ba` is the required terminal performance check and remains pending.
...
| W26.3a — SQLite and disk-backed PGlite composition over real Ozone | Functional composition complete; first concurrency remediation published; hard IOPS qualification pending fresh hosted result | 100% functional; 88% production qualification | Terminal `ozone-compositions` job `106451808739` on run `35635486040` completed all SQLite/R2 and PGlite/R2 lifecycle operations with zero timeout/cleanup failures but measured 61.97 and 63.56 IOPS against 1,000; its artifact remains diagnostic only. The W26.15 implementation commit `c4378dc1` now overlaps immutable block reads/writes outside the volume-wide metadata gate while preserving revision/base checks and serialized conflict fallback. Local 8-test concurrency, 14 chunked-unit, full workspace test and strict Clippy gates pass. Replacement run `35641941218` on `88b707ba` is active, not acceptance evidence. | Inspect terminal SQLite/R2 and PGlite/R2 metrics and pass markers from run `35641941218`; continue W26-owned concurrency work if either row remains below 1,000 or the aggregate fails. | ~1–4 d follow-up performance/design; 0.5–1.5 d hosted review | Current Ozone fixture capacity/topology and customer target capacity remain external, but the prior volume-wide serialization was W26-owned and is under remediation. |
...
| W26.3c — independent durable three-node FoundationDB composition over Ozone | Durable provider lifecycle and restart markers passed; concurrency remediation published; hard IOPS qualification pending fresh hosted result | 90% base; 84% extension | The arm64 durable FoundationDB proof and provider-backed bounded-listing contract remain green. Prior hosted job `106451808796` completed all 1,200 lifecycle operations and bounded seed/reopen markers but measured 23.07 IOPS; its artifact is diagnostic only. The new read/write overlap path and lifecycle shutdown barrier are covered by local concurrency tests and full workspace gates. Fresh `ozone-foundationdb` job `106473112409` in run `35641941218` is still in progress. Local FoundationDB binary linking remains blocked by missing `libfdb_c`. | Inspect the terminal FoundationDB/R2 artifact and preserve durable restart, fencing, bounded-listing and cleanup assertions; continue performance remediation if the hard threshold remains open. | ~1–4 d shared concurrency follow-up; 0.75–1.5 h hosted review | FoundationDB client image/runtime, Ozone capacity/topology and customer secure durability remain external; local provider-native linking remains blocked. |
| W26.3d — durable multi-node TiDB composition over Ozone | Durable TiDB lifecycle and N-API bounded markers passed; concurrency remediation published; hard IOPS qualification pending fresh hosted result | 100% base; 84% extension | Prior hosted job `106451808745` completed all 1,200 lifecycle operations and markers but measured 14.14 IOPS; its artifact is diagnostic only. The new W26.15 path overlaps remote block I/O while retaining fencing/CAS and serial conflict fallback, with local full-workspace tests and Clippy green. Fresh `ozone-tidb` job `106473112915` in run `35641941218` is queued and has no result yet. | Inspect the terminal durable TiDB/R2 artifact, preserve restart/ambiguous-commit/fencing/bounded-listing/cleanup assertions, and continue performance remediation if the hard threshold remains open. | ~1–4 d shared concurrency follow-up; 0.75–1.5 h hosted review | GitHub-hosted TiDB/PD/TiKV, N-API runtime, Ozone capacity/topology and customer secure durability remain external. |
...
| W26.4b — CI wiring, pinned images, cleanup, and documentation | Implementation complete; replacement one-revision packet active after first concurrency remediation | 100% implementation; 60% hosted packet qualification | CI installs NAPI/PGlite with locked Rust dependencies, runs the Ozone composition matrix, Node/CLI coverage, the shipped HTTP server/client reopen path, and durable Ozone/TiDB/FoundationDB jobs. The prior run `35635486040` retained all four producer artifacts and failed closed on missing `OZONE_IOPS_PASS`; the replacement packet `35641941218` is bound to merged tip `88b707ba` and remains non-terminal. | Require all producer jobs and `w26-ozone-evidence` to terminate successfully on one revision; retain exact artifacts, policy markers, end-to-end markers, performance markers and cleanup before promotion. | ~1–4 d performance follow-up; 0.75–1.5 d hosted review per retained run | Workflow concurrency, hosted provider startup, Ozone capacity/topology, artifact service and customer TLS/IAM remain external; no failed/queued/skipped artifact is acceptance evidence. |
| W26.14 — complete end-to-end Ozone evidence-surface gate | Implementation complete; replacement packet active after first concurrency remediation; customer admission open | 100% implementation; 42% production qualification | Expanded `scripts/verify-w26-ozone-evidence-packet.mjs` to require gateway health/ready/restart, SQLite/PGlite composition and bounded listing, Rust CLI, Node provider matrix/CLI, remote HTTP CLI, TiDB Rust/N-API seed/reopen and FoundationDB Rust/N-API seed/reopen markers. Local benchmark/evidence tests, Node/shell/YAML checks, diff checks, full locked workspace tests and strict Clippy pass; the focused ChunkedFs security scan `e7c03690-856b-42ae-8d47-8af0dc788a54` found zero reportable findings with hosted/customer coverage deferred. Historical run `35635486040` failed closed on all four hard IOPS rows. Replacement run `35641941218` is bound to `88b707ba` and remains active, so no packet result is promoted. | Close the current hosted packet only after every producer and aggregate job emits terminal pass evidence; if any hard IOPS row remains below 1,000, continue W26.15 rather than weakening or skipping it. Retain native/mount and customer secure-runtime evidence separately. | ~1–4 d W26 performance follow-up; ~0.75–1.5 d hosted review; ~0.75–1.25 h security review | Ozone fixture capacity/topology, hosted provider/N-API execution, artifact authorization, native mount runners, customer TLS/IAM/rotation and measured SLO/RPO/RTO remain external. |
...
| --- | --- | --- | --- |
| W26 integration readiness | **NO-GO** | The retained green packet is still `35585066458`, but the newer terminal run `35635486040` failed closed: all four configured provider rows completed their lifecycle samples yet missed the hard 1,000-IOPS target, and aggregate job `106458415293` rejected the missing pass marker. W26-owned packet, policy, metric, retention and end-to-end verifier controls are implemented and locally tested; W26.15 is open for the performance remediation/qualification path | A current-tip retained revision must close W26.15 and produce terminal pass markers for every configured provider plus the full end-to-end packet; customer Ozone deployment, DR and release remain explicit external dependencies |
...
| Qualification baseline | Historical green packet only | Revision `9c098e5` and hosted run `35585066458` are the last accepted packet within their documented boundaries. Terminal run `35635486040` on `cfe7e29e001dc01f2fa430a54bc8981b44db0b05` is diagnostic and failed the hard provider-performance gate; the current `origin/main` needs a fresh retained qualification after W26.15 | Any performance/concurrency change gets a fresh full packet on the tested revision; no failed, queued, canceled or skipped result is promoted |
...
| P8 — load, capacity, soak and cost envelope | Hosted/provider CI plus W26.15 remediation | Hard-threshold and one-revision packet gates implemented; replacement packet active | 42% | The benchmark records successful write+read+delete lifecycle IOPS, supports a hard `--min-iops` threshold, strict `--require-configured` provider qualification and redacted JSON. The fixed 4 KiB/400-iteration/concurrency-64 profile is enforced. Terminal run `35635486040` completed all 1,200 lifecycle operations per row with zero timeouts/cleanup failures but measured only 61.97/63.56/14.14/23.07 IOPS; aggregate job `106458415293` failed closed. W26.15 now overlaps remote immutable reads/writes outside the metadata gate, with local concurrency, full workspace tests and strict Clippy green; replacement run `35641941218` on `88b707ba` is not terminal. | Inspect the replacement packet; if any provider remains below 1,000, continue correctness-preserving concurrency/publication work or obtain production-like Ozone capacity/topology evidence. Then retain p95/p99 latency, errors, CPU/memory and topology, and add soak/capacity variants before treating the target as qualified | 1–4 d W26 performance follow-up; 1–2 d hosted review; soak effort provisional | Ozone fixture capacity/topology, stable hosted runners, artifact service, provider quotas and customer 99.99% capacity remain external; do not lower the target |
...
| P14 — final W26 integration-readiness review | W26 implementation / hosted CI / handoff | NO-GO review documented; cannot close while W26.15 is open | 24% | The terminal packet and failed-closed aggregate are audited: base Ozone passed, all four provider lifecycles completed but missed the hard IOPS target, and the focused current-tip security scan found zero local reportable findings with hosted/customer controls deferred. The replacement packet `35641941218` is active on `88b707ba`; local read/write overlap, shutdown ordering and full regression evidence are green, but no hosted performance result exists yet. | Close W26.15, rerun/retain one current-tip packet with every provider and end-to-end marker, attach provider/platform/security/performance evidence, list customer/Ozone dependencies and hand off an explicit integration-ready or NO-GO decision to the release/deployment streams | 1–2 d review after performance gate; 1–4 d W26.15 follow-up | All W26-owned CI gates plus external customer Ozone secure topology, capacity/SLO, DR and release-stream confirmations |
...
- [ ] P8/W26.15: close the per-drive 1,000-IOPS CI workload with latency,
  ...
  correctly failed closed on terminal run `35635486040` after measuring
  SQLite/R2 61.97, PGlite/R2 63.56, TiDB/R2 14.14 and FoundationDB/R2 23.07
  IOPS. Resolve or qualify the shared concurrency/capacity blocker, then rerun
  the full packet; do not lower the threshold or convert failed rows to skips.
...
| --- | --- | --- |
| Hosted result inspection | 0.5–1.5 d engineering plus external queue time | Historical green packet `35585066458` remains the last accepted hosted result. Terminal run `35635486040` on `cfe7e29e001dc01f2fa430a54bc8981b44db0b05` is diagnostic: base Ozone passed, all four provider rows completed but missed 1,000 IOPS, and aggregate job `106458415293` failed closed. A current-tip retained packet is required after W26.15 remediation or explicit Ozone capacity qualification. |
...
| Performance and end-to-end matrix (P8, P11, P14, W26.15) | 12–32 d engineering | The workload and packet are implemented, but terminal Ozone evidence failed the hard provider target. Requires a safe concurrency/publication decision or production-like Ozone capacity qualification, stable hosted CI runners, native runners and a one-revision audit. |
...
| 2026-09-22 — Optimistic ChunkedFs read-path remediation | Moved remote block reads outside the volume-wide metadata gate; atime publication now uses a revision/base check and safe orphan handling, while the lifecycle barrier covers shutdown. Added a blocked-read gate-release/shutdown regression. | ~1.5–3 h | 8 concurrency tests, 14 chunked unit tests, full locked workspace tests, strict workspace Clippy, formatting and diff checks passed; hosted execution pending | Implementation commit `c4378dc1` was reconciled with concurrent mainline changes and pushed in merged tip `88b707ba`. Fresh hosted run `35641941218` is active on this exact tip; no producer or aggregate result is promoted. |
...
| 2026-09-22 — Replacement hosted W26 packet dispatch | Dispatched CI workflow `35641941218` against merged `origin/main` tip `88b707ba` after the write/read concurrency remediation. | ~0.25 h | Latest inspection: `ozone` `106473112763` in progress, `ozone-compositions` `106473112920` in progress, `ozone-foundationdb` `106473112409` in progress and `ozone-tidb` `106473112915` queued; aggregate not yet terminal | Wait for terminal producer and aggregate results; preserve the hard 1,000-IOPS gate and update this ledger only from terminal evidence. |
...
and terminal diagnosis above: run `35635486040` is terminally failed, its
provider artifacts are diagnostic only, W26.15 is open, replacement run
`35641941218` on `88b707ba` is still non-terminal, and no queued, in-progress,
canceled or failed job is promoted to acceptance.
...
`35641941218` is the authoritative pending hosted check for this code.
