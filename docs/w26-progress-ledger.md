...
| Repository | `mount-rs` |
| Snapshot base | `12ba117` (latest published origin/main head carrying the strict provider-configuration IOPS gate, artifact-integrity verifier, fail-closed artifact retention and concurrent mainline changes; current hosted workflows are pending or queued and are not evidence) |
| Checklist completion | 7 of 7 W26 tracker rows checked: 100% |
| Provisional execution completion | W26 qualification: 100%; production-rollout readiness: 40% (scope, benchmark matrix, lifecycle safety, bounded remote enumeration, provider-bounded KV/N-API contract, durable-provider bounded-listing tests, strict provider-specific hard-threshold IOPS wiring, fail-closed artifact/profile verification and retention, expanded credential-free all-provider production-config policy, current-revision security reviews and CI acceptance baseline captured; no terminal production gates yet). Customer deployment, native, provider-durability and release-stream gates remain separately bounded |
| Current acceptance state | The retained W26 packet on `9c098e5` remains accepted within its documented provider/platform boundaries. The newer provider-bounded KV/N-API implementation, unstorage parity fixtures, SQLite/PGlite/TiDB/FoundationDB Ozone bounded-listing assertions, FoundationDB and TiDB Ozone Node/N-API bounded assertions, exact-limit regression, SDK debug redaction, dedicated TiDB/FoundationDB Ozone IOPS harnesses, strict provider-configuration qualification, fail-closed IOPS artifact/profile verification and retention policy, and expanded credential-free production-config policy are locally green, syntax-checked or compile-checked. Latest published head `12ba117` has CI run `35628709359` pending, Fault injection `35628709207` queued, W08 release targets `35628709298` pending and W08 release policy `35628709428` pending; W04 production policy `35628709324` succeeded but is unrelated, and no Live Cloudflare R2 or Live AWS S3 run was recorded for this tip. None is W26 acceptance evidence. Focused IOPS diff scans `5fc5a943-07bb-4979-9f37-efd87a7f505e`, `60269206-bb22-4b78-aaf7-f05d16ffcca0` and `1d97028f-e153-4b49-9fac-c3a8c1fc1117`, production-config diff scan `d74e3e86-2e0a-45cf-9819-e31f428eb5d4`, negative-path scan `7addeeb5-4601-4951-aca9-becffb9bd4b9` and artifact-retention scan `18010cad-ed69-4da3-b0a9-57163091e878` found zero reportable findings with hosted/provider coverage explicitly deferred; `MOUNTX_SOURCE` parity and hosted/customer security controls remain bounded |
...
| Current-head CI attempt | Published tip `12ba117` has CI run `35628709359` pending, Fault injection run `35628709207` queued, W08 release targets run `35628709298` pending and W08 release policy run `35628709428` pending; W04 production policy run `35628709324` succeeded but is unrelated, and no Live Cloudflare R2 or Live AWS S3 run was recorded for this tip. Only terminal W26 jobs with strict provider markers and retained artifacts can be promoted; the retained W26 packet above remains the last accepted hosted result. |
...
| Production rollout track | Open, currently **NO-GO**; 0 of 15 production gates are terminally accepted. The 40% figure reflects scope decisions, local hardening, provider-boundary implementation, strict provider-configuration and artifact-integrity/retention qualification, expanded credential-free security policy and an acceptance baseline, not deployable readiness |
...
| P8 — load, capacity, soak and cost envelope | Hosted/provider CI | Per-provider hard-threshold gates implemented; terminal hosted results pending | 47% | The benchmark records successful write+read+delete lifecycle IOPS, supports a hard `--min-iops` threshold, strict `--require-configured` provider qualification and redacted JSON. The generic `scripts/test-ozone.sh` lane requests SQLite/R2 and PGlite/R2 only, with strict mode; dedicated TiDB and FoundationDB lanes request their own rows with the same strict mode. Wrapper settings cannot lower the target below 1,000 or weaken the 4 KiB/400-iteration/concurrency-64 production profile. `scripts/verify-w26-ozone-iops-artifact.mjs` requires the exact provider set, zero skipped/configuration-failed rows, successful cleanup, per-size lifecycle success, target attainment and a valid retained JSON artifact before the generic or dedicated pass marker is emitted. The three W26 IOPS uploads now fail the CI job when an expected JSON artifact is absent. Published code chunk `de9d267` at `414a469` extends the dedicated durable TiDB and FoundationDB Ozone jobs to retain provider-specific artifacts; `66f3670` at `00d2b80` adds the fixed-profile/artifact gate and `c0f8370` at `12ba117` makes retention fail closed. Benchmark unit tests, missing-provider regression, verifier negative cases, shell syntax, YAML parsing and rebuilt N-API chunked lifecycle checks pass; no live local IOPS result is claimed. | Run terminal hosted `ozone-compositions`, `ozone-tidb` and `ozone-foundationdb` jobs; retain `artifacts/ozone-iops.json`, `artifacts/ozone-tidb-iops.json` and `artifacts/ozone-foundationdb-iops.json`; verify verifier PASS markers, exact profile, strict no-skip behavior, artifact upload, p95/p99 latency, errors, CPU/memory and topology, then add soak/capacity variants before treating the target as qualified | 3–8 d after harness implementation | Stable hosted CI runners, Ozone fixture startup, provider quotas/topologies and enough runtime for meaningful soak; CI does not prove customer capacity or 99.99%/RPO/RTO |
