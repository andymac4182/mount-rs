...
| Repository | `mount-rs` |
| Snapshot base | `80344aa` (latest published origin/main head carrying the strict provider-configuration IOPS gate plus concurrent mainline changes; current hosted workflows are cancelled or in progress and are not evidence) |
| Checklist completion | 5 of 5 W26 tracker rows checked: 100% |
| Provisional execution completion | W26 qualification: 100%; production-rollout readiness: 37% (scope, benchmark matrix, lifecycle safety, bounded remote enumeration, provider-bounded KV/N-API contract, durable-provider bounded-listing tests, strict provider-specific hard-threshold IOPS wiring, expanded credential-free all-provider production-config policy, current-revision security reviews and CI acceptance baseline captured; no terminal production gates yet). Customer deployment, native, provider-durability and release-stream gates remain separately bounded |
| Current acceptance state | The retained W26 packet on `9c098e5` remains accepted within its documented provider/platform boundaries. The newer provider-bounded KV/N-API implementation, unstorage parity fixtures, SQLite/PGlite/TiDB/FoundationDB Ozone bounded-listing assertions, FoundationDB and TiDB Ozone Node/N-API bounded assertions, exact-limit regression, SDK debug redaction, dedicated TiDB/FoundationDB Ozone IOPS harnesses, strict provider-configuration qualification and expanded credential-free production-config policy are locally green, syntax-checked or compile-checked. Latest published head `80344aa` has CI run `35626681581` cancelled, Fault injection `35626681773` in progress and W08 release targets `35626681753` in progress; W08 release policy `35626681600` is cancelled and no Live Cloudflare R2 or Live AWS S3 run was recorded for this tip. None is W26 acceptance evidence. Focused IOPS diff scans `5fc5a943-07bb-4979-9f37-efd87a7f505e` and `60269206-bb22-4b78-aaf7-f05d16ffcca0`, production-config diff scan `d74e3e86-2e0a-45cf-9819-e31f428eb5d4` and negative-path scan `7addeeb5-4601-4951-aca9-becffb9bd4b9` found zero reportable findings with hosted/provider coverage explicitly deferred; `MOUNTX_SOURCE` parity and hosted/customer security controls remain bounded |
...
| Current-head CI attempt | Published tip `80344aa` has CI run `35626681581` cancelled, Fault injection run `35626681773` in progress, W08 release targets run `35626681753` in progress and W08 release policy run `35626681600` cancelled; no Live Cloudflare R2 or Live AWS S3 run was recorded for this tip. Only terminal W26 jobs with strict provider markers and retained artifacts can be promoted; the retained W26 packet above remains the last accepted hosted result. |
...
| Production rollout track | Open, currently **NO-GO**; 0 of 15 production gates are terminally accepted. The 37% figure reflects scope decisions, local hardening, provider-boundary implementation, strict provider-configuration qualification, expanded credential-free security policy and an acceptance baseline, not deployable readiness |
...
| Release/acceptance decision | W26 qualification accepted within the documented local/hosted provider scope on terminal run `35585066458`; latest completed standard security scan `5ad61e60-20e3-4223-885a-d4b516d49bb1`, focused production-config diff scan `d74e3e86-2e0a-45cf-9819-e31f428eb5d4`, negative-path scan `7addeeb5-4601-4951-aca9-becffb9bd4b9` and strict IOPS diff scan `60269206-bb22-4b78-aaf7-f05d16ffcca0` report zero reportable findings, while hosted/provider follow-up remains deferred; production rollout remains **NO-GO** and no broader native release claim is made |
...
| P2 — production metadata-provider support matrix | Hosted/provider CI | Ozone provider matrix and durable-provider hard-threshold paths expanded; terminal all-provider evidence pending | 60% | The benchmark has explicit SQLite/R2, PGlite/R2, TiDB/R2 and FoundationDB/R2 rows. The generic Ozone composition now requests its configured SQLite/R2 and PGlite/R2 rows with `--require-configured`; dedicated TiDB/FoundationDB paths are also strict, so a requested but unavailable provider fails the qualification result instead of being promoted as a pass. The key-value contract can enforce a bound through `get_keys_bounded`, and unstorage can opt in through `getKeysBounded`; capable and legacy/absent callback fixtures pass locally. SQLite/PGlite Ozone composition plus the TiDB/RustFS and FoundationDB/RustFS composition tests exercise bounded success and overflow against provider-backed `ChunkedFs`; the feature-built FoundationDB and TiDB Node/N-API Ozone lanes exercise the same contract. Published code chunk `de9d267` at `414a469` adds dedicated TiDB/FoundationDB Ozone IOPS invocations, positive-setting validation and retained JSON/pass-marker paths, while `d46e091` at `74fe4c7` makes requested-provider skips fail closed. No terminal hosted provider-parity or performance result exists. | Retain terminal Ozone results for each configured provider on one revision, including `ozone-tidb` and `ozone-foundationdb` artifacts/markers; record provider versions/HA/failure semantics and unsupported combinations, add capable KV/provider rows, and do not promote an explicitly skipped provider | 3–8 d | CI capacity, provider images/versions, provider-side listing APIs, managed-service access if required and provider-specific operator limits |
...
| P8 — load, capacity, soak and cost envelope | Hosted/provider CI | Per-provider hard-threshold gates implemented; terminal hosted results pending | 40% | The benchmark records successful write+read+delete lifecycle IOPS, supports a hard `--min-iops` threshold, strict `--require-configured` provider qualification and redacted JSON. The generic `scripts/test-ozone.sh` lane requests SQLite/R2 and PGlite/R2 only, with strict mode; dedicated TiDB and FoundationDB lanes request their own rows with the same strict mode. A missing requested provider now fails the result with machine-readable configuration failures rather than being silently promoted as a skip. Published code chunk `de9d267` at `414a469` extends the dedicated durable TiDB and FoundationDB Ozone jobs to run the same 4 KiB, 400-iteration, concurrency-64, `--min-iops 1000` benchmark, validate settings before provider setup, retain `artifacts/ozone-tidb-iops.json` and `artifacts/ozone-foundationdb-iops.json`, and emit provider-specific pass markers only after successful JSON-producing runs. Benchmark unit tests, missing-provider regression, shell syntax, YAML parsing and rebuilt N-API chunked lifecycle checks pass; no live local IOPS result is claimed. | Run terminal hosted `ozone-compositions`, `ozone-tidb` and `ozone-foundationdb` jobs; retain `artifacts/ozone-iops.json`, `artifacts/ozone-tidb-iops.json` and `artifacts/ozone-foundationdb-iops.json`; verify strict no-skip behavior, review p95/p99 latency, errors, CPU/memory and topology, then add soak/capacity variants before treating the target as qualified | 3–8 d after harness implementation | Stable hosted CI runners, Ozone fixture startup, provider quotas/topologies and enough runtime for meaningful soak; CI does not prove customer capacity or 99.99%/RPO/RTO |
...
| --- | --- | --- |
| Hosted result inspection | 0.5–1.5 d engineering plus external queue time | The historical W26 packet is green on `35585066458`; the latest published tip `80344aa` has CI `35626681581` cancelled, Fault injection `35626681773` in progress, W08 release targets `35626681753` in progress and W08 release policy `35626681600` cancelled, with no Live Cloudflare R2 or Live AWS S3 run. Dedicated TiDB/FoundationDB IOPS artifacts, strict no-skip evidence and the production-config policy output still require a terminal retained W26 run. |
...
tip CI `35625939459` is pending, Fault injection `35625939844` is pending, W08
release targets `35625939440` and policy `35625939441` are pending, and Live
Cloudflare R2 `35625939496` is in progress; none is W26 acceptance evidence.
...
and the latest concurrent reconciliation at `80344aa`, which is the current
`origin/main` head at this ledger snapshot. Its CI run `35626681581` is
cancelled, W08 release targets `35626681753` are in progress, W08 release
policy `35626681600` is cancelled and Fault injection `35626681773` is in
progress; these states are not W26 acceptance evidence.
