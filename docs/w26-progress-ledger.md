...
| Repository | `mount-rs` |
| Snapshot base | `07e5255a86d82805864a0dea36fbd13cb63b7944` (`origin/main`, including the published R2 content-addressing/cache, metadata mutation batching, queue-cancellation hardening and corrected Ozone content-addressed block-contract test; the exact W26 commits and concurrent-mainline merges are recorded below) |
...
| Current acceptance state | The retained W26 packet on `9c098e5` remains the last accepted hosted result within its documented provider/platform boundaries. The published W26 code now includes optimistic read/write paths, atomic whole-file `FsDriver::write_file`, lazy atime/EOF handling, content-addressed R2 blocks with a bounded process-local cache, fenced metadata mutation batching, a bounded pending queue, cancellation-safe runner cleanup and a corrected real-Ozone block-contract assertion for content-addressed IDs. Local full locked-workspace tests and strict Clippy pass; the focused ChunkedFs suite has 18 passing tests including canceled-runner recovery and one-publication concurrent mutation coverage. The last terminal W26 producer packet, run `35655276021` on `4efec58c93a0cd6e6027676f069edf0c4cde3132`, completed all four provider lifecycle matrices with zero timeouts/cleanup failures but missed the hard 1,000-IOPS target; its producer jobs failed and the aggregate failed closed. The queue-hardening scan `6d5a7665-d8cd-46b7-a9bd-73fec54f5431` and Ozone test-contract scan `46d4cf32-9d57-4f7e-975f-2616ddc53dc5` completed with zero reportable findings; hosted/provider/customer controls remain explicit external boundaries. No Live Cloudflare R2 or Live AWS S3 result is recorded. |
| Latest hosted workflow | Fresh manual run `35663517544` on exact tested revision `07e5255a86d82805864a0dea36fbd13cb63b7944` is currently queued. W26 producer jobs are `ozone-compositions` `106543976622`, `ozone-foundationdb` `106543976753`, `ozone` `106543976811` and `ozone-tidb` `106543976840`; the aggregate job is not yet present. This run includes the corrected content-addressed Ozone test plus all published mutation batching/cache/queue hardening and is the required current-tip qualification. |
...
| W26.3a — SQLite and disk-backed PGlite composition over real Ozone | Functional composition complete; latest terminal packet failed hard IOPS; corrected batching/cache rerun queued | 100% functional; 88% production qualification | Terminal packet `35655276021` completed 1,200/1,200 lifecycle operations with zero timeout/cleanup failures but measured SQLite/R2 20.35 and PGlite/R2 205.48 IOPS, both below 1,000; the producer and aggregate failed closed. Published R2 content-addressing/cache, metadata batching/queue hardening and corrected Ozone content-addressed-ID contract are included in current revision `07e5255a`; manual rerun `35663517544` is queued on that exact SHA. | Inspect the retained current-tip composition artifacts and close only on strict provider pass markers plus aggregate end-to-end success; otherwise continue correctness-preserving publication/performance work. | ~1–4 d follow-up performance/design; 0.5–1.5 d hosted review | Current Ozone fixture capacity/topology and customer target capacity remain external, but the remaining publication/provider latency cost is still a W26 gate. |
...
| W26.3c — independent durable three-node FoundationDB composition over Ozone | Durable provider lifecycle and restart markers passed; latest terminal packet failed hard IOPS; corrected batching/cache rerun queued | 90% base; 84% extension | The arm64 durable FoundationDB proof and provider-backed bounded-listing contract remain green. Terminal hosted job `106517158383` in run `35655276021` completed all 1,200 lifecycle operations and measured 40.50 IOPS, with zero timeout/cleanup failures but `IOPS_TARGET_NOT_MET`; the artifact is diagnostic only. Current revision `07e5255a` adds mutation batching, cancellation safety and the corrected block-contract assertion; manual rerun `35663517544` job `106543976753` is queued. Local FoundationDB binary linking remains blocked by missing `libfdb_c`. | Inspect the retained rerun while preserving durable restart, fencing, bounded-listing and cleanup assertions; do not promote until the provider and aggregate markers pass. | ~1–4 d shared concurrency follow-up; 0.75–1.5 h hosted review | FoundationDB client image/runtime, Ozone capacity/topology and customer secure durability remain external; local provider-native linking remains blocked. |
| W26.3d — durable multi-node TiDB composition over Ozone | Durable TiDB lifecycle and N-API bounded markers passed; latest terminal packet failed hard IOPS; corrected batching/cache rerun queued | 100% base; 84% extension | Terminal hosted job `106517158249` in run `35655276021` completed all 1,200 lifecycle operations and measured 18.09 IOPS, with zero timeout/cleanup failures but `IOPS_TARGET_NOT_MET`; the artifact is diagnostic only. Current revision `07e5255a` adds mutation batching, cancellation safety and the corrected block-contract assertion; manual rerun `35663517544` job `106543976840` is queued. The atomic path retains fencing/CAS and serialized conflict fallback, with local full-workspace tests and Clippy green. | Inspect the retained rerun while preserving restart/ambiguous-commit/fencing/bounded-listing/cleanup assertions; do not promote until the provider and aggregate markers pass. | ~1–4 d shared concurrency follow-up; 0.75–1.5 h hosted review | GitHub-hosted TiDB/PD/TiKV, N-API runtime, Ozone capacity/topology and customer secure durability remain external. |
...
`07e5255a86d82805864a0dea36fbd13cb63b7944`, and this checkout matches it.
...
commit `d152fa1a` and the Ozone test-contract fix `edb6a43e` are included through
the current concurrent-mainline tip. The 18-test focused ChunkedFs suite,
...
`46d4cf32-9d57-4f7e-975f-2616ddc53dc5` are complete with zero reportable
findings. The last terminal W26 producer/aggregate packet is `35655276021` on
`4efec58c` and failed the strict IOPS gate; corrected manual run
`35663517544` is queued on `07e5255a`. No queued, in-progress, canceled or
failed job is production acceptance evidence.
