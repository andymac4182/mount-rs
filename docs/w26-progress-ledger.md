...
| Repository | `mount-rs` |
| Snapshot base | `fd670b1` (latest published origin/main head carrying the credential-free Ozone production-config policy gate after concurrent mainline changes; current hosted workflows are pending or queued and are not evidence) |
| Checklist completion | 4 of 4 W26 tracker rows checked: 100% |
| Provisional execution completion | W26 qualification: 100%; production-rollout readiness: 34% (scope, benchmark matrix, lifecycle safety, bounded remote enumeration, provider-bounded KV/N-API contract, durable-provider bounded-listing tests, dedicated durable-provider hard-threshold IOPS wiring, credential-free all-provider production-config policy, current-revision security reviews and CI acceptance baseline captured; no terminal production gates yet). Customer deployment, native, provider-durability and release-stream gates remain separately bounded |
| Current acceptance state | The retained W26 packet on `9c098e5` remains accepted within its documented provider/platform boundaries. The newer provider-bounded KV/N-API implementation, unstorage parity fixtures, SQLite/PGlite/TiDB/FoundationDB Ozone bounded-listing assertions, FoundationDB and TiDB Ozone Node/N-API bounded assertions, exact-limit regression, SDK debug redaction, dedicated TiDB/FoundationDB Ozone IOPS harnesses and credential-free production-config policy gate are locally green, syntax-checked or compile-checked. Latest head `fd670b1` has CI run `35623237887` pending, Fault injection `35623237965` queued, W08 release targets `35623237899` pending and W08 release policy `35623237800` pending; no current-tip result is evidence. Focused IOPS diff scan `5fc5a943-07bb-4979-9f37-efd87a7f505e` and production-config diff scan `d74e3e86-2e0a-45cf-9819-e31f428eb5d4` found zero reportable findings with hosted/provider coverage explicitly deferred; `MOUNTX_SOURCE` parity and hosted/customer security controls remain bounded |
...
| Current-head CI attempt | Published tip `fd670b1` has CI run `35623237887` pending, Fault injection run `35623237965` queued, W08 release targets run `35623237899` pending and W08 release policy run `35623237800` pending; no terminal Live Cloudflare R2 or Live AWS S3 result was recorded for this tip. Pending or queued runs are not promoted to evidence; the retained W26 packet above remains the last accepted hosted result. |
...
| Production rollout track | Open, currently **NO-GO**; 0 of 15 production gates are terminally accepted. The 34% figure reflects scope decisions, local hardening, provider-boundary implementation, credential-free security policy and an acceptance baseline, not deployable readiness |
...
| Release/acceptance decision | W26 qualification accepted within the documented local/hosted provider scope on terminal run `35585066458`; latest completed standard security scan `5ad61e60-20e3-4223-885a-d4b516d49bb1` and focused production-config diff scan `d74e3e86-2e0a-45cf-9819-e31f428eb5d4` report zero reportable findings, while IOPS diff scan `5fc5a943-07bb-4979-9f37-efd87a7f505e` also reports zero with hosted/provider follow-up deferred; production rollout remains **NO-GO** and no broader native release claim is made |
...
| W26.4b — CI wiring, pinned images, cleanup, and documentation | HTTP, durable-provider IOPS and credential-free production-config policy gates implemented; hosted result pending | 100% implementation; 90% hosted packet | CI installs NAPI/PGlite with locked Rust dependencies, runs the Ozone composition matrix, Node/CLI coverage, the shipped HTTP server/client reopen path, and durable Ozone/TiDB/FoundationDB jobs. The HTTP runner restricts its prefix below the unique Ozone run scope and verifies deletion before returning. FoundationDB harness uses platform-specific pinned official 7.4.7 image manifests and transaction readiness probes; the dedicated `ozone-foundationdb` job is wired for the Ozone-backed durable mode. The published `de9d267` code chunk adds hard-threshold TiDB/FoundationDB IOPS invocation, fixed artifact paths and always-upload steps for the dedicated jobs. The published `b8d8fb1` policy chunk adds the credential-free all-provider configuration step and an insecure negative fixture. | Run and review `ozone-compositions`, `ozone-tidb` and `ozone-foundationdb` on a retained revision; record the HTTP pass, IOPS JSON/pass markers, production-config policy PASS/FAIL output and cleanup markers; keep W26 production readiness NO-GO until all open gates are terminal. | 0.75–1.5 h hosted review | GitHub workflow concurrency, nested provider startup, NAPI/PGlite builds and other workstream pushes can cancel runs; canceled/queued/failed jobs and missing artifacts are not evidence. |
| W26.7 — credential-free Ozone production configuration policy | Implementation complete; hosted/customer security gate open | 100% implementation; 55% production qualification | `scripts/verify-w26-ozone-production-config.mjs` rejects inline secret-like values, unknown provider fields, non-HTTPS R2 endpoints, unsafe paths/prefixes, non-durable metadata and non-TLS TiDB URLs. Positive SQLite, PGlite, TiDB and FoundationDB fixtures plus an insecure HTTP fixture are exercised in the `ozone` CI job without credentials or provider connections. Local positive/negative execution passed. Commit `b8d8fb1` was published in reconciled tip `4d8b5fd`; focused security diff scan `d74e3e86-2e0a-45cf-9819-e31f428eb5d4` found zero reportable findings with partial external/provider coverage. | Obtain terminal CI output on a retained revision; bind the policy to customer Ozone endpoint/IAM/certificate/secret-rotation evidence and confirm runtime deployment configuration matches the validated references. | 1.5–2.5 h implementation; 0.5–1.5 h review/handoff | Hosted CI execution, customer Ozone TLS/IAM, secret manager and rotation, host permissions, provider-native security and audit evidence remain external. |
...
| Qualification baseline | Complete for W26 only | Revision `9c098e5` and hosted run `35585066458` are the retained W26 acceptance packet; implementation tip `fd670b1` carries the policy/IOPS gates but has no terminal current-tip CI result | Any production implementation change gets a fresh qualification and production evidence packet on the tested revision |
...
| P2 — production metadata-provider support matrix | Hosted/provider CI | Ozone provider matrix and durable-provider hard-threshold paths expanded; terminal all-provider evidence pending | 58% | The benchmark has explicit SQLite/R2, PGlite/R2, TiDB/R2 and FoundationDB/R2 rows. SQLite is eligible from the Ozone composition job; PGlite, TiDB and FoundationDB are configuration/feature-gated and appear as explicit skips when their CI topology is absent. The key-value contract can enforce a bound through `get_keys_bounded`, and unstorage can opt in through `getKeysBounded`; capable and legacy/absent callback fixtures pass locally. SQLite/PGlite Ozone composition plus the TiDB/RustFS and FoundationDB/RustFS composition tests exercise bounded success and overflow against provider-backed `ChunkedFs`; the feature-built FoundationDB and TiDB Node/N-API Ozone lanes exercise the same contract. Published code chunk `de9d267` at `414a469` adds dedicated TiDB/FoundationDB Ozone IOPS invocations, positive-setting validation and retained JSON/pass-marker paths, but no terminal hosted provider-parity or performance result exists. | Retain terminal Ozone results for each configured provider on one revision, including `ozone-tidb` and `ozone-foundationdb` artifacts/markers; record provider versions/HA/failure semantics and unsupported combinations, add capable KV/provider rows, and do not promote an explicitly skipped provider | 3–8 d | CI capacity, provider images/versions, provider-side listing APIs, managed-service access if required and provider-specific operator limits |
| P3 — authentication, TLS, secret lifecycle and redaction | Implementation / hosted/provider CI | Local transport and credential-free production-config policy hardened; secure integration gate open | 58% | R2/Ozone config parsing and runtime validation reject non-HTTP(S), embedded credentials, query/fragment, missing-authority and remote plaintext-HTTP endpoints before client construction; HTTP config and runtime now reject non-loopback binds, require loopback behind a TLS reverse proxy, keep credentials as environment references and redact diagnostics. New `b8d8fb1` policy gate validates SQLite/PGlite/TiDB/FoundationDB metadata shapes, HTTPS R2 blocks, exact external secret references and TiDB TLS options without provider connections; local positive/negative policy checks passed | Add authenticated HTTPS endpoint CI where available, certificate identity/rotation checks, secret injection/rotation references, least privilege and clean-client negative tests; retain no secret values; review customer runtime configuration against the policy | 3–7 d | Secure Ozone CI endpoint or customer-supplied fixture, certificates/identity, secret manager integration, customer IAM and security review |
...
| P8 — load, capacity, soak and cost envelope | Hosted/provider CI | Per-provider hard-threshold gates implemented; terminal hosted results pending | 35% | The benchmark records successful write+read+delete lifecycle IOPS, supports a hard `--min-iops` threshold and redacted JSON. `scripts/test-ozone.sh` requests SQLite/R2, PGlite/R2, TiDB/R2 and FoundationDB/R2 rows; each configured provider must meet the 1,000 target, while absent prerequisites remain visible as skips. Published code chunk `de9d267` at `414a469` extends the dedicated durable TiDB and FoundationDB Ozone jobs to run the same 4 KiB, 400-iteration, concurrency-64, `--min-iops 1000` benchmark, validate settings before provider setup, retain `artifacts/ozone-tidb-iops.json` and `artifacts/ozone-foundationdb-iops.json`, and emit provider-specific pass markers only after successful JSON-producing runs. Benchmark unit tests, no-credential skip output, shell syntax, YAML parsing and rebuilt N-API chunked lifecycle checks pass; no live local IOPS result is claimed. The current policy chunk does not weaken this threshold. | Run terminal hosted `ozone-compositions`, `ozone-tidb` and `ozone-foundationdb` jobs; retain `artifacts/ozone-iops.json`, `artifacts/ozone-tidb-iops.json` and `artifacts/ozone-foundationdb-iops.json`; verify no provider row is silently skipped, review p95/p99 latency, errors, CPU/memory and topology, then add soak/capacity variants before treating the target as qualified | 3–8 d after harness implementation | Stable hosted CI runners, Ozone fixture startup, provider quotas/topologies and enough runtime for meaningful soak; CI does not prove customer capacity or 99.99%/RPO/RTO |
...
| --- | --- | --- |
| Hosted result inspection | 0.5–1.5 d engineering plus external queue time | The historical W26 packet is green on `35585066458`; the latest `fd670b1` attempt is pending/queued, so dedicated TiDB/FoundationDB IOPS artifacts and the production-config policy output still require a terminal retained run. |
...
the published revision was verified to match `origin/main`. The new policy
script and fixtures cover SQLite, PGlite, TiDB and FoundationDB metadata,
durable settings, HTTPS R2 blocks, scoped prefixes, exact external secret
references and TiDB TLS options without contacting a provider. Local positive
and negative executions, Node syntax, YAML parsing and diff checks passed. The
focused scan `d74e3e86-2e0a-45cf-9819-e31f428eb5d4` completed with zero
reportable findings; hosted/customer controls remain partial/deferred. Current
tip CI run `35622054417` was canceled before jobs started and Live Cloudflare
R2 run `35622054352` failed, so neither is evidence for the policy gate.

The documentation chunk `3741b29` was merged with concurrent mainline changes
and published at `fd670b1`, which is the current `origin/main` head. Its CI
run `35623237887` and W08 release runs `35623237899`/`35623237800` are pending,
while Fault injection `35623237965` is queued; these states are not evidence.
