...
| Repository | `mount-rs` |
| Snapshot base | `3fca802` (current `origin/main` after concurrent W08/W25 qualification changes and the SDK credential-redaction fix; W26 CI attempts at this tip are still non-terminal and are not evidence yet) |
| Checklist completion | 4 of 4 W26 tracker rows checked: 100% |
| Provisional execution completion | W26 qualification: 100%; production-rollout readiness: 29% (scope, benchmark matrix, lifecycle safety, bounded remote enumeration, provider-bounded KV/N-API contract, current-revision security review and CI acceptance baseline captured; no terminal production gates yet). Customer deployment, native, provider-durability and release-stream gates remain separately bounded |
| Current acceptance state | The retained W26 packet on `9c098e5` remains accepted within its documented provider/platform boundaries. The newer provider-bounded KV/N-API implementation, unstorage parity fixtures, Ozone composition bounded-listing assertions, exact-limit regression and current-tip SDK debug redaction are locally green. Current-revision hosted CI and `MOUNTX_SOURCE` provider-parity evidence are not terminal; the current standard security scan is complete but partial by design because hosted/customer controls remain deferred |
...
| Current-head CI attempt | Published tip `3fca802` has GitHub Actions CI run `35612976567` pending, Fault injection run `35612976712` queued, Live Cloudflare R2 run `35612976618` queued and Live AWS S3 run `35612976513` queued. No queued, in-progress, canceled or failed run is promoted to evidence; the retained W26 packet above remains the last accepted hosted result. |
...
| Release/acceptance decision | W26 qualification accepted within the documented local/hosted provider scope on terminal run `35585066458`; current-tip security review `70af8d4e-9fb3-4d2c-b688-54a9b6535739` reports zero local reportable findings with explicit deferred hosted/customer coverage; production rollout remains **NO-GO** and no broader native release claim is made |
...
| W26.6 — bounded remote directory enumeration and response materialization | Built-in and KV/N-API provider-boundary implementation complete; provider/native qualification open | 100% implementation; 70% production qualification | `FsDriver::readdir_bounded` fails closed with `ENOTSUP` by default. HTTP `/entries` and directory-file routes request the bound before serialization and map overflow to the existing 413/connection-close contract. Memory, host, chunked, versioned, persisted, observability, CLI and native N-API wrappers implement or forward the boundary. `KeyValueStore::get_keys_bounded` is an optional provider-side contract; the unstorage bridge exposes `getKeysBounded(prefix, maxKeys)` and `Filesystem.readdirBounded(path, maxEntries)`, while providers without the callback remain fail-closed. Terminal local evidence: KV 13 integration tests, N-API Rust 16 tests, HTTP 8 unit + 12 integration, core 11, host 5, chunked 14, CLI 44, observability 4 and persistence 4; release N-API packaging, unstorage bridge, typecheck, chunked smoke, strict affected-package Clippy, formatting and diff checks passed. The follow-up parity fixtures pass capable-provider overflow/success and absent/legacy-callback `ENOTSUP`; the SQLite/PGlite Ozone composition gate asserts bounded success and `EOVERFLOW` against provider-backed `ChunkedFs`, with locked compile and strict Clippy passing. The adapter now forwards exactly the caller's limit, with a 13-test regression asserting provider limits `[2, 3]`. `MOUNTX_SOURCE` parity suites were explicitly skipped because the provider source is unset. The current-tip standard security scan `70af8d4e-9fb3-4d2c-b688-54a9b6535739` is complete on `3fca802` with zero local reportable findings across 16 surfaces; its partial status still defers customer Ozone TLS/IAM/rotation, provider-native bounded allocation, dependency provenance and production SLO/recovery controls. | Review terminal Ozone composition results for both provider-backed assertions, exercise the contract in the remaining hosted provider jobs, add provider-native pagination where a backend can safely enforce it, retain `ENOTSUP` for legacy providers, run `MOUNTX_SOURCE` parity, and retain the explicit security follow-ups. | ~3.25–5.25 h implementation/local verification; ~1–3 d provider/security/hosted review | Provider-side key enumeration, JavaScript callback implementation, hosted provider topologies, secure Ozone fixture, mountx source and customer security controls remain external or cross-workstream gates. |
