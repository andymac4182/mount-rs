...
| Latest hosted workflow | Run `35677828440` selected exact SHA `918662349cd24b37e65433b835987b8234e89e87`; base Ozone job `106587969438` passed, composition job `106587969430` failed its PGlite/R2 hard IOPS row, TiDB job `106587969980` failed its hard IOPS row, and FoundationDB job `106587968903` failed before its benchmark because `tests/foundationdb/Cargo.lock` lacked `tokio`. Aggregate job `106590078024` was queued at this snapshot. This packet is diagnostic only and is not acceptance evidence. |
...
| Next hosted qualification | Dispatch after this ledger publication on the fully lockfile-verified `origin/main` tip (current merged tip `ca13a33f`, or the later documentation tip) | Run the complete Ozone/provider matrix on one exact SHA. Accept W26.15 only if every configured provider, FoundationDB preflight, the complete end-to-end packet and the aggregate emit terminal pass markers. The strict 1,000-IOPS target is unchanged. | GitHub-hosted runners, Ozone/provider startup, provider latency and artifact retention are external gates; queued, in-progress, canceled, failed or skipped jobs are not evidence |
...
| Release/acceptance decision | W26 implementation and local qualification controls remain accepted within their documented scope; production rollout remains **NO-GO** because the last complete terminal packet `35674425514` failed every hard 1,000-IOPS provider row, and newer run `35677828440` still failed PGlite/R2 and TiDB/R2 while FoundationDB was blocked by a pre-fix lockfile gate. The Ozone and FoundationDB lockfile corrections are published through merged tip `ca13a33f`; the next exact-SHA packet is required. Security scan `739f4f5b-8cc4-4154-93f7-9e486745eab1` is sealed with zero reportable findings and complete changed-file coverage. No failed, queued or pending packet is promoted, and no broader native, customer secure-runtime, Ozone backup/DR or release claim is made |
