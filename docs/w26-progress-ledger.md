...
| Repository | `mount-rs` |
| Snapshot base | `74fe4c7` (latest published origin/main head carrying the strict provider-configuration IOPS gate plus concurrent mainline changes; current hosted workflows are pending or in progress and are not evidence) |
...
| Current acceptance state | The retained W26 packet on `9c098e5` remains accepted within its documented provider/platform boundaries. The newer provider-bounded KV/N-API implementation, unstorage parity fixtures, SQLite/PGlite/TiDB/FoundationDB Ozone bounded-listing assertions, FoundationDB and TiDB Ozone Node/N-API bounded assertions, exact-limit regression, SDK debug redaction, dedicated TiDB/FoundationDB Ozone IOPS harnesses, strict provider-configuration qualification and expanded credential-free production-config policy are locally green, syntax-checked or compile-checked. Latest published head `74fe4c7` has CI run `35625939459` pending, Fault injection `35625939844` pending, W08 release targets `35625939440` pending, W08 release policy `35625939441` pending and Live Cloudflare R2 `35625939496` in progress; none is W26 acceptance evidence. Focused IOPS diff scans `5fc5a943-07bb-4979-9f37-efd87a7f505e` and `60269206-bb22-4b78-aaf7-f05d16ffcca0`, production-config diff scan `d74e3e86-2e0a-45cf-9819-e31f428eb5d4` and negative-path scan `7addeeb5-4601-4951-aca9-becffb9bd4b9` found zero reportable findings with hosted/provider coverage explicitly deferred; `MOUNTX_SOURCE` parity and hosted/customer security controls remain bounded |
...
| Current-head CI attempt | Published tip `74fe4c7` has CI run `35625939459` pending, Fault injection run `35625939844` pending, W08 release targets run `35625939440` pending, W08 release policy run `35625939441` pending and Live Cloudflare R2 run `35625939496` in progress; no Live AWS S3 run was recorded for this tip. Only terminal W26 jobs with strict provider markers and retained artifacts can be promoted; the retained W26 packet above remains the last accepted hosted result. |
...
| --- | --- | --- |
| Hosted result inspection | 0.5–1.5 d engineering plus external queue time | The historical W26 packet is green on `35585066458`; the latest published tip `74fe4c7` has CI `35625939459` pending, Fault injection `35625939844` pending, W08 release targets `35625939440` pending, W08 release policy `35625939441` pending and Live Cloudflare R2 `35625939496` in progress. Dedicated TiDB/FoundationDB IOPS artifacts, strict no-skip evidence and the production-config policy output still require a terminal retained W26 run. |
...
published at `3681f27`, which is the current `origin/main` head at this ledger
snapshot. Its CI run `35624846453` is pending, W08 release targets
`35624846413` are pending, W08 release policy `35624846541` is pending and
Fault injection `35624846354` is in progress; these states are not W26
acceptance evidence.
