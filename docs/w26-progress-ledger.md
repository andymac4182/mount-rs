...
| Next hosted qualification | Dispatch after publishing the SQL publication fast path on exact `origin/main` `8c8f678a` | Re-run the complete Ozone/provider matrix. Accept W26.15 only if every configured provider, FoundationDB preflight, the complete end-to-end packet and the aggregate emit terminal pass markers on one exact revision. The strict 1,000-IOPS target is unchanged. | GitHub-hosted runners, Ozone/provider startup, provider latency and artifact retention are external gates; queued, in-progress, canceled, failed or skipped jobs are not evidence |
...
| Release/acceptance decision | W26 implementation and local qualification controls remain accepted within their documented scope; production rollout remains **NO-GO** because terminal run `35678993571` failed every hard 1,000-IOPS provider row: SQLite/R2 802.37, PGlite/R2 614.90, TiDB/R2 86.53 and FoundationDB/R2 360.14. Aggregate `106595737360` failed closed. The SQL publication fast path `214b9a6b` is included in current merged tip `8c8f678a`, with full locked tests, strict Clippy and security scan `5a8fcd70-71d4-461b-bb2a-dec8461c22bc` green before the concurrent reconciliation; a fresh hosted packet is still required. No failed, queued or pending packet is promoted, and no broader native, customer secure-runtime, Ozone backup/DR or release claim is made |
...
- [ ] P8/W26.15: close the per-drive 1,000-IOPS CI workload with latency,
  ...
  `origin/main` `8c8f678a`). Retain the next terminal artifacts before
...
| --- | --- | --- |
| Hosted result inspection | 0.5–1.5 d engineering plus external queue time | Historical green packet `35585066458` remains the last accepted hosted result. Current terminal run `35678993571` is diagnostic: base Ozone passed, all four provider rows completed 1,200/1,200 operations with zero timeouts/cleanup failures but missed 1,000 IOPS, and aggregate job `106595737360` failed closed. The SQL publication fast path `214b9a6b` is included in current `origin/main` `8c8f678a`; the next exact-SHA producer/aggregate packet requires review. |
...
`8c8f678adaf81beda78f092d343ed8585152e500`, and this checkout is being
...
diagnostic packet. Current shared tip `8c8f678a` contains the
...
`origin/main` tip `8c8f678a`; no hosted IOPS result exists for this revision,
so production remains **NO-GO** pending a new exact-SHA packet.
