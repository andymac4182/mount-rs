...
| Repository | `mount-rs` |
| Snapshot base | `76650bd1ac580a668f22a727946702c08778ca9a` (`origin/main`, including the published R2 content-addressing/cache, metadata mutation batching, queue-cancellation hardening, concurrent whole-file-create inode rebasing, corrected Ozone content-addressed block-contract test and deduplicated content-addressed cleanup plus the latest concurrent-mainline reconciliation; the exact W26 commits and concurrent-mainline merges are recorded below) |
...
| Provisional execution completion | W26 implementation scope: 100% for the 11 shipped tracker rows; current terminal qualification packet: **FAILED / NO-GO** because W26.15's hard performance gate is open; production-rollout readiness: 54% (scope, benchmark matrix, lifecycle safety, bounded remote enumeration, provider-bounded KV/N-API contract, durable-provider bounded-listing tests, strict provider-specific hard-threshold IOPS wiring, fail-closed artifact/profile/metric verification and retention, all-provider credential-free production-config policy, one-revision CI evidence-packet aggregation, complete end-to-end surface marker enforcement, the credential-free customer rollout contract, content-addressed R2 caching, metadata mutation batching, concurrent-create inode rebasing and queue cancellation safety are implemented and locally tested; no terminal production gates yet). Customer deployment, native, provider-durability, capacity/SLO, backup/DR and release-stream gates remain separately bounded |
| Current acceptance state | The retained W26 packet on `9c098e5` remains the last accepted hosted result within its documented provider/platform boundaries. The published W26 code now includes optimistic read/write paths, atomic whole-file `FsDriver::write_file`, lazy atime/EOF handling, content-addressed R2 blocks with a bounded process-local cache, fenced metadata mutation batching, a bounded pending queue, cancellation-safe runner cleanup, same-revision rebasing for concurrent new-file creates, a corrected real-Ozone block-contract assertion for content-addressed IDs and cleanup that deletes identical content-addressed IDs only once. Local full locked-workspace tests and strict Clippy pass; the focused ChunkedFs suite has 19 passing tests including canceled-runner recovery, one-publication concurrent mutation coverage and concurrent-create inode rebasing. The last terminal W26 producer packet, run `35655276021` on `4efec58c93a0cd6e6027676f069edf0c4cde3132`, completed all four provider lifecycle matrices with zero timeouts/cleanup failures but missed the hard 1,000-IOPS target; its producer jobs failed and the aggregate failed closed. Security scans `6d5a7665-d8cd-46b7-a9bd-73fec54f5431`, `46d4cf32-9d57-4f7e-975f-2616ddc53dc5`, cleanup-fix scan `85c31e86-1fc8-4a6d-bb95-ef2d956b9ed8` and concurrent-create scan `b8f6e846-e497-4ae6-b972-7ae3701ec722` completed with zero reportable findings within their local scopes; hosted/provider/customer controls remain explicit external boundaries. No Live Cloudflare R2 or Live AWS S3 result is recorded. |
| Latest hosted workflow | Fresh manual run `35665734382` selected exact tested revision `8c53d2c926137fba6034bf0edcb662999c71d426`, a descendant containing the published concurrent-create fix. W26 producer jobs are `ozone-foundationdb` `106550958697` (in progress), `ozone-compositions` `106550958843` (queued), `ozone-tidb` `106550958877` (completed failure) and `ozone` `106550958998` (queued); the aggregate job is not yet present. The TiDB failure is diagnostic until its retained logs/artifact are inspected; no job is acceptance evidence until every producer and the aggregate are terminally successful on this one revision. |
...
| Next hosted qualification | Manual run `35665734382` against exact SHA `8c53d2c926137fba6034bf0edcb662999c71d426` | Await FoundationDB, compositions and base-Ozone terminal output, inspect the TiDB failure logs/artifact, and determine whether the packet failed on performance, lifecycle, provider startup or another W26-owned contract. Accept W26.15 only if every configured provider and the complete end-to-end packet emit terminal pass markers on this one revision; otherwise continue correctness-preserving performance work. The strict 1,000-IOPS target is unchanged. | GitHub-hosted runners, Ozone/provider startup, provider latency and artifact retention are external gates; queued, in-progress, canceled, failed or skipped jobs are not evidence |
...
| Production rollout track | Open, currently **NO-GO**; 0 of 15 production gates are terminally accepted. The 52% figure reflects scope decisions, local hardening, provider-boundary implementation, strict provider-configuration and artifact-integrity/metric/retention qualification, expanded credential-free security policy, one-revision evidence-packet aggregation, complete end-to-end packet surface enforcement, the credential-free customer rollout contract, content-addressed block caching, metadata batching and cancellation safety, not deployable readiness |
...
| Release/acceptance decision | W26 implementation and local qualification controls remain accepted within their documented scope; production rollout remains **NO-GO** because terminal run `35655276021` failed every hard 1,000-IOPS provider row and aggregate job `106522172018` failed closed. The concurrent-create fix is published through current `origin/main` `76650bd1` and security-scanned with zero reportable findings, but fresh hosted run `35665734382` on exact SHA `8c53d2c9` has a completed TiDB failure, one producer in progress, two queued and no aggregate job. Earlier standard, policy, negative-path, strict-IOPS, evidence-packet and rollout-contract scans remain recorded above; no failed, queued or in-progress run is promoted, and no broader native, customer secure-runtime, Ozone backup/DR or release claim is made |
...
The W26-owned performance/safety chunks are published on current
`origin/main` `76650bd1`. The R2 block path uses content-addressed object IDs and a
...
Manual hosted qualification run `35661836627` is queued on this exact
...
fix is committed as `edb6a43e` in current `origin/main` `07e5255a`. Fresh
...
| W26.3a — SQLite and disk-backed PGlite composition over real Ozone | Functional composition complete; latest terminal packet failed hard IOPS; current-create-rebase qualification in progress | 100% functional; 90% production qualification | Terminal packet `35655276021` completed 1,200/1,200 lifecycle operations with zero timeout/cleanup failures but measured SQLite/R2 20.35 and PGlite/R2 205.48 IOPS, both below 1,000; the producer and aggregate failed closed. Published R2 content-addressing/cache, metadata batching/queue hardening, same-revision concurrent-create inode rebasing, corrected Ozone content-addressed-ID contract and deduplicated cleanup are included in current `origin/main` `76650bd1`; manual run `35665734382` targets exact SHA `8c53d2c9` with composition job `106550958843` queued. | Inspect the retained current-tip composition artifacts and close only on strict provider pass markers plus aggregate end-to-end success; otherwise continue correctness-preserving publication/performance work. | ~1–4 d follow-up performance/design; 0.5–1.5 d hosted review | Current Ozone fixture capacity/topology and customer target capacity remain external, but the remaining publication/provider latency cost is still a W26 gate. |
...
| W26.3c — independent durable three-node FoundationDB composition over Ozone | Durable provider lifecycle and restart markers passed; latest terminal packet failed hard IOPS; current-create-rebase qualification in progress | 90% base; 86% extension | The arm64 durable FoundationDB proof and provider-backed bounded-listing contract remain green. Terminal hosted job `106517158383` in run `35655276021` completed all 1,200 lifecycle operations and measured 40.50 IOPS, with zero timeout/cleanup failures but `IOPS_TARGET_NOT_MET`; the artifact is diagnostic only. Current `origin/main` `76650bd1` includes mutation batching, cancellation safety, same-revision concurrent-create rebasing, the corrected block-contract assertion and deduplicated content-addressed cleanup; manual run `35665734382` job `106550958697` is in progress. Local FoundationDB binary linking remains blocked by missing `libfdb_c`. | Inspect the retained rerun while preserving durable restart, fencing, bounded-listing and cleanup assertions; do not promote until the provider and aggregate markers pass. | ~1–4 d shared concurrency follow-up; 0.75–1.5 h hosted review | FoundationDB client image/runtime, Ozone capacity/topology and customer secure durability remain external; local provider-native linking remains blocked. |
| W26.3d — durable multi-node TiDB composition over Ozone | Durable TiDB lifecycle and N-API bounded markers passed; latest terminal packet failed hard IOPS; current-create-rebase qualification in progress | 100% base; 86% extension | Terminal hosted job `106517158249` in run `35655276021` completed all 1,200 lifecycle operations and measured 18.09 IOPS, with zero timeout/cleanup failures but `IOPS_TARGET_NOT_MET`; the artifact is diagnostic only. Current `origin/main` `76650bd1` includes mutation batching, cancellation safety, same-revision concurrent-create rebasing, the corrected block-contract assertion and deduplicated content-addressed cleanup; manual run `35665734382` job `106550958877` has completed with failure and its artifact/logs require inspection. The atomic path retains fencing/CAS and serialized conflict fallback, with local full-workspace tests and Clippy green. | Inspect the retained rerun while preserving restart/ambiguous-commit/fencing/bounded-listing/cleanup assertions; do not promote until the provider and aggregate markers pass. | ~1–4 d shared concurrency follow-up; 0.75–1.5 h hosted review | GitHub-hosted TiDB/PD/TiKV, N-API runtime, Ozone capacity/topology and customer secure durability remain external. |
...
| W26.4b — CI wiring, pinned images, cleanup, and documentation | Implementation complete; latest terminal packet failed the hard provider gate; current-create-rebase qualification in progress | 100% implementation; 67% hosted packet qualification | CI installs NAPI/PGlite with locked Rust dependencies, runs the Ozone composition matrix, Node/CLI coverage, the shipped HTTP server/client reopen path, and durable Ozone/TiDB/FoundationDB jobs. Terminal run `35655276021` retained the base/provider artifacts; base Ozone passed, every configured provider missed 1,000 IOPS and aggregate `106522172018` failed closed. Its FoundationDB retry also exposed the stale content-addressed-ID assertion before the benchmark; cleanup then required deduplicating the now-equal IDs. Manual run `35665734382` targets exact SHA `8c53d2c9` with W26 producers `106550958998` (`ozone`, queued), `106550958843` (`ozone-compositions`, queued), `106550958877` (`ozone-tidb`, failed) and `106550958697` (`ozone-foundationdb`, in progress); the aggregate job is not yet present. | Retain exact current-tip artifacts, policy markers, end-to-end markers, performance markers and cleanup; promote only after all producer jobs and `w26-ozone-evidence` are terminally successful on one revision. | ~1–4 d performance follow-up; 0.75–1.5 d hosted review per retained run | Workflow concurrency, hosted provider startup, Ozone capacity/topology, artifact service and customer TLS/IAM remain external; no failed/queued/skipped artifact is acceptance evidence. |
| W26.14 — complete end-to-end Ozone evidence-surface gate | Implementation complete; latest terminal packet failed performance; current-create-rebase packet in progress | 100% implementation; 48% production qualification | Expanded `scripts/verify-w26-ozone-evidence-packet.mjs` to require gateway health/ready/restart, SQLite/PGlite composition and bounded listing, Rust CLI, Node provider matrix/CLI, remote HTTP CLI, TiDB Rust/N-API seed/reopen and FoundationDB Rust/N-API seed/reopen markers. Local benchmark/evidence tests, Node/shell/YAML checks, diff checks, full locked workspace tests and strict Clippy pass. Terminal run `35655276021` retained complete provider logs/artifacts, but all four hard IOPS rows failed and aggregate `106522172018` failed closed; its FoundationDB lane additionally found a stale test assertion before benchmarking and then exposed duplicate cleanup of an identical content-addressed ID. Cleanup was corrected and security-scanned with zero findings. Current `origin/main` `76650bd1` adds the locally tested concurrent-create inode-rebase fix; manual run `35665734382` targets `8c53d2c9`, has a failed TiDB producer, one producer in progress, two queued and no aggregate result yet. No packet result is promoted. | Close the packet only after a post-remediation run has terminal pass evidence from every producer and aggregate; retain native/mount and customer secure-runtime evidence separately. | ~1–4 d W26 performance follow-up; ~0.75–1.5 d hosted review; ~0.75–1.25 h security review | Ozone fixture capacity/topology, hosted provider/N-API execution, artifact authorization, native mount runners, customer TLS/IAM/rotation and measured SLO/RPO/RTO remain external. |
| W26.15 — per-drive 1,000-IOPS qualification and safe performance remediation | Metadata batching, content-addressed cache, queue-cancellation safety, concurrent-create inode rebasing and Ozone test-contract/cleanup corrections implemented/published; hard target still open; **NO-GO** | 84% W26-owned implementation / 35% hosted qualification | Published W26 performance work includes atomic whole-file write `96a25f17`, lazy atime/EOF `d1bc8fb9`, R2 content addressing/cache, mutation batching `c4e9a253` (published through `a1cb4ca9`), queue hardening `d152fa1a` and concurrent whole-file-create rebasing `d3d6299e` (published through `059f801e` and included in current origin descendants). The Ozone integration test asserts identical content reuses its ID and different content does not (`edb6a43e`); cleanup now deletes the repeated content-addressed ID once (`0282df64`, published through current origin). The batcher publishes eligible whole-file/unlink mutations once under the lease, bounds pending requests at 1,024, drains canceled-runner requests with `EIO` and skips closed replies before applying a queued mutation. New-file mutations from one batch rebase to unique inodes at the current revision while stale existing-file mutations still conflict; regression coverage proves four concurrent creates share one metadata publication and have unique inodes. Focused ChunkedFs tests pass 19/19; Ozone test compile-check, full locked workspace tests, strict Clippy and diff checks pass; concurrent-create security scan `b8f6e846-e497-4ae6-b972-7ae3701ec722` completed with zero reportable findings within its local scope. Terminal run `35655276021` remains diagnostic at SQLite/R2 20.35, PGlite/R2 205.48, TiDB/R2 18.09 and FoundationDB/R2 40.50 IOPS; every row completed 1,200/1,200 operations with zero timeout/cleanup failures but failed `IOPS_TARGET_NOT_MET`, and aggregate `106522172018` failed closed. Manual run `35665734382` targets exact revision `8c53d2c9` with all four producer jobs listed in the snapshot; the aggregate is not yet present. | Inspect current-tip provider metrics, artifacts and aggregate markers. If the hard target remains open, continue correctness-preserving provider/publication work or obtain an Ozone/customer capacity qualification; do not lower the target or convert failed rows to skips. | ~1.5–4 d implementation/design; ~0.75–1.5 d hosted review per rerun; external queue time excluded | GitHub runner/provider startup, Ozone fixture capacity/topology, provider-native performance, artifact retention, customer secure topology and capacity are external gates; no staging environment exists. |
...
| P8 — load, capacity, soak and cost envelope | Hosted/provider CI plus W26.15 remediation | Hard-threshold and one-revision packet gates implemented; current packet is non-terminal and production remains NO-GO | 56% | The benchmark records successful write+read+delete lifecycle IOPS, supports a hard `--min-iops` threshold, strict `--require-configured` provider qualification and redacted JSON. The fixed 4 KiB/400-iteration/concurrency-64 profile is enforced. Terminal run `35655276021` completed all 1,200 lifecycle operations per row with zero timeouts/cleanup failures but measured only 20.35/205.48/18.09/40.50 IOPS; aggregate job `106522172018` failed closed. Published `96a25f17` optimized whole-file publication, `d1bc8fb9` avoids per-read/EOF metadata publication, later chunks add R2 content addressing/cache, mutation batching, queue cancellation safety, concurrent-create inode rebasing and `0282df64` duplicate-cleanup correction. Fresh manual run `35665734382` targets exact SHA `8c53d2c9`; TiDB has completed with failure, FoundationDB is in progress, compositions and base Ozone are queued, and no aggregate is present. | Inspect the current-tip all-provider packet, retrieve and diagnose the failed TiDB artifact, then retain p95/p99 latency, errors, CPU/memory and topology; add soak/capacity variants only after every hard provider row closes | 1.5–4 d W26 performance follow-up; 1–2 d hosted review; soak effort provisional | Ozone fixture capacity/topology, stable hosted runners, artifact service, provider quotas and customer 99.99% capacity remain external; do not lower the target |
...
| P14 — final W26 integration-readiness review | W26 implementation / hosted CI / handoff | NO-GO review documented; cannot close while W26.15 is open | 35% | Terminal packet `35655276021` is audited: base Ozone passed, all four provider lifecycles completed but missed the hard IOPS target, aggregate `106522172018` failed closed. Concurrent-create rebasing, cleanup correction `0282df64`, local compile/tests/Clippy and focused security scans are published through current `origin/main` `76650bd1`; manual run `35665734382` targets exact SHA `8c53d2c9` with a failed TiDB producer, one in progress, two queued and no aggregate yet. | Inspect the current one-revision packet and failed TiDB artifact, close W26.15 only with terminal pass markers for every provider and the full end-to-end surface, attach provider/platform/security/performance evidence, list customer/Ozone dependencies and hand off an explicit integration-ready or NO-GO decision to release/deployment streams | 1–2 d review after performance gate; 1.5–4 d W26.15 follow-up | All W26-owned CI gates plus external customer Ozone secure topology, capacity/SLO, DR and release-stream confirmations |
...
| --- | --- | --- |
| Hosted result inspection | 0.5–1.5 d engineering plus external queue time | Historical green packet `35585066458` remains the last accepted hosted result. Current terminal run `35655276021` is diagnostic: base Ozone passed, all four provider rows completed 1,200/1,200 operations with zero timeouts/cleanup failures but missed 1,000 IOPS, and aggregate job `106522172018` failed closed. Concurrent-create rebasing `d3d6299e` is published through current origin descendants; fresh run `35665734382` targets exact SHA `8c53d2c9` and requires terminal producer/aggregate review. |
...
| Performance and end-to-end matrix (P8, P11, P14, W26.15) | 12–32 d engineering | The workload and packet are implemented, but terminal diagnostic run `35655276021` failed the hard provider target at SQLite/PGlite/TiDB/FoundationDB 20.35/205.48/18.09/40.50 IOPS. The next design must reduce or safely defer more metadata publication while preserving POSIX/fencing/recovery semantics, then requires the queued cleanup-corrected packet, possible provider-latency work or production-like Ozone capacity qualification, stable hosted CI runners, native runners and a one-revision audit. |
...
`76650bd1ac580a668f22a727946702c08778ca9a`, and this checkout matches it.
...
descendants. The 19-test focused ChunkedFs suite,
...
`85c31e86-1fc8-4a6d-bb95-ef2d956b9ed8` and `b8f6e846-e497-4ae6-b972-7ae3701ec722`
are complete with zero reportable findings within their local scopes. The last
terminal W26 producer/aggregate packet is `35655276021` on `4efec58c` and failed
the strict IOPS gate; fresh manual run `35665734382` selected `8c53d2c9` and is
not terminal. No queued, in-progress,
...
and fresh run `35665734382` is pending on `8c53d2c9` with a failed TiDB
producer and remaining jobs non-terminal. No queued, in-progress,
...
`76650bd1` contains the lazy-atime/EOF, batching/cache, queue-safety,
concurrent-create, corrected content-addressed contract and deduplicated-cleanup
