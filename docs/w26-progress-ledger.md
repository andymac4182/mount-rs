...
| Repository | `mount-rs` |
| Snapshot base | `4d8b5fd` (published credential-free Ozone production-config policy gate after the durable-provider IOPS qualification baseline and concurrent mainline changes; current hosted workflows are canceled or failed and are not evidence) |
...
| Current-head CI attempt | Published tip `4d8b5fd` has CI run `35622054417` canceled before jobs started, Fault injection run `35622054305` canceled, W08 release targets run `35622054350` canceled, W08 release policy run `35622054310` canceled and Live Cloudflare R2 run `35622054352` failed; no Live AWS S3 run was recorded for this tip. No canceled or failed run is promoted to evidence; the retained W26 packet above remains the last accepted hosted result. |
...
| Qualification baseline | Complete for W26 only | Revision `9c098e5` and hosted run `35585066458` are the retained W26 acceptance packet; implementation tip `4d8b5fd` adds policy/IOPS gates but has no terminal current-tip CI result | Any production implementation change gets a fresh qualification and production evidence packet on the tested revision |
...
| --- | --- | --- |
| Hosted result inspection | 0.5–1.5 d engineering plus external queue time | The historical W26 packet is green on `35585066458`; the new `4d8b5fd` attempt was canceled before jobs started, so dedicated TiDB/FoundationDB IOPS artifacts and the production-config policy output still require a terminal retained run. |
