...
| Repository | `mount-rs` |
| Snapshot base | `0842474` (current `origin/main` after the FoundationDB Ozone Node/N-API bounded-listing qualification and concurrent mainline changes; current hosted workflows are pending or failed and are not evidence) |
...
| Current acceptance state | The retained W26 packet on `9c098e5` remains accepted within its documented provider/platform boundaries. The newer provider-bounded KV/N-API implementation, unstorage parity fixtures, SQLite/PGlite/TiDB/FoundationDB Ozone bounded-listing assertions, FoundationDB Ozone Node/N-API bounded assertions, exact-limit regression and SDK debug redaction are locally green, syntax-checked or compile-checked. Revision `0842474` hosted CI is pending and Live Cloudflare R2 has already failed, so no current-tip result is evidence; `MOUNTX_SOURCE` parity and the current standard security scan's hosted/customer controls remain explicitly bounded |
...
| Current-head CI attempt | Published tip `0842474` has CI run `35617837145` pending, Fault injection run `35617836990` pending, W08 release targets run `35617837043` pending, W08 release policy run `35617836987` pending and Live Cloudflare R2 run `35617837005` failed; no Live AWS S3 run was recorded for this tip. No pending or failed run is promoted to evidence; the retained W26 packet above remains the last accepted hosted result. |
