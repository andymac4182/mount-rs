...
## Current authority override — 2026-09-22, TiDB publication chunk

This is the newest implementation and production-readiness boundary. The
TiDB code is published, but the next hosted packet has not yet been run on
that exact code. The prior terminal packet remains the latest hosted evidence
and is retained below; it is diagnostic because the hard per-drive target was
not met by all providers.

| Field | Current value |
| --- | --- |
| Shared build-on tip | `origin/main` = `4098c7df04a03f65269ef932cb921b96d6368297`, verified with `git ls-remote`; the worktree is clean. A documentation/tracker publication is the next push before dispatching the hosted packet. |
| Current implementation chunk | `4098c7df` (`perf(w26): use tidb autocommit publication fast path`). Successful TiDB metadata publication now uses one parameterized autocommit conditional UPDATE acknowledgement; zero-row outcomes retain the explicit pessimistic locked classification transaction. Statement conflicts remain retryable, while lost/non-retryable acknowledgements fail closed as ambiguous `EIO`. Lease owner, fence, expiry and revision predicates are unchanged. |
| Local implementation evidence | `cargo fmt --all -- --check`, `git diff --check`, focused `mount-rs-tidb` tests (8 passed, 5 provider/service-gated ignored), full `./scripts/cargo-shared test --workspace --all-targets --locked`, focused strict TiDB Clippy and strict workspace Clippy with `-D warnings` all passed. No live TiDB service was available locally, so the five ignored rows remain an external provider gate. |
| Security evidence | Diff scan `c36f104e-965b-4e85-bfde-2d3d222f0de2` completed and sealed with complete changed-file coverage, 2 reviewed surfaces and zero reportable findings. Measured usage: `2,241,675` total tokens, `2,234,825` input, `2,141,952` cached input. Report: `/private/var/folders/qx/1pyrtldd3nb1l0p44xbmd97h0000gn/T/codex-security-scans-MW6oGi/mount-rs/7f5c27824eaf713dbae88d50ffa8a4564f8abff6_20260922T060338Z_v9roccul/report.md`. |
| Latest hosted evidence | Run `35691451007` selected earlier exact SHA `dccd8351690ba21b4ea01ab8680369c76c442041`, before `4098c7df`: base passed; compositions, TiDB, FoundationDB and aggregate failed. SQLite/R2 `213.947686`, PGlite/R2 `2,065.669446`, TiDB/R2 `333.356025`, FoundationDB/R2 `363.254472` IOPS; every row completed 1,200/1,200 lifecycle operations with zero timeouts and zero cleanup failures. |
| Production decision | **NO-GO**. The new TiDB code is not hosted-qualified yet; the latest packet had only PGlite above the 1,000-IOPS-per-drive threshold, SQLite was highly variable, and TiDB/FoundationDB missed it. Customer deployment, secure Ozone topology, 99.99% availability, five-minute RPO/RTO, backup/DR and release ownership remain explicit external boundaries. |
| Next action | Publish this ledger/tracker chunk, dispatch `ci.yml` from the resulting exact `origin/main`, wait for all W26 producers and the aggregate to become terminal, retain digests/logs, and update the decision only from one-revision evidence. |
...
| --- | --- | --- | --- | ---: | --- |
| W26.1 — pinned Ozone gateway and health/readiness harness | **PASS / 100% implementation; prior base gate passed** | Run `35691451007` base job `106629129201` passed gateway, readiness, restart/reopen and cleanup markers | Reconfirm markers in the packet containing `4098c7df` | 0–1 h review | Hosted image/runner and customer Ozone topology |
| W26.2 — immutable block contract through Ozone S3 gateway | **PASS / 100% implementation; prior functional gate passed** | Prior composition/base logs passed create/CAS, binary/range read, stopped-gateway fault, restart/reopen and owned-cleanup markers | Reconfirm on the next exact revision | 0.5–1 d review | Ozone gateway capacity, customer TLS/IAM and tenant isolation |
...
| W26.3c — FoundationDB/R2 composition | **FAIL / 100% implementation; hosted performance unresolved** | Latest row: `363.254472` IOPS, 1,200/1,200, timeout `0`, cleanup `0`; durable readiness, restart/reopen, bounded-listing and N-API markers passed | Investigate transaction/runner/provider latency after the TiDB packet | 0.5–1.5 d per review/remediation cycle | Hosted FoundationDB image/client, Ozone topology and durability |
| W26.3d — TiDB/R2 composition | **IMPLEMENTED / 100% code; hosted performance unresolved** | `4098c7df` published autocommit CAS chunk; latest pre-chunk row was `333.356025` IOPS with all 1,200 lifecycle operations and TiDB/reopen markers passing | Run exact-SHA Ozone TiDB job and compare IOPS/p95; preserve fencing and ambiguous-commit markers | 1–3 d per review/remediation cycle | Hosted TiDB/PD/TiKV, Ozone topology and provider-native latency |
...
| W26.11 — one-revision evidence aggregation | **IMPLEMENTED / 100% local verifier; latest packet failed closed** | Aggregate `106631474430` rejected missing `OZONE_IOPS_PASS` for SQLite/R2 and PGlite/R2; no provider failure was promoted | Run aggregate against exact `4098c7df` descendant and require all producer markers | 0.5–1.5 d review | Workflow scheduling and artifact availability |
...
| P14 — final integration-readiness review | **NO-GO / 45% provisional** | Local code, security and verifier gates are strong; hosted packet is terminal but failed the hard target; customer security/SLO/DR/native/release evidence is not W26-owned | Re-audit a terminal all-provider/end-to-end pass and issue an explicit readiness decision; no deployment or release claim | 1–2 d after W26.15 | Customer Ozone security/SLO/DR and separate release stream |
