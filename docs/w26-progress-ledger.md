...
does not change the tested source or lockfile. The producer jobs are terminal;
the single-revision aggregate job remains queued, so this packet is not yet a
...
| One-revision aggregate | **QUEUED — job `106685891954`** | GitHub has not emitted a terminal aggregate conclusion for run `35707725455`; queued state is not acceptance and no aggregate marker is promoted. | Wait for the aggregate, retrieve its logs/artifact, and verify every producer row binds to `1ceaa964` before updating acceptance. |
...
| W26.4 — Rust/Node/CLI/HTTP/N-API end-to-end surfaces | **OPEN / 100% implementation; producer markers observed, aggregate pending** | Current composition/FDB/TiDB artifacts contain the advertised surface, reopen/restart and cleanup markers. | Require one terminal aggregate containing every surface marker. | 0.5–1.5 d review | Native runners, provider fixtures and CI orchestration. |
...
| W26.11 — one-revision evidence aggregation | **OPEN / 100% verifier; 0% current aggregate acceptance** | Aggregate job `106685891954` is queued; no conclusion or marker is promoted. | Wait, retrieve and validate the terminal aggregate on `1ceaa964`. | 0.5–1.5 d review | GitHub scheduling and artifact availability. |
...
| W26.14 — complete end-to-end packet surface enforcement | **OPEN / 100% local control; aggregate pending** | Current producer artifacts contain the required gateway, provider, restart, listing, Node/native and cleanup markers. | Produce and review one terminal exact-head all-surface packet. | 0.75–1.5 d review | Native runners, provider fixtures and CI orchestration. |
...
| 2026-09-22 — next gate | Wait for aggregate job `106685891954`, retrieve terminal output, then choose the next SQLite/TiDB/FoundationDB correctness-preserving optimization. | ~0.25–0.75 h provisional | ~0.5–2 h provisional CI/provider queue | Do not mark W26.15 or P14 complete until four provider rows and the aggregate pass. |
