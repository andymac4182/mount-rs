...
| Repository | `mount-rs` |
| Snapshot base | `9de159a` (the bounded directory-enumeration implementation `fddf4e1` is published over concurrent mainline changes; hosted CI run `35606250115` is pending and is not evidence yet) |
| Checklist completion | 4 of 4 W26 tracker rows checked: 100% |
| Provisional execution completion | W26 qualification: 100%; production-rollout readiness: 24% (scope, benchmark matrix, lifecycle safety, bounded remote enumeration and CI acceptance baseline captured; no terminal production gates yet). Customer deployment, native, provider-durability and release-stream gates remain separately bounded |
...
| Current-head CI attempt | The provider-matrix chunk was published at `a0b2fae`; lifecycle reconciliation and streamed-listing/zero-grace safety were published at `69dd740`; bounded directory enumeration was implemented in `fddf4e1` and published at `9de159a`. GitHub Actions run `35606250115` for `9de159a` is currently pending. No queued, in-progress, canceled or failed run is promoted to evidence; the retained W26 packet above remains the last accepted hosted result. |
...
| W26.6 — bounded remote directory enumeration and response materialization | Built-in implementation complete; provider/native contracts open | 100% implementation; 55% production qualification | `FsDriver::readdir_bounded` fails closed with `ENOTSUP` by default. HTTP `/entries` and directory-file routes request the bound before serialization and map overflow to the existing 413/connection-close contract. Memory, host, chunked, versioned, persisted, observability, CLI and native N-API wrappers implement or forward the boundary; JavaScript-owned callbacks remain fail-closed because their callback contract cannot constrain provider-side enumeration. Terminal local evidence: HTTP 8 unit + 12 integration, core 11, host 5, chunked 14, CLI 44 and observability 4 tests; affected workspace check and formatting passed. | Add provider-native bounded/paginated enumeration for key-value and JavaScript-owned metadata providers; retain provider-specific skips/`ENOTSUP` until those contracts exist; add hosted secure/provider tests and review the fresh security scan on the final revision. | ~2–3 h implementation/local verification; ~1–3 d provider/security/hosted review | Key-value store listing API, JavaScript callback/provider contract, hosted provider topologies, secure Ozone fixture and security-review execution are external or cross-workstream gates. |
...
| P2 — production metadata-provider support matrix | Hosted/provider CI | Ozone provider matrix expanded; terminal all-provider evidence pending | 40% | The benchmark now has explicit SQLite/R2, PGlite/R2, TiDB/R2 and FoundationDB/R2 rows. SQLite is eligible from the Ozone composition job; PGlite, TiDB and FoundationDB are configuration/feature-gated and appear as explicit skips when their CI topology is absent. Existing SQLite/PGlite, durable TiDB and durable FoundationDB composition lanes remain separately tracked. | Retain terminal Ozone results for each configured provider on one revision, record provider versions/HA/failure semantics and unsupported combinations; do not promote an explicitly skipped provider | 3–8 d | CI capacity, provider images/versions, managed-service access if required and provider-specific operator limits |
...
| P10 — security, privacy, tenancy and audit review | Implementation / hosted CI / security review | Threat-model checkpoint and local controls recorded; lifecycle and pre-materialization listing hardening added; final review open | 50% | The delegated architecture worker mapped provider composition, credentials, object prefixes, cleanup, Rust/Node/CLI/N-API/HTTP/native paths and customer/Ozone ownership. Local R2 endpoint/TLS policy, HTTP loopback-only binding, bearer isolation/constant-time comparison, redaction, connection/request bounds, pre-materialization directory entry limits, serialized-response limits, telemetry redaction and scoped cleanup checks pass; strict affected-workspace check is green; scoped reconciliation fails closed for unsupported providers, renews the writer lease, protects live/open-unlinked roots, validates block IDs and deletes only aged objects under the configured prefix. `FsDriver::readdir_bounded` now fails closed for unsupported providers and is implemented/forwarded for built-in Rust paths; key-value and JavaScript provider-side enumeration remain explicit gates. Standard scan `943a7c01-e25f-48ee-952f-040b7421e80d` is still running against older revision `f54dff8`, while current-revision run `35606250115` is pending; no final no-findings or vulnerability result is claimed. | Complete discovery/validation on the current revision or a refreshed diff, review dependency/image provenance, authorization/isolation, encryption expectations, audit fields and abuse limits; implement provider-side pagination for remaining providers or retain documented fail-closed contracts; close findings or record exceptions; retain secure Ozone auth/rotation evidence where available | 4–10 d | Security reviewer, scan worker completion, customer identity/tenancy model, secure Ozone endpoint/certificates, compliance requirements and scanning infrastructure |
| P11 — end-to-end client, mount and platform qualification | Native/provider CI / cross-workstream | HTTP path and built-in bounded-listing contract added to Ozone CI; full matrix required | 30% | W26 covers Rust/Node/CLI and the shipped HTTP server/client path through the Ozone composition gate. The HTTP row verifies binary/full/range I/O, cross-drive auth rejection, graceful shutdown/reopen and scoped object cleanup; the built-in Rust HTTP providers now enforce the directory bound before response materialization; native mount, JavaScript/key-value bounded providers and every platform are not yet accepted | Review terminal `ozone-compositions` evidence for the HTTP marker and cleanup, add bounded provider rows for key-value/JavaScript-owned metadata, then exercise every advertised native/mount surface; retain platform/provider matrices, restart/recovery and negative capability evidence | 5–15 d depending on advertised platforms | macOS/Linux/Windows runners, privileged mount facilities, native workstreams, provider callback contracts, signing and provider connectivity |
...
- [x] Add a fail-closed bounded directory-enumeration contract before HTTP
  ...
  contract. Key-value and JavaScript-owned providers remain explicit
  `ENOTSUP`/pagination gates until their provider APIs can bound enumeration.
...
| 2026-09-21 — Bounded directory materialization hardening | Added `FsDriver::readdir_bounded`, made HTTP directory routes request the bound before serialization, implemented bounded enumeration for MemoryFs, HostFs, ChunkedFs and VersionedView, forwarded it through persisted/observability/CLI/native wrappers, and kept JavaScript-owned callbacks fail-closed. | ~2–3 h | ~0.25 h shared-target build/test wait; hosted CI run `35606250115` pending | HTTP 8 unit + 12 integration, core 11, host 5, chunked 14, CLI 44, observability 4 and persistence 4 local tests/targets passed; affected workspace check and formatting passed. Key-value and JavaScript provider-side pagination remain open production gates. |
...
is recorded above. Hosted run `35606250115` is pending and is not treated as a
pass; key-value and JavaScript-owned provider enumeration remain fail-closed
until provider-native pagination/limits are implemented or explicitly accepted
as support-matrix gates.
