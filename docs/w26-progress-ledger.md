...
| Repository | `mount-rs` |
| Snapshot base | `ce7b365a` (current origin/main before this ledger publication; it carries the W26 evidence-packet verifier, complete-surface marker checks, metric-integrity checks, credential-free rollout contract and concurrent mainline changes; no matching current-tip hosted W26 result is promoted) |
| Checklist completion | 11 of 11 W26 tracker rows checked: 100% implementation scope; hosted/provider production gates remain open |
| Provisional execution completion | W26 qualification: 100%; production-rollout readiness: 50% (scope, benchmark matrix, lifecycle safety, bounded remote enumeration, provider-bounded KV/N-API contract, durable-provider bounded-listing tests, strict provider-specific hard-threshold IOPS wiring, fail-closed artifact/profile/metric verification and retention, all-provider credential-free production-config policy, one-revision CI evidence-packet aggregation, complete end-to-end surface marker enforcement and the credential-free customer rollout contract are implemented and locally tested; no terminal production gates yet). Customer deployment, native, provider-durability, capacity/SLO, backup/DR and release-stream gates remain separately bounded |
| Current acceptance state | The retained W26 packet on `9c098e5` remains accepted within its documented provider/platform boundaries. The newer provider-bounded KV/N-API implementation, unstorage parity fixtures, SQLite/PGlite/TiDB/FoundationDB Ozone bounded-listing assertions, durable-provider Node/N-API assertions, exact-limit regression, SDK debug redaction, dedicated TiDB/FoundationDB Ozone IOPS harnesses, strict provider-configuration qualification, fail-closed IOPS artifact/profile/metric verification and retention policy, expanded credential-free all-provider production-config policy, complete evidence-packet verifier, complete end-to-end surface marker gate and credential-free customer rollout contract are locally green, syntax-checked or compile-checked. The full locked workspace test command `./scripts/cargo-shared test --workspace --all-targets --locked` also exited 0; explicitly environment-gated native/provider rows remain skips, not passes. Current `origin/main` is `ce7b365a`; the revision-bound manual run below is not yet terminal and no current-tip W26 result is promoted. No current-tip Live Cloudflare R2 or Live AWS S3 result is recorded. Focused scans `5fc5a943-07bb-4979-9f37-efd87a7f505e`, `60269206-bb22-4b78-aaf7-f05d16ffcca0`, `1d97028f-e153-4b49-9fac-c3a8c1fc1117`, `c67ae8e0-99af-4ade-8284-a612d599b5e4`, `04c7ba9d-ad9f-40aa-a5b2-d28b9a46a564`, `33c09a35-77f5-417c-862c-e5848185f50e` and `e4ce1aab-c6cd-44e4-b20d-3130ca357412` found zero reportable findings with hosted/provider/customer coverage explicitly deferred; production-config scan `d74e3e86-2e0a-45cf-9819-e31f428eb5d4`, negative-path scan `7addeeb5-4601-4951-aca9-becffb9bd4b9`, retention scan `18010cad-ed69-4da3-b0a9-57163091e878` and contract fixtures also pass locally |
...
| Current-head CI attempt | Manual workflow-dispatch run `35635486040` is qualifying head `cfe7e29e001dc01f2fa430a54bc8981b44db0b05`, which has the same W26 packet/workflow surfaces as the current tip (`git diff` is empty for the W26 verifier, benchmark, Ozone scripts, provider tests and CI workflow). At the last review, `ozone` job `106451808629`, `ozone-compositions` `106451808739` and `ozone-tidb` `106451809745` were in progress; `ozone-foundationdb` `106451808796` was queued and the aggregate job had not yet appeared. This is not evidence until every W26 producer and `w26-ozone-evidence` are terminally successful on one revision. No Live Cloudflare R2 or Live AWS S3 result is recorded for this tip. Only terminal W26 jobs with strict provider markers, retained logs/artifacts and a passing aggregate packet verifier can be promoted; the retained W26 packet above remains the last accepted hosted result. |
...
| W26.14 — complete end-to-end Ozone evidence-surface gate | Implementation complete; terminal hosted packet and customer admission open | 100% implementation; 35% production qualification | Expanded `scripts/verify-w26-ozone-evidence-packet.mjs` to require the markers emitted by every currently wired Ozone surface: gateway health/ready/restart, SQLite and PGlite composition plus bounded listing, Rust CLI, Node provider matrix, Node CLI, remote HTTP CLI, TiDB Rust composition and N-API seed/reopen, and FoundationDB Rust composition/restart and N-API seed/reopen. Synthetic packet coverage now proves a missing Node CLI marker fails closed. Local benchmark/evidence tests, Node/shell syntax, YAML parsing, diff checks and the full locked workspace test command exited 0 (with explicit native/provider skips retained). Code commit `20a06b8` was reconciled with concurrent mainline changes and published at `0e0454d`; focused scan `e4ce1aab-c6cd-44e4-b20d-3130ca357412` found zero reportable findings. | Review one terminal retained packet on the same revision and record every required surface marker, strict provider artifact, restart/recovery marker and cleanup outcome; then obtain native/mount and customer secure-runtime evidence. | ~0.75–1.5 h implementation/tests; ~0.75–1.25 h security review; ~0.5–1 h hosted review | Hosted provider/N-API execution, artifact authorization, native mount runners, customer TLS/IAM/rotation and measured SLO/RPO/RTO remain external. |
...
| --- | --- | --- |
| Hosted result inspection | 0.5–1.5 d engineering plus external queue time | The historical W26 packet is green on `35585066458`; revision-bound manual run `35635486040` is now in progress on head `cfe7e29e001dc01f2fa430a54bc8981b44db0b05`. Its Ozone, composition and TiDB jobs are active and its FoundationDB job is queued; the aggregate packet job has not appeared yet. Dedicated TiDB/FoundationDB IOPS artifacts, verifier PASS markers, strict no-skip evidence and the production-config policy output still require a terminal retained W26 run. |
