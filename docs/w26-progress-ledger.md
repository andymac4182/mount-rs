...
| Repository | `mount-rs` |
| Snapshot base | `63dbdbdb1dd23dd472d1b6c9bcce6d7089db99b0` (`origin/main` after the latest hosted TiDB restart attempt) |
...
| Current acceptance state | Local Ozone, SQLite/PGlite, single-node TiDB/Ozone, durable three-node FoundationDB/Ozone, hosted Ozone, hosted SQLite/PGlite, Node SDK, and CLI evidence passed; durable multi-node TiDB/Ozone hosted restart acceptance failed at the first TiDB frontend restart and remains open |
| Latest hosted workflow | GitHub Actions run `35581168122` on `63dbdbd`; `ozone` job `106274147767` and `ozone-compositions` job `106274147763` passed; `tidb` job `106274147942` and `tidb-rustfs` job `106274147791` failed at the same restart readiness boundary |
...
| W26.3d — durable multi-node TiDB composition over Ozone | Harness implementation complete; hosted acceptance failed at frontend restart | 80% | Hosted run `35581168122` on `63dbdbd`: direct TiDB identity/provider tests and standalone ambiguous-commit passed; `tidb-rustfs` also passed direct provider, `TIDB_CHUNKED_RUSTFS_SEED_PASS`, and its ambiguous-commit phase. Both jobs then timed out after 300 seconds at `TiDB phase=TiDB restart readiness`; the TiDB container stayed running but never exposed SQL/status. The 30-second graceful restart change (`63dbdbd`) did not alter the failure. | Run the next bounded isolation fix with the dropped-COMMIT failure injection after durable restart/reopen. Review the new `tidb` and `tidb-rustfs` jobs together; only a terminal `TIDB_ACCEPTANCE evidence=durable-multinode-restart` closes this item. | 1–4 h implementation/retry; 5–15 min result review | GitHub-hosted runner scheduling, concurrent pushes that cancel in-progress workflows, and the hosted Docker/TiDB/PD/TiKV environment. This gate cannot be accepted from local underprovisioned Docker. |
...
- An active, queued, skipped, canceled, or failed hosted job is not a pass.
  ...
  restart gate; W26 therefore remains open.
...
- [ ] Move the dropped-COMMIT failure injection after durable restart/reopen,
  synchronize with the latest `origin/main`, and rerun the bounded hosted
  gate; do not report the failed or canceled jobs as acceptance.
- [ ] Review the next terminal `ozone`, `ozone-compositions`, `tidb`, and
  `tidb-rustfs` jobs together on the retained revision.
...
| 2026-09-21 — Ledger refresh and restart isolation | Updated this ledger and `WORK_TRACKER.md` with workflow `35581168122` and separated the intentional commit-drop test from the durable restart sequence in `scripts/test-tidb.sh`. | ~0.5 h | ~0.5 h log retrieval/review | This chunk is ready to push; the next hosted run will validate the isolation change. |
