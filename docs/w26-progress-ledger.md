...
| Repository | `mount-rs` |
| Snapshot base | `76450474` (current `origin/main`; concurrent W01/W04 mainline work is preserved, while the W26 code exercised by hosted run `35641941218` is exact head `88b707ba`; the W26 evidence-packet verifier, complete-surface marker checks, metric-integrity checks, credential-free rollout contract and optimistic write/read concurrency path remain in the published history) |
...
| Current acceptance state | The retained W26 packet on `9c098e5` remains the last accepted hosted result within its documented provider/platform boundaries. The W26-owned `ChunkedFs` optimistic write path (published implementation commit `a2075af0`, merged tip `cd6dea28`) overlaps immutable block I/O while preserving revision/base checks, lease fencing and serialized publication; the follow-up read path and blocked-read/shutdown plus stale-read/concurrent-write regressions are published in implementation commit `c4378dc1` at tested code tip `88b707ba`. The full locked workspace test and strict Clippy commands both exited 0; the 8-test concurrency suite and 14 chunked unit tests are green, while environment-gated native/provider rows remain skips, not passes. Replacement run `35641941218` is terminal for every W26 producer and aggregate: all four configured provider rows completed 1,200/1,200 lifecycle operations with zero timeouts/cleanup failures but missed the hard 1,000-IOPS target, so the producer and aggregate jobs failed closed and no pass marker was emitted. Its overall workflow is still in progress only because unrelated native jobs remain active. Focused security scan `e7c03690-856b-42ae-8d47-8af0dc788a54` completed with zero reportable findings for the changed ChunkedFs file; its coverage is partial and explicitly defers hosted/provider/customer controls. No Live Cloudflare R2 or Live AWS S3 result is recorded. |
...
| Current-head CI attempt | The replacement packet improved measured lifecycle IOPS to SQLite/R2 85.70, PGlite/R2 94.04, TiDB/R2 14.50 and FoundationDB/R2 30.46, but all remain far below 1,000. Each row recorded 400 writes, 400 reads, 400 verified reads and 400 deletes; 1,200/1,200 operations succeeded with zero timeouts and cleanup failures. The W26-owned remediation overlaps remote immutable reads and writes outside the volume-wide metadata gate, reacquires the gate for fenced revision-checked publication, retains serialized conflict fallback and blocks shutdown until optimistic operations finish. Local correctness evidence is green, but the hosted result proves that this first overlap design is insufficient; W26.15 remains open and the threshold is unchanged. |
...
| P8 — load, capacity, soak and cost envelope | Hosted/provider CI plus W26.15 remediation | Hard-threshold and one-revision packet gates implemented; replacement packet failed performance | 40% | The benchmark records successful write+read+delete lifecycle IOPS, supports a hard `--min-iops` threshold, strict `--require-configured` provider qualification and redacted JSON. The fixed 4 KiB/400-iteration/concurrency-64 profile is enforced. Replacement run `35641941218` completed all 1,200 lifecycle operations per row with zero timeouts/cleanup failures but measured only 85.70/94.04/14.50/30.46 IOPS; aggregate job `106477164582` failed closed. W26.15 overlaps remote immutable reads/writes outside the metadata gate, with local concurrency, full workspace tests and strict Clippy green, but this design is insufficient. | Continue correctness-preserving metadata-publication/concurrency work or obtain production-like Ozone capacity/topology evidence. Then retain p95/p99 latency, errors, CPU/memory and topology, and add soak/capacity variants before treating the target as qualified | 1–4 d W26 performance follow-up; 1–2 d hosted review; soak effort provisional | Ozone fixture capacity/topology, stable hosted runners, artifact service, provider quotas and customer 99.99% capacity remain external; do not lower the target |
...
| P14 — final W26 integration-readiness review | W26 implementation / hosted CI / handoff | NO-GO review documented; cannot close while W26.15 is open | 24% | Replacement packet `35641941218` is audited: base Ozone passed, all four provider lifecycles completed but missed the hard IOPS target, aggregate `106477164582` failed closed, and the focused current-tip security scan found zero local reportable findings with hosted/customer controls deferred. Local read/write overlap, shutdown ordering and full regression evidence are green, but hosted performance remains below target. | Close W26.15, rerun/retain one current-tip packet with every provider and end-to-end marker, attach provider/platform/security/performance evidence, list customer/Ozone dependencies and hand off an explicit integration-ready or NO-GO decision to the release/deployment streams | 1–2 d review after performance gate; 1–4 d W26.15 follow-up | All W26-owned CI gates plus external customer Ozone secure topology, capacity/SLO, DR and release-stream confirmations |
...
- [ ] P8/W26.15: close the per-drive 1,000-IOPS CI workload with latency,
  ...
  lifecycle operations with zero timeout/cleanup failures, so the remaining
  blocker is throughput/metadata publication rather than lifecycle completion.
  Resolve or qualify the shared concurrency/capacity blocker, then rerun the
  full packet; do not lower the threshold or convert failed rows to skips.
...
| --- | --- | --- |
| Hosted result inspection | 0.5–1.5 d engineering plus external queue time | Historical green packet `35585066458` remains the last accepted hosted result. Replacement run `35641941218` on exact W26 code head `88b707ba` is terminal for all W26 jobs but diagnostic: base Ozone passed, all four provider rows completed 1,200/1,200 operations with zero timeouts/cleanup failures but missed 1,000 IOPS, and aggregate job `106477164582` failed closed. A new current-tip retained packet is required after further W26.15 remediation or explicit Ozone capacity qualification. |
...
| Performance and end-to-end matrix (P8, P11, P14, W26.15) | 12–32 d engineering | The workload and packet are implemented, but both terminal diagnostic packets failed the hard provider target. The first read/write-overlap design improved IOPS but remained far below target, especially for TiDB/FoundationDB. Requires a further safe metadata-publication/concurrency decision or production-like Ozone capacity qualification, stable hosted CI runners, native runners and a one-revision audit. |
