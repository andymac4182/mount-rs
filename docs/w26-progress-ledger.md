...
| Repository | `mount-rs` |
| Snapshot base | `3c884bd8c0d0199a17e4c355c36d45f660c7c786` (`origin/main`, including the published R2 content-addressing/cache, metadata mutation batching, queue-cancellation hardening, concurrent whole-file-create inode rebasing, corrected Ozone content-addressed block-contract test and deduplicated content-addressed cleanup; the exact W26 commits and concurrent-mainline merges are recorded below) |
...
| Latest hosted workflow | Fresh manual run `35665734382` selected exact tested revision `8c53d2c926137fba6034bf0edcb662999c71d426`, a descendant containing the published concurrent-create fix. W26 producer jobs are `ozone-foundationdb` `106550958697` (queued), `ozone-compositions` `106550958843` (queued), `ozone-tidb` `106550958877` (in progress) and `ozone` `106550958998` (queued); the aggregate job is not yet present. This is the required current-tip performance and end-to-end qualification; no job is acceptance evidence until every producer and the aggregate are terminally successful on this one revision. |
...
| Next hosted qualification | Manual run `35665734382` against exact SHA `8c53d2c926137fba6034bf0edcb662999c71d426` | Await terminal producer and aggregate output, retain provider JSON/log artifacts, and inspect whether concurrent-create inode rebasing materially closes the hard target. Accept W26.15 only if every configured provider and the complete end-to-end packet emit terminal pass markers on this one revision; otherwise continue correctness-preserving performance work. The strict 1,000-IOPS target is unchanged. | GitHub-hosted runners, Ozone/provider startup, provider latency and artifact retention are external gates; queued, in-progress, canceled, failed or skipped jobs are not evidence |
...
| Release/acceptance decision | W26 implementation and local qualification controls remain accepted within their documented scope; production rollout remains **NO-GO** because terminal run `35655276021` failed every hard 1,000-IOPS provider row and aggregate job `106522172018` failed closed. The concurrent-create fix is published through `origin/main` `3c884bd8` and security-scanned with zero reportable findings, but fresh hosted run `35665734382` on exact SHA `8c53d2c9` is still queued/in progress with no aggregate job. Earlier standard, policy, negative-path, strict-IOPS, evidence-packet and rollout-contract scans remain recorded above; no failed, queued or in-progress run is promoted, and no broader native, customer secure-runtime, Ozone backup/DR or release claim is made |
...
`origin/main` `3c884bd8`. The R2 block path uses content-addressed object IDs and a
...
`3c884bd8c0d0199a17e4c355c36d45f660c7c786`, and this checkout matches it.
...
and fresh run `35665734382` is pending on `8c53d2c9`. No queued, in-progress,
...
`3c884bd8` contains the lazy-atime/EOF, batching/cache, queue-safety,
