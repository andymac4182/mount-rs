...
| Repository | `mount-rs` |
| Snapshot base | `3681f27` (latest published origin/main head carrying the expanded credential-free Ozone security negative paths plus concurrent mainline changes; current hosted workflows are pending or in progress and are not evidence) |
...
| Current-head CI attempt | Published tip `3681f27` has CI run `35624846453` pending, Fault injection run `35624846354` in progress, W08 release targets run `35624846413` pending and W08 release policy run `35624846541` pending; no Live Cloudflare R2 or Live AWS S3 run was recorded for this tip. Only a terminal W26 CI job with its required markers can be promoted; the retained W26 packet above remains the last accepted hosted result. |
