...
| Repository | `mount-rs` |
| Snapshot base | `414a469` (published durable-provider IOPS qualification baseline after the TiDB/FoundationDB Ozone Node/N-API bounded-listing work and concurrent mainline changes; current hosted workflows are canceled or failed and are not evidence) |
| Checklist completion | 4 of 4 W26 tracker rows checked: 100% |
| Provisional execution completion | W26 qualification: 100%; production-rollout readiness: 32% (scope, benchmark matrix, lifecycle safety, bounded remote enumeration, provider-bounded KV/N-API contract, durable-provider bounded-listing tests, dedicated durable-provider hard-threshold IOPS wiring, current-revision security review and CI acceptance baseline captured; no terminal production gates yet). Customer deployment, native, provider-durability and release-stream gates remain separately bounded |
| Current acceptance state | The retained W26 packet on `9c098e5` remains accepted within its documented provider/platform boundaries. The newer provider-bounded KV/N-API implementation, unstorage parity fixtures, SQLite/PGlite/TiDB/FoundationDB Ozone bounded-listing assertions, FoundationDB and TiDB Ozone Node/N-API bounded assertions, exact-limit regression, SDK debug redaction and dedicated TiDB/FoundationDB Ozone IOPS harnesses are locally green, syntax-checked or compile-checked. Revision `414a469` hosted CI was canceled before jobs started and Live Cloudflare R2 failed, so no current-tip result is evidence; focused diff scan `5fc5a943-07bb-4979-9f37-efd87a7f505e` found zero reportable findings with partial hosted/provider coverage; `MOUNTX_SOURCE` parity and the current standard security scan's hosted/customer controls remain explicitly bounded |
...
| Current-head CI attempt | Published tip `414a469` has CI run `35619958098` canceled before jobs started, Fault injection run `35619957904` canceled, W08 release targets run `35619957926` canceled, W08 release policy run `35619957983` canceled and Live Cloudflare R2 run `35619958070` failed; no Live AWS S3 run was recorded for this tip. No canceled or failed run is promoted to evidence; the retained W26 packet above remains the last accepted hosted result. |
...
| Production rollout track | Open, currently **NO-GO**; 0 of 15 production gates are terminally accepted. The 30% figure reflects scope decisions, local hardening, provider-boundary implementation and an acceptance baseline, not deployable readiness |
...
| Release/acceptance decision | W26 qualification accepted within the documented local/hosted provider scope on terminal run `35585066458`; current published-tip standard security scan `5ad61e60-20e3-4223-885a-d4b516d49bb1` reports zero reportable findings across 16 W26-relevant surfaces with semantic coverage explicitly partial, and focused IOPS diff scan `5fc5a943-07bb-4979-9f37-efd87a7f505e` reports zero reportable findings with hosted/provider follow-up deferred; production rollout remains **NO-GO** and no broader native release claim is made |
...
| --- | --- | --- |
| Hosted result inspection | 0.5–1.5 d engineering plus external queue time | The historical W26 packet is green on `35585066458`; the new `414a469` attempt was canceled before jobs started, so dedicated TiDB/FoundationDB IOPS artifacts still require a terminal retained run. |
