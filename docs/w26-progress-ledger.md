...
| Repository | `mount-rs` |
| Snapshot base | `5d53f08a289c0ba0662c29e6070f9519b47e19a3` (`origin/main`, including the published R2 content-addressing/cache, metadata mutation batching and queue-cancellation hardening; the exact W26 commits and concurrent-mainline merges are recorded below) |
...
| Current acceptance state | The retained W26 packet on `9c098e5` remains the last accepted hosted result within its documented provider/platform boundaries. The published W26 code now includes optimistic read/write paths, atomic whole-file `FsDriver::write_file`, lazy atime/EOF handling, content-addressed R2 blocks with a bounded process-local cache, fenced metadata mutation batching, a bounded pending queue and cancellation-safe runner cleanup. Local full locked-workspace tests and strict Clippy pass; the focused ChunkedFs suite has 18 passing tests including canceled-runner recovery and one-publication concurrent mutation coverage. The last terminal W26 producer packet, run `35655276021` on `4efec58c93a0cd6e6027676f069edf0c4cde3132`, completed all four provider lifecycle matrices with zero timeouts/cleanup failures but missed the hard 1,000-IOPS target; its producer jobs failed and the aggregate failed closed. The queue-hardening security diff scan `6d5a7665-d8cd-46b7-a9bd-73fec54f5431` completed with zero reportable findings; hosted/provider/customer controls remain explicit external boundaries. No Live Cloudflare R2 or Live AWS S3 result is recorded. |
| Latest hosted workflow | Manual run `35661836627` on exact tested revision `5d53f08a289c0ba0662c29e6070f9519b47e19a3` is currently queued. W26 producer jobs are `ozone` `106538643680`, `ozone-compositions` `106538643693`, `ozone-tidb` `106538643743` and `ozone-foundationdb` `106538643926`; the aggregate job is not yet terminal. This run includes the published mutation batching and queue-cancellation hardening and is the required current-tip qualification. |
...
| W26.3a — SQLite and disk-backed PGlite composition over real Ozone | Functional composition complete; latest terminal packet failed hard IOPS; current batching/cache rerun queued | 100% functional; 88% production qualification | Terminal packet `35655276021` completed 1,200/1,200 lifecycle operations with zero timeout/cleanup failures but measured SQLite/R2 20.35 and PGlite/R2 205.48 IOPS, both below 1,000; the producer and aggregate failed closed. Published R2 content-addressing/cache and metadata batching/queue hardening are included in current revision `5d53f08a`; manual rerun `35661836627` is queued on that exact SHA. | Inspect the retained current-tip composition artifacts and close only on strict provider pass markers plus aggregate end-to-end success; otherwise continue correctness-preserving publication/performance work. | ~1–4 d follow-up performance/design; 0.5–1.5 d hosted review | Current Ozone fixture capacity/topology and customer target capacity remain external, but the remaining publication/provider latency cost is still a W26 gate. |
...
| W26.3c — independent durable three-node FoundationDB composition over Ozone | Durable provider lifecycle and restart markers passed; latest terminal packet failed hard IOPS; current batching/cache rerun queued | 90% base; 84% extension | The arm64 durable FoundationDB proof and provider-backed bounded-listing contract remain green. Terminal hosted job `106517158383` in run `35655276021` completed all 1,200 lifecycle operations and measured 40.50 IOPS, with zero timeout/cleanup failures but `IOPS_TARGET_NOT_MET`; the artifact is diagnostic only. Current revision `5d53f08a` adds mutation batching and cancellation safety; manual rerun `35661836627` job `106538643926` is queued. Local FoundationDB binary linking remains blocked by missing `libfdb_c`. | Inspect the retained rerun while preserving durable restart, fencing, bounded-listing and cleanup assertions; do not promote until the provider and aggregate markers pass. | ~1–4 d shared concurrency follow-up; 0.75–1.5 h hosted review | FoundationDB client image/runtime, Ozone capacity/topology and customer secure durability remain external; local provider-native linking remains blocked. |
| W26.3d — durable multi-node TiDB composition over Ozone | Durable TiDB lifecycle and N-API bounded markers passed; latest terminal packet failed hard IOPS; current batching/cache rerun queued | 100% base; 84% extension | Terminal hosted job `106517158249` in run `35655276021` completed all 1,200 lifecycle operations and measured 18.09 IOPS, with zero timeout/cleanup failures but `IOPS_TARGET_NOT_MET`; the artifact is diagnostic only. Current revision `5d53f08a` adds mutation batching and cancellation safety; manual rerun `35661836627` job `106538643743` is queued. The atomic path retains fencing/CAS and serialized conflict fallback, with local full-workspace tests and Clippy green. | Inspect the retained rerun while preserving restart/ambiguous-commit/fencing/bounded-listing/cleanup assertions; do not promote until the provider and aggregate markers pass. | ~1–4 d shared concurrency follow-up; 0.75–1.5 h hosted review | GitHub-hosted TiDB/PD/TiKV, N-API runtime, Ozone capacity/topology and customer secure durability remain external. |
...
| W26.4b — CI wiring, pinned images, cleanup, and documentation | Implementation complete; latest terminal packet failed the hard provider gate; current revision rerun queued | 100% implementation; 63% hosted packet qualification | CI installs NAPI/PGlite with locked Rust dependencies, runs the Ozone composition matrix, Node/CLI coverage, the shipped HTTP server/client reopen path, and durable Ozone/TiDB/FoundationDB jobs. Terminal run `35655276021` retained the base/provider artifacts; base Ozone passed, every configured provider missed 1,000 IOPS and aggregate job `106522172018` failed closed. Manual run `35661836627` is queued on `5d53f08a` with W26 producer jobs `106538643680`, `106538643693`, `106538643743` and `106538643926`. | Retain exact current-tip artifacts, policy markers, end-to-end markers, performance markers and cleanup; promote only after all producer jobs and `w26-ozone-evidence` are terminally successful on one revision. | ~1–4 d performance follow-up; 0.75–1.5 d hosted review per retained run | Workflow concurrency, hosted provider startup, Ozone capacity/topology, artifact service and customer TLS/IAM remain external; no failed/queued/skipped artifact is acceptance evidence. |
| W26.14 — complete end-to-end Ozone evidence-surface gate | Implementation complete; latest terminal packet failed performance; current revision rerun queued | 100% implementation; 44% production qualification | Expanded `scripts/verify-w26-ozone-evidence-packet.mjs` to require gateway health/ready/restart, SQLite/PGlite composition and bounded listing, Rust CLI, Node provider matrix/CLI, remote HTTP CLI, TiDB Rust/N-API seed/reopen and FoundationDB Rust/N-API seed/reopen markers. Local benchmark/evidence tests, Node/shell/YAML checks, diff checks, full locked workspace tests and strict Clippy pass. Terminal run `35655276021` retained complete provider logs/artifacts, but all four hard IOPS rows failed and aggregate `106522172018` failed closed; manual run `35661836627` is queued on the current mutation-batching revision. No packet result is promoted. | Close the packet only after a post-remediation run has terminal pass evidence from every producer and aggregate; retain native/mount and customer secure-runtime evidence separately. | ~1–4 d W26 performance follow-up; ~0.75–1.5 d hosted review; ~0.75–1.25 h security review | Ozone fixture capacity/topology, hosted provider/N-API execution, artifact authorization, native mount runners, customer TLS/IAM/rotation and measured SLO/RPO/RTO remain external. |
| W26.15 — per-drive 1,000-IOPS qualification and safe performance remediation | Metadata batching, content-addressed cache and queue-cancellation safety implemented/published; hard target still open; **NO-GO** | 78% W26-owned implementation / 31% hosted qualification | Published W26 performance work now includes atomic whole-file write `96a25f17`, lazy atime/EOF `d1bc8fb9`, R2 content addressing/cache, mutation batching `c4e9a253` (published through `a1cb4ca9`) and queue hardening `d152fa1a` (included in current `origin/main` `5d53f08a`). The batcher publishes eligible whole-file/unlink mutations once under the lease, bounds pending requests at 1,024, drains canceled-runner requests with `EIO` and skips closed replies before applying a queued mutation. Focused ChunkedFs tests pass 18/18; full locked workspace tests, strict Clippy and diff checks pass; security scan `6d5a7665-d8cd-46b7-a9bd-73fec54f5431` found zero reportable findings within its local scope and explicitly defers hosted/customer gates. Terminal run `35655276021` remains diagnostic at SQLite/R2 20.35, PGlite/R2 205.48, TiDB/R2 18.09 and FoundationDB/R2 40.50 IOPS; every row completed 1,200/1,200 operations with zero timeout/cleanup failures but failed `IOPS_TARGET_NOT_MET`, and aggregate `106522172018` failed closed. Manual rerun `35661836627` is queued on exact revision `5d53f08a`. | Inspect current-tip provider metrics, artifacts and aggregate markers. If the hard target remains open, continue correctness-preserving provider/publication work or obtain an Ozone/customer capacity qualification; do not lower the target or convert failed rows to skips. | ~1.5–4 d implementation/design; ~0.75–1.5 d hosted review per rerun; external queue time excluded | GitHub runner/provider startup, Ozone fixture capacity/topology, provider-native performance, artifact retention, customer secure topology and capacity are external gates; no staging environment exists. |
...
| Qualification baseline | Historical green packet only | Revision `9c098e5` and hosted run `35585066458` are the last accepted packet within their documented boundaries. Terminal run `35655276021` on exact revision `4efec58c` is diagnostic and failed the hard provider-performance gate; current `origin/main` `5d53f08a` contains the lazy-atime/EOF, R2 cache, metadata batching and queue-hardening changes and is being qualified by queued run `35661836627` | Any performance/concurrency change gets a fresh full packet on the tested revision; no failed, queued, canceled or skipped result is promoted |
...
`5d53f08a289c0ba0662c29e6070f9519b47e19a3`, and this checkout matches it.
...
commit `d152fa1a` is included through the current concurrent-mainline tip. The
18-test focused ChunkedFs suite, full locked workspace tests, strict workspace
Clippy and diff checks are green. Security scan
`6d5a7665-d8cd-46b7-a9bd-73fec54f5431` is complete with zero reportable
findings. The last terminal W26 producer/aggregate packet is
`35655276021` on `4efec58c` and failed the strict IOPS gate; manual run
`35661836627` is queued on `5d53f08a`. No queued, in-progress, canceled or
