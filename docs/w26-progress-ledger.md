...
| Repository | `mount-rs` |
| Snapshot base | `75b900f` (latest observed origin/main head carrying the strict provider-configuration IOPS gate plus concurrent mainline changes; current hosted workflows are pending and are not evidence) |
...
| Current-head CI attempt | Observed tip `75b900f` has CI run `35627060518` pending, Fault injection run `35627060723` pending, W08 release targets run `35627060616` pending and W08 release policy run `35627060642` pending; no Live Cloudflare R2 or Live AWS S3 run was recorded for this tip. Only terminal W26 jobs with strict provider markers and retained artifacts can be promoted; the retained W26 packet above remains the last accepted hosted result. |
...
the concurrent reconciliation at `80344aa`, the later reconciliation at
`3552344`, and the latest observed concurrent reconciliation at `75b900f`.
Its CI run `35627060518`, W08 release targets `35627060616`, W08 release
policy `35627060642` and Fault injection `35627060723` are pending; these
states are not W26 acceptance evidence.
