...
| Repository | `mount-rs` |
| Snapshot base | `e1765b7b` (published atomic-write code plus the previous ledger checkpoint; the final remote tip after this documentation publication is recorded in the publication/session rows below) |
...
| Current acceptance state | The retained W26 packet on `9c098e5` remains the last accepted hosted result within its documented provider/platform boundaries. The W26-owned optimistic read/write paths and atomic whole-file `FsDriver::write_file` contract remain published; the atomic implementation is in `96a25f17`, reconciled into `origin/main` at `92f900be` and exercised by current-tip run `35649202405` on `e1765b7b`. The atomic path stages immutable blocks outside the metadata gate, flushes them, then performs one fenced namespace publication; optimistic conflicts retain the existing serialized open/write/close fallback. The full locked workspace test and strict Clippy commands both exited 0, including 15 ChunkedFs unit tests and 16 N-API tests; environment-gated native/provider rows remain skips, not passes. Current run `35649202405` is terminal for every W26 producer and aggregate: base Ozone passed, all four configured provider rows completed 1,200/1,200 lifecycle operations with zero timeouts/cleanup failures but missed the hard 1,000-IOPS target, and the aggregate failed closed without a pass marker. Focused security diff scan `11fa7e54-bb66-488d-a369-c1a7ac46da81` completed with zero reportable findings for all three changed files; its hosted/provider/customer coverage is partial and explicitly deferred. No Live Cloudflare R2 or Live AWS S3 result is recorded. |
...
| Next hosted qualification | After the next substantive W26-owned metadata-publication/performance chunk | The next retained CI packet must rerun `ozone`, `ozone-compositions`, `ozone-tidb`, `ozone-foundationdb` and `w26-ozone-evidence` on one revision, retain all four provider JSON/log artifacts, and report whether the strict 1,000-IOPS gate closes. Until then, current run `35649202405` is terminal diagnostic evidence only. | GitHub-hosted runners, Ozone/provider startup, provider latency and artifact retention are external gates; queued, canceled, failed or skipped jobs are not evidence |
...
| W26.15 — per-drive 1,000-IOPS qualification and safe performance remediation | Atomic whole-file write implementation measured; hard target still open; **NO-GO** | 62% W26-owned implementation / 31% hosted qualification | Implementation commit `96a25f17`, reconciled/pushed at `92f900be`, adds the public `FsDriver::write_file` contract, N-API adoption and a `ChunkedFs` optimistic block-stage/flush/one-publication path with serialized conflict fallback. Local 15-test ChunkedFs suite, full locked workspace tests, strict workspace Clippy, N-API check, storage benchmark unit tests, formatting/diff checks and security scan `11fa7e54-bb66-488d-a369-c1a7ac46da81` all pass/return zero findings. Current hosted run `35649202405` on `e1765b7b` measured SQLite/PGlite/TiDB/FoundationDB at 91.94/95.82/16.10/34.97 IOPS; every row completed 1,200/1,200 operations with zero timeout/cleanup failures but failed `IOPS_TARGET_NOT_MET`, and aggregate `106501216372` failed closed. | Implement and locally verify the next safe metadata-publication reduction; rerun the complete one-revision Ozone packet only after that substantive change, retaining exact provider metrics, latency, resource envelope, artifacts, cleanup and complete-surface markers. | ~1.5–4 d implementation/design; ~0.75–1.5 d hosted review per rerun; external queue time excluded | GitHub runner/provider startup, Ozone fixture capacity/topology, provider-native performance, artifact retention, customer secure topology and capacity are external gates; no staging environment exists. |
...
| P8 — load, capacity, soak and cost envelope | Hosted/provider CI plus W26.15 remediation | Hard-threshold and one-revision packet gates implemented; current packet failed performance; atomic write measured but insufficient | 48% | The benchmark records successful write+read+delete lifecycle IOPS, supports a hard `--min-iops` threshold, strict `--require-configured` provider qualification and redacted JSON. The fixed 4 KiB/400-iteration/concurrency-64 profile is enforced. Current run `35649202405` completed all 1,200 lifecycle operations per row with zero timeouts/cleanup failures but measured only 91.94/95.82/16.10/34.97 IOPS; aggregate job `106501216372` failed closed. Published commit `96a25f17` removed an avoidable public `writeFile` metadata publication through a fenced `ChunkedFs` whole-file path, and the hosted result shows a measured but inadequate improvement. | Continue correctness-preserving metadata-publication/provider-latency work, then retain a current-tip all-provider packet. After the hard target closes, retain p95/p99 latency, errors, CPU/memory and topology, and add soak/capacity variants before treating the target as qualified | 1.5–4 d W26 performance follow-up; 1–2 d hosted review; soak effort provisional | Ozone fixture capacity/topology, stable hosted runners, artifact service, provider quotas and customer 99.99% capacity remain external; do not lower the target |
...
workflow jobs were still running at an earlier poll.
