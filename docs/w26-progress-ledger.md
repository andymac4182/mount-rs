...
| W26.4b — CI wiring, pinned images, cleanup, and documentation | Implementation complete; new hosted provider gate pending | 100% implementation; 90% hosted packet | CI installs NAPI/PGlite with locked Rust dependencies, runs the Ozone composition matrix, Node/CLI coverage, and durable Ozone/TiDB job. FoundationDB harness uses platform-specific pinned official 7.4.7 image manifests and transaction readiness probes; the dedicated `ozone-foundationdb` job is now wired for the Ozone-backed durable mode. `tests/ozone/README.md`, `WORK_TRACKER.md`, and this ledger record the acceptance boundaries. Final retained W26 jobs `ozone` `106286459564`, `ozone-compositions` `106286459622`, and `ozone-tidb` `106286459540` passed on `35585066458`; the new FoundationDB job needs a terminal run before the packet is refreshed. | Run and review the new `ozone-foundationdb` job on a retained revision; keep W26 production readiness NO-GO until that result and the other open gates are terminal. | 0.5–1 h hosted review | GitHub workflow concurrency, nested FoundationDB client image startup and other workstream pushes can cancel runs; canceled/queued jobs are not evidence. |
...
| P3 — authentication, TLS, secret lifecycle and redaction | Implementation / hosted/provider CI | Plaintext remote endpoint gate improved; secure integration gate open | 40% | R2/Ozone config parsing and runtime validation reject non-HTTP(S), embedded credentials, query/fragment, missing-authority and remote plaintext-HTTP endpoints before client construction; HTTP remains allowed only for loopback/Docker test gateways; credentials remain environment references and diagnostics are redacted | Add authenticated HTTPS endpoint CI where available, certificate identity/rotation checks, secret injection/rotation references, least privilege and clean-client negative tests; retain no secret values | 3–7 d | Secure Ozone CI endpoint or customer-supplied fixture, certificates/identity, secret manager integration and security review |
