...
| Repository | `mount-rs` |
| Snapshot base | `0e0454d` (latest published origin/main head carrying the W26 evidence-packet verifier, complete-surface marker checks, metric-integrity checks, credential-free rollout contract and concurrent mainline changes; no matching current-tip hosted W26 result is promoted) |
...
| Current-head CI attempt | Published tip `0e0454d` contains the W26 credential-free rollout contract and complete end-to-end packet marker enforcement. Its push-triggered CI run is subject to the repository's cancel-on-push concurrency and is not evidence until `ozone`, `ozone-compositions`, `ozone-tidb`, `ozone-foundationdb` and `w26-ozone-evidence` are all terminally successful on this same revision. No Live Cloudflare R2 or Live AWS S3 result is recorded for this tip. Only terminal W26 jobs with strict provider markers, retained logs/artifacts and a passing aggregate packet verifier can be promoted; the retained W26 packet above remains the last accepted hosted result. |
