...
| Repository | `mount-rs` |
| Snapshot base | `ec58ba5` (Ozone composition bounded-listing qualification is published over concurrent mainline changes; current CI and fault-injection runs are still non-terminal and are not evidence yet) |
...
| Current-head CI attempt | Published tip `ec58ba5` has GitHub Actions CI run `35610948086` queued and Fault injection run `35610947492` queued; Live Cloudflare R2 run `35610947637` is in progress. No queued, in-progress, canceled or failed run is promoted to evidence; the retained W26 packet above remains the last accepted hosted result. |
