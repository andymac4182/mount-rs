...
| Repository | `mount-rs` |
| Snapshot base | `73cdc93` (exact provider-bound correction and regression test are published over concurrent mainline changes; current CI and fault-injection runs are still non-terminal and are not evidence yet) |
...
| Current-head CI attempt | Published tip `73cdc93` has GitHub Actions CI run `35611505333` pending, Fault injection run `35611505169` queued and Live Cloudflare R2 run `35611505545` queued. No queued, in-progress, canceled or failed run is promoted to evidence; the retained W26 packet above remains the last accepted hosted result. |
...
limits. Revision-specific CI `35611505333` is pending, Fault injection
`35611505169` is queued, and Live Cloudflare R2 `35611505545` is queued; these
remain nonterminal gates.
