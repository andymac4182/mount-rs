...
| Repository | `mount-rs` |
| Snapshot base | `9b3b90b` (latest published origin/main head carrying the expanded credential-free Ozone security negative paths plus concurrent mainline changes; current hosted workflows are pending or queued and are not evidence) |
...
| Current-head CI attempt | Published tip `9b3b90b` has CI run `35624670863` pending, Fault injection run `35624670707` queued, W08 release targets run `35624670789` pending and W08 release policy run `35624670708` pending; no Live Cloudflare R2 or Live AWS S3 run was recorded for this tip. Only a terminal W26 CI job with its required markers can be promoted; the retained W26 packet above remains the last accepted hosted result. |
...
| --- | --- | --- |
| Hosted result inspection | 0.5–1.5 d engineering plus external queue time | The historical W26 packet is green on `35585066458`; the latest published tip `9b3b90b` has CI `35624670863` pending, Fault injection `35624670707` queued, W08 release targets `35624670789` pending and W08 release policy `35624670708` pending. Dedicated TiDB/FoundationDB IOPS artifacts and the production-config policy output still require a terminal retained W26 run. |
...
published at `9b3b90b`, which is the current `origin/main` head at this ledger
snapshot. Its CI run `35624670863` is pending, W08 release targets
`35624670789` are pending, W08 release policy `35624670708` is pending and
Fault injection `35624670707` is queued; these states are not W26 acceptance
evidence.
