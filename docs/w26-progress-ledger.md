...
| Repository | `mount-rs` |
| Snapshot base | `6a1b94b` (latest published origin/main head carrying the expanded credential-free Ozone security negative paths after concurrent mainline changes; current hosted workflows are queued, pending, in progress or failed and are not evidence) |
| Checklist completion | 4 of 4 W26 tracker rows checked: 100% |
| Provisional execution completion | W26 qualification: 100%; production-rollout readiness: 35% (scope, benchmark matrix, lifecycle safety, bounded remote enumeration, provider-bounded KV/N-API contract, durable-provider bounded-listing tests, dedicated durable-provider hard-threshold IOPS wiring, expanded credential-free all-provider production-config policy, current-revision security reviews and CI acceptance baseline captured; no terminal production gates yet). Customer deployment, native, provider-durability and release-stream gates remain separately bounded |
| Current acceptance state | The retained W26 packet on `9c098e5` remains accepted within its documented provider/platform boundaries. The newer provider-bounded KV/N-API implementation, unstorage parity fixtures, SQLite/PGlite/TiDB/FoundationDB Ozone bounded-listing assertions, FoundationDB and TiDB Ozone Node/N-API bounded assertions, exact-limit regression, SDK debug redaction, dedicated TiDB/FoundationDB Ozone IOPS harnesses and expanded credential-free production-config policy are locally green, syntax-checked or compile-checked. Latest head `6a1b94b` has CI run `35623755790` queued, Fault injection `35623755596` completed successfully for its separate workflow, W08 release targets `35623755638` pending, W08 release policy `35623755613` in progress and Live Cloudflare R2 `35623755595` failed; none is W26 acceptance evidence. Focused IOPS diff scan `5fc5a943-07bb-4979-9f37-efd87a7f505e`, production-config diff scan `d74e3e86-2e0a-45cf-9819-e31f428eb5d4` and negative-path scan `7addeeb5-4601-4951-aca9-becffb9bd4b9` found zero reportable findings with hosted/provider coverage explicitly deferred; `MOUNTX_SOURCE` parity and hosted/customer security controls remain bounded |
...
| Current-head CI attempt | Published tip `6a1b94b` has CI run `35623755790` queued, Fault injection run `35623755596` successful for its separate workflow, W08 release targets run `35623755638` pending, W08 release policy run `35623755613` in progress and Live Cloudflare R2 run `35623755595` failed; no Live AWS S3 run was recorded for this tip. Only a terminal W26 CI job with its required markers can be promoted; the retained W26 packet above remains the last accepted hosted result. |
...
| Production rollout track | Open, currently **NO-GO**; 0 of 15 production gates are terminally accepted. The 35% figure reflects scope decisions, local hardening, provider-boundary implementation, expanded credential-free security policy and an acceptance baseline, not deployable readiness |
...
| --- | --- | --- |
| Hosted result inspection | 0.5–1.5 d engineering plus external queue time | The historical W26 packet is green on `35585066458`; the latest published tip `6a1b94b` has CI `35623755790` queued, W08 release targets `35623755638` pending, W08 release policy `35623755613` in progress and Live Cloudflare R2 `35623755595` failed. Dedicated TiDB/FoundationDB IOPS artifacts and the production-config policy output still require a terminal retained W26 run. |
...
published at `6a1b94b`, which is the current `origin/main` head at this ledger
snapshot. Its CI run `35623755790` is queued, W08 release targets
`35623755638` are pending, W08 release policy `35623755613` is in progress,
Fault injection `35623755596` completed successfully for its separate workflow,
and Live Cloudflare R2 `35623755595` failed; these states are not W26
acceptance evidence.
