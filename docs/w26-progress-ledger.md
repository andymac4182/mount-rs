...
| Repository | `mount-rs` |
| Snapshot base | `44b01a7` (current `origin/main` after the durable TiDB/FoundationDB bounded-listing qualification tests and concurrent mainline changes; all hosted workflows for this tip are canceled or failed and are not evidence) |
| Checklist completion | 4 of 4 W26 tracker rows checked: 100% |
| Provisional execution completion | W26 qualification: 100%; production-rollout readiness: 30% (scope, benchmark matrix, lifecycle safety, bounded remote enumeration, provider-bounded KV/N-API contract, durable-provider bounded-listing tests, current-revision security review and CI acceptance baseline captured; no terminal production gates yet). Customer deployment, native, provider-durability and release-stream gates remain separately bounded |
| Current acceptance state | The retained W26 packet on `9c098e5` remains accepted within its documented provider/platform boundaries. The newer provider-bounded KV/N-API implementation, unstorage parity fixtures, SQLite/PGlite/TiDB/FoundationDB Ozone bounded-listing assertions, exact-limit regression and SDK debug redaction are locally green or compile-checked. Revision `44b01a7` hosted workflows are canceled/failed, so they are not evidence; `MOUNTX_SOURCE` parity and the current standard security scan's hosted/customer controls remain explicitly bounded |
...
| Current-head CI attempt | Published tip `44b01a7` has CI run `35614910586` canceled, Fault injection run `35614910488` canceled, W08 release policy run `35614910527` canceled and Live Cloudflare R2 run `35614910486` failed; no Live AWS S3 run was recorded for this tip. No canceled or failed run is promoted to evidence; the retained W26 packet above remains the last accepted hosted result. |
...
| Release/acceptance decision | W26 qualification accepted within the documented local/hosted provider scope on terminal run `35585066458`; current published-tip security scan `5ad61e60-20e3-4223-885a-d4b516d49bb1` reports zero reportable findings across 16 W26-relevant surfaces with semantic coverage explicitly partial and hosted/customer controls deferred; production rollout remains **NO-GO** and no broader native release claim is made |
...
| W26.3c — independent durable three-node FoundationDB composition over Ozone | Implementation and local proof complete; hosted Ozone lane pending | 90% | The arm64 durable run used three fixed-address FoundationDB 7.4.7 containers, three coordinators, persistent per-server volumes, `double` redundancy and SSD storage. Transaction probes passed before and after node-2 restart; first-client and fresh-client Ozone compositions passed; `FOUNDATIONDB_TEST_PASS topology=durable`; owned resources were removed and verified absent. The published test extension `d1c9e44` now asserts provider-backed bounded listing and `EOVERFLOW` in both composition and reopen paths. TiDB/FoundationDB compile checks pass locally; FoundationDB binary linking is blocked on this macOS host by missing `libfdb_c`. CI has a dedicated `ozone-foundationdb` job that runs the same durable mode against the live Ozone gateway. | Review a terminal `ozone-foundationdb` job on a retained revision and record the exact FoundationDB/Ozone versions, bounded-listing markers, restart marker, cleanup and client image evidence. | 0.5–1 h hosted review | Hosted CI capacity, nested FoundationDB client image startup, and provider/native runtime are external. Local and hosted proofs remain loopback/nonsecure and do not cover production auth/TLS, power-loss behavior, or customer replication. |
