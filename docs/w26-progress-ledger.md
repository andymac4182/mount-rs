...
| Repository | `mount-rs` |
| Snapshot base | `3552344` (latest published origin/main head carrying the strict provider-configuration IOPS gate plus concurrent mainline changes; current hosted workflows are pending and are not evidence) |
...
| Current-head CI attempt | Published tip `3552344` has CI run `35626892113` pending, Fault injection run `35626892072` pending, W08 release targets run `35626892129` pending and W08 release policy run `35626892082` pending; no Live Cloudflare R2 or Live AWS S3 run was recorded for this tip. Only terminal W26 jobs with strict provider markers and retained artifacts can be promoted; the retained W26 packet above remains the last accepted hosted result. |
...
the concurrent reconciliation at `80344aa`, and the latest concurrent
reconciliation at `3552344`, which is the current `origin/main` head at this
ledger snapshot. Its CI run `35626892113`, W08 release targets
`35626892129`, W08 release policy `35626892082` and Fault injection
`35626892072` are pending; these states are not W26 acceptance evidence.
