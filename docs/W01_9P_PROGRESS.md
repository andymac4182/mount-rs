...
- The direct N-API session member-surface audit compares the native runtime's
  ...
  Exact SHA `fb532b46fd8b6c5af66dc9b771e84116b2997ca3` has hosted Native 9P run
  `35694841984` queued with N-API job `106639369581` and Rust job
  `106639369868`; those jobs are pending evidence and are not promoted to
  PASS. Production remains NO-GO until the hosted run reaches a terminal state.
- The attached-connection API parity packet was published at exact SHA
  ...
  is still pending with `jobs: []`; no hosted PASS is claimed. Production
  remains NO-GO.
