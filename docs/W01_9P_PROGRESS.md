...
- **PASS:** the dedicated hosted pinned-oracle 9P TCP conformance job passed
  `144` cases and reported `2` skipped root-gated ownership cases out of
  `146` total at published SHA `a6b3e2aa10cfdb3ee730d41c5886436b02c260de`,
  [Native 9P run `35707546973`](https://github.com/andymac4182/mount-rs/actions/runs/35707546973),
  job `106680012604`. The same run passed the existing N-API lifecycle job
  `106680012485` and Rust native lifecycle job `106680013203`; the local
  focused oracle/public-surface checks also passed. The two skips are the
  conformance suite's explicit non-root `lchown` cases, not unreported 9P
  protocol omissions.
...
- Keep the dedicated `upstream-9p` job in that workflow as the revision-matched
  pinned-oracle TCP protocol gate. It checks out oracle revision
  `85361a8212ff9bff8e69f62fa8993ef2c2ec51e8`, builds `p9_oracle` through
  `scripts/cargo-shared`, and runs `p9-conformance.test.mjs`; hosted SHA
  `a6b3e2aa10cfdb3ee730d41c5886436b02c260de` passed `144/146` with only the
  two explicit non-root ownership skips in job `106680012604`.
