...
- The next bounded server contract packet caches native `P9Connection` wrappers
  by their stable transport id, so repeated `P9Server.clients` reads preserve
  connection, session, and closed-promise identity like the pinned oracle. The
  new real-TCP regression also verifies removal after `close()`/`waitClosed()`;
  local syntax, focused execution, and adjacent checks are required before a
  hosted result is claimed. Production remains NO-GO.
...
| 2026-09-22 | N-API 9P native server client identity | The postlude now caches native `P9Connection` wrappers by stable transport id, preserving repeated `P9Server.clients` connection/session/closed identity and pruning wrappers after transport removal. A real-TCP regression covers registration, stable views, `close()`/`waitClosed()`, and cleanup; hosted Native 9P rerun is pending | Broader upstream session/protocol parity, automatic cross-transport signal ownership, supervisor-owned crash/reset recovery, and W01 acceptance remain open; production remains NO-GO |
