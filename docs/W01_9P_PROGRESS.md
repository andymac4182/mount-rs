...
| Native-listener stream boundary | Explicit supported-scope decision | Native Tokio-accepted connections expose `stream: undefined` and the N-API `peer: null`; callers requiring a Node `Duplex` use `server.attach`, whose attached connection retains the supplied stream and peer fallback |
...
- N-API option snapshots intentionally contain only serializable effective
  ...
  `address()`/`path` representations. A hosted direct-mount rerun is required
  to qualify the native `stream: undefined` and `peer: null` assertions.
...
| --- | --- | --- | --- |
| 2026-09-22 | N-API 9P member representation boundaries | The focused session metadata regression now checks effective callback-hook omission from serializable option snapshots, attached `Duplex`/peer/closed state, and pre-listen string/null address/path views; the direct native test also asserts the native listener's `stream: undefined` and `peer: null`. Local typecheck, metadata, mount-helper, syntax, and diff checks passed | Hosted direct `./9p` rerun is required for the native stream/peer assertions; broader upstream member parity, automatic cross-transport signal ownership, supervisor-owned crash/reset/half-close recovery, and W01 acceptance remain open |
