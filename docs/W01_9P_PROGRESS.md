...
| Session and connection objects | Local PASS for current exposed members; parity remains partial | `P9Session.handleCall`/`destroy`, scalar `options`, live `driver`, `userFor`, debug-gated assertions, request-error/assertion callbacks, live `locks` and `fids`, stats/lifecycle, live property-shaped `clients`, peer, `closed`, attached stream exposure, and identity/handle tests; lock-table option injection and mount helpers remain open |
...
- Native Tokio listener connections deliberately expose no transferable Node
  ...
  native and attached connections. Lock-table option injection and 9P mount
