...
| Session and connection objects | Local PASS for current exposed members; parity remains partial | `P9Session.handleCall`/`destroy`, scalar `options`, `userFor`, stats/lifecycle, clients, peer, `closed`, attached stream exposure, and identity tests; upstream driver/fid/lock/assertion surfaces remain open |
...
- Native Tokio listener connections deliberately expose no transferable Node
  ...
  low-level session now exposes scalar `options` and `userFor(fid)` in addition
  to direct call/destroy, stats, and lifecycle. Upstream `driver`, `fids`,
  `locks`, assertion/debug, property-shaped `clients`, and 9P mount/barrel
  helpers remain applicable parity work; they are not silently accepted out of
  scope.
