...
- The direct `./9p` mount-helper facade currently supports Linux-client probing,
  ...
  shared-server injection. Signals, extended server-policy fields, session
  callbacks, and the remaining oracle mount controls are not claimed by this
  packet. A hosted N-API native mount run is still required before these views
  are treated as native runtime-qualified.
...
- Native Tokio listener connections deliberately expose no transferable Node
  ...
  richer oracle mount options and hosted N-API native mount lifecycle remain
