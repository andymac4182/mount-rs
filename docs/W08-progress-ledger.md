...
| 2026-09-21 10:17–10:24 | Added and locally compiled the opt-in TiDB TLS feature through the provider, Rust SDK, CLI and N-API; added a dedicated CI compile gate and documented the boundary between TLS capability and a credentialed production handshake. | ~10 min | ~2 min compile wait | Public consumers can ship the TLS client graph; P01/P07 remain open pending target topology, certificates, secrets and live provider evidence. |
