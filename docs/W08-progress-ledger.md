...
Status snapshot: **2026-09-21 21:31 AEST**
Repository: `andymac4182/mount-rs`  
Current published tip: `origin/main` at `0e624d3`
...
| **P06** | Capacity, performance, load and soak qualification | Hosted/provider + implementation | **Open — 20%** | RustFS block benchmark and bounded provider/consumer tests passed, but they are not a representative production workload or capacity claim. `tests/provider_matrix/tidb-rustfs-soak.mjs` now provides a bounded TiDB/RustFS write-read workload with configurable operations, concurrency and payload size plus p50/p95/p99/throughput markers; the hosted composition job is configured to run 64 operations at concurrency 8 with 64 KiB payloads. | Establish workload mix and dataset/concurrency targets; retain a terminal soak marker, baseline/peak/saturation/failover and multi-hour results against production-like capacity; record latency/throughput/error budgets, resource growth, headroom, scaling and cost limits. | **2–4 engineer-days** | Representative workload, load generators, production-sized service capacity, resource collector and performance acceptance thresholds are required. |
...
| **W08.8** Bounded TiDB/RustFS load and soak evidence harness | **Implementation slice landed — 100%; hosted qualification pending** | `tests/provider_matrix/tidb-rustfs-soak.mjs` validates bounded operation/concurrency/payload inputs, performs concurrent write/read integrity checks through the public N-API chunked driver, computes p50/p95/p99 latency and throughput, cleans only its owned RustFS prefix, and fails closed when the TiDB URL or RustFS environment is absent. Local syntax, skip and missing-gate rejection checks passed. | `.github/workflows/ci.yml` enables 64 operations, concurrency 8 and 65,536-byte payloads in the real `tidb-rustfs` composition. A terminal hosted `TIDB_RUSTFS_SOAK_PASS` marker is required before recording hosted evidence; this remains bounded qualification, not a production capacity or cost claim. | Run the current hosted job, retain both seed/reopen soak markers and metrics, then repeat with the approved production-shaped workload, resource collector, failover and multi-hour duration. | **~1 engineer-day** implementation and local verification; hosted/provider execution tracked separately. | Real TiDB/RustFS service, N-API build, workload approval, production-sized capacity, resource telemetry and performance owner are external. |
