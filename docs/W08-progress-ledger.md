...
| **W08.6** Production deployment-contract policy and fail-closed configuration checks | **Implementation slice landed — 100%; production gate open** | `scripts/verify-w08-production-config.mjs` validates a `splitstore` shape with durable TiDB metadata, durable HTTPS S3/R2 blocks, external secret references, positive chunk size, no placeholders and TLS-required `MOUNT_RS_TIDB_TLS_URL`. The positive fixture passed, the insecure fixture failed closed, and the public Rust CLI `validate-config` command accepted the positive fixture. | The CI `tidb-tls-compile` job now runs the same positive/negative policy check without credentials or network. A hosted terminal pass is required before recording the CI evidence row; policy success remains implementation evidence and does not close topology, IAM, certificate, provider or production gates. | Run this verifier against the approved staging/production artifact; add the target topology, secret-manager, certificate and owner evidence to P01/P02/P07. | **~1 h** implementation and local verification; hosted wait tracked separately. | Approved deployment artifact, secret manager, IAM policy, certificates, network controls and provider endpoint are external. |
