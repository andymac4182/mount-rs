...
Status snapshot: **2026-09-21 22:17 AEST**
Repository: `andymac4182/mount-rs`  
Publication snapshot: `origin/main` at `d6abcbe` after the W08.9 HTTP health/readiness
implementation chunk `30dec1d` was published; this documentation chunk records
that publication and the remaining production gates.
...
| **P05** | Observability, SLOs, health checks and alerting | Implementation + hosted/provider | **Open — 15% gate weight; local probe slice 100%** | The HTTP transport now exposes unauthenticated `GET`/`HEAD /healthz` for listener/process liveness and `/readyz` for a non-empty configured drive registry; an empty registry returns `503 not_ready`. The locked `mount-rs-http` suite passed 8 unit tests and 10 integration tests, including the healthy/not-ready and method-boundary cases, and strict Clippy passed. These are process/configuration probes only: no production collector, dashboard, alert route, SLO, redaction, TiDB, RustFS or TLS health evidence is recorded here. | Define availability, mount/unmount, metadata latency, block latency, error-budget and recovery SLOs; map these probes and provider-specific checks to metrics/logs/traces, dashboards, alerts, paging, retention and secret/PII redaction; exercise an alert end-to-end. If the selected deployment needs provider-aware readiness, add and credential-test that separate probe. | **1–2 engineer-days**; **~0.5 engineer-day** for this implementation slice | Production collector, paging destination, ownership, alert thresholds, selected deployment topology and provider-aware probe credentials are external gates. |
