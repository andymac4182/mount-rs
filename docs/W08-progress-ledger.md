...
Status snapshot: **2026-09-21 22:31 AEST**
Repository: `andymac4182/mount-rs`  
Publication snapshot: `origin/main` at `b26819e` after the W08.9 implementation,
tracking and hosted-gate chunks were published; this documentation chunk records
the terminal hosted P05 job and remaining production gates.
...
| **P05** | Observability, SLOs, health checks and alerting | Implementation + hosted/provider | **Open — 15% gate weight; local and hosted implementation slices passed** | The HTTP transport now exposes unauthenticated `GET`/`HEAD /healthz` for listener/process liveness and `/readyz` for a non-empty configured drive registry; an empty registry returns `503 not_ready`. The default locked `mount-rs-http` suite passed 8 unit tests and 10 integration tests, while the all-features suite passed 8 unit tests and 13 integration tests including application telemetry and OTLP propagation; both strict-Clippy gates passed. Hosted CI run `35599817215`, source `b26819e`, job `http-observability` `106333141914` reached terminal success for the same all-features tests and lint. These are process/configuration and local/hosted exporter-path checks only: no production collector, dashboard, alert route, SLO, redaction, TiDB, RustFS or TLS health evidence is recorded here. | Define availability, mount/unmount, metadata latency, block latency, error-budget and recovery SLOs; map these probes and provider-specific checks to metrics/logs/traces, dashboards, alerts, paging, retention and secret/PII redaction; exercise an alert end-to-end. If the selected deployment needs provider-aware readiness, add and credential-test that separate probe. | **1–2 engineer-days**; **~0.5 engineer-day** for the HTTP slice plus local observability qualification | Production collector, paging destination, ownership, alert thresholds, selected deployment topology and provider-aware probe credentials are external gates. |
...
1. No required W08 functional implementation or hosted acceptance action
   remains for the defined scope. W08.6 adds a credential-free policy gate, but
   it is not a production deployment result. Preserve the exact terminal job
   IDs above; do not replace them with a later queued/cancelled aggregate
   status.
