...
The production target is ten servers, 10,000 concurrent clients, 10,000 distinct Drives across 5,000 Partitions, and 1,000 files in each Drive (ten million files). The balanced qualification fixture places two Drives per Partition; each client connection selects only its assigned Partition and is granted its own Drive. Ramp toward that exact target; preserve both 100-active mostly idle clients and all clients continuously performing I/O. Use authentic TLS transport, per-sandbox authorization checks, immutable expected-byte ledgers, and fresh-driver verification. Retain failure artifacts, setup time, peak resources, latency, datastore sessions, amplification, and cleanup. Exercise sequential and random reads, sequential and random overwrites, mixed read/write ratios, hot-file skew, append/truncate, and create/delete/rename churn. The initial mixed-size assumption is 990 files of 4 KiB, nine of 128 KiB, and one of 1 MiB per Drive, pending the user's preference. Record any changed distribution. Report namespace preparation separately from payload preparation and steady I/O. Verify both cross-Partition and unauthorized same-Partition sibling-Drive denial. Distinguish actual full-target execution from small-point validation or extrapolation. Local loopback qualification does not establish cross-host production capacity. Fix demonstrated blockers, rerun the affected point, and attempt the target within measured machine resources.
