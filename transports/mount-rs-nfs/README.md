...
`wait_closed` registers before checking completion, so a close racing that
check cannot strand a waiter; already closed connections also return promptly.
Close also interrupts a reader waiting for an in-flight slot, so a queued RPC cannot
