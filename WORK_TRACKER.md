...
- [x] W26.6 Bound remote directory enumeration before HTTP response
  ...
  chunked (14), CLI (44), observability (4) and persistence (4); JavaScript
  callbacks and key-value stores remain explicit provider-pagination gates.
...
| P7 — integration observability and error contract | Local HTTP/OTLP telemetry evidence passed; bounded-listing forwarding and reconciliation report exposed; production integration open | 40% | `mount-rs-http` passed 8 unit and 12 integration tests; OTLP-enabled HTTP passed 11 integration tests; full-feature observability/local collector/exporter-failure tests and CLI observability passed locally, including bounded timeout/connection/listing behavior; `reconcileBlocks` returns scanned/protected/recent/deleted counts and fails closed when unsupported, and `readdir_bounded` is forwarded through observability/CLI wrappers; deployed collector, retry/fencing/recovery dashboards and customer operations handoff remain open |
...
| P10 — security, privacy, tenancy and audit | Threat-model checkpoint and local controls recorded; lifecycle and pre-materialization listing hardening added; final review open | 50% | Delegated architecture threat model covers provider, credential, prefix, client/native and customer/Ozone boundaries; endpoint/TLS/redaction/auth/isolation, loopback-only bind, connection-cap, stalled-request and pre-materialization directory entry/response-byte limit tests pass locally; strict affected-workspace check is green; scoped reconciliation fails closed for unsupported providers, renews the writer lease, protects live/open-unlinked roots, validates block IDs and deletes only aged objects under the configured prefix; `FsDriver::readdir_bounded` fails closed for unsupported providers and is implemented/forwarded for built-in Rust paths, while key-value and JavaScript provider pagination remain open; standard scan `943a7c01-e25f-48ee-952f-040b7421e80d` remains running on older revision `f54dff8`, current revision run `35606250115` is pending, and provenance, secure Ozone auth/rotation and final security evidence remain open |
| P11 — end-to-end client/platform matrix | HTTP path and built-in bounded-listing contract added to Ozone CI; full matrix open | 30% | Rust/Node/CLI and the shipped HTTP server/client path now run through the Ozone composition gate with scoped cleanup; built-in Rust HTTP providers enforce the directory bound before response materialization; native mounts, JavaScript/key-value bounded providers, every advertised platform and terminal hosted evidence remain open |
