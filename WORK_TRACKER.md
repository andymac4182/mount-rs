...
| W07 | FoundationDB | Provider/composition and hosted durable RustFS acceptance passed; latest mainline Linux Node/CLI/native-FUSE qualification is green at `35655579378`/`0354116` with six hosted rollout-ledger regression cases and the NO-GO guard; the explicit lease-publication policy implementation and inline-secret/unsafe-TTL negative fixtures passed; its base latency was 81,981µs p95/p99 and five-round soak p95/p99 ranged 188,168–737,468µs with throughput 9.40–39.58 ops/s, so capacity evidence remains open; the prior `35632139449` N-API build blocker was corrected and requalified; target-gated root member and Rust SDK/CLI selection landed; production authority, complete Node/native platform matrix and the W07.7 production rollout gate remain open | Maxwell (complete slice) / Main |
...
- [ ] W07.7 **Production rollout readiness and go/no-go:** the demo and local
  ...
  - [ ] **Production-like workload and soak:** run multi-chunk reads/writes,
    ...
    latency and throughput; latest hosted run `35655579378` recorded base
    `operations=15 p50_us=7227 p95_us=81981 p99_us=81981 total_ms=191
    throughput_ops_per_sec=78.17` at revision `0354116`; its five soak-round
    p95/p99 values ranged from 188,168µs to 737,468µs and throughput ranged
    from 9.40 to 39.58 ops/s. This remains bounded
  ...
  - [ ] **Hosted and platform evidence:** the latest hosted FoundationDB/RustFS,
    Node, CLI/native Linux checkpoint is green for revision `0354116` in run
    `35655579378` on `ubuntu-24.04`, with the retained schema-2
...
- [x] W08.36 **Production evidence placeholder rejection:** the machine-
  ...
  The current integrated public mainline source
  `6a19ff6555635b20385acdc30d7aa2da143f6931` was then reverified with the full
  locked workspace test suite (exit 0) and strict workspace Clippy with
  `-D warnings` (exit 0). Provider/native tests requiring TiDB, RustFS,
  PGlite, R2, FUSE or NFS remained explicit opt-in skips; this is current
...
- [ ] W26.15 Resolve the terminal per-drive IOPS qualification blocker. The
  ...
  profile. The W26-owned write/read overlap remediations and atomic
  `FsDriver::write_file` path are published in `96a25f17` and prior merged
  tips. The latest chunk is `d1bc8fb9`, published in merged tip `6429c7ba`:
  `ChunkedFs::read_at` returns EOF before metadata work and coalesces ordinary
  read atime in coordinator state, with local stat visibility and explicit
  mutation/sync/fsync/shutdown publication. The regression
  `reads_coalesce_atime_and_eof_does_not_publish` proves no per-read or EOF
  metadata revision and persisted atime after sync. The full locked workspace
  tests, strict workspace Clippy and 16-test ChunkedFs suite pass; focused scan
  `5c0c59fb-a2c6-4e03-be6c-c334ca7cf0e7` found zero reportable findings and
  records the intentional atime crash boundary. Hosted run `35649202405`
  predates this chunk and remains diagnostic: SQLite/R2 91.94, PGlite/R2 95.82,
  TiDB/R2 16.10 and FoundationDB/R2 34.97 IOPS, with 1,200/1,200 lifecycle
  operations and zero timeout/cleanup failures, all failed `IOPS_TARGET_NOT_MET`;
  aggregate `106501216372` failed closed. Dispatch the complete packet against
  `6429c7ba`; preserve fencing, revision CAS, immutable-block ordering, POSIX
  semantics, cleanup and fail-closed artifact verification. Do not lower the
  1,000 target or convert failed rows into skips.
...
| P8 — 1,000 IOPS per-drive CI workload | Per-provider hard-threshold gates implemented; current packet failed hosted qualification; lazy-atime/EOF reduction published and awaiting hosted result | 52% | Benchmark measures successful write+read+delete lifecycle IOPS and fails below 1,000 for each configured Ozone-backed metadata provider. The generic Ozone composition requests SQLite/R2 and PGlite/R2 with 4 KiB payloads, 400 iterations, concurrency 64, `--min-iops 1000` and `--require-configured`; dedicated TiDB/FoundationDB Ozone jobs request their own rows with the same strict mode. Wrapper settings cannot lower the target below 1,000 or weaken the fixed profile. `scripts/verify-w26-ozone-iops-artifact.mjs` requires the exact provider set, zero skipped/configuration-failed rows, successful cleanup, per-size lifecycle success and a valid retained JSON artifact before the generic or dedicated pass marker is emitted. Current run `35649202405` on exact revision `e1765b7b` recorded SQLite/R2 91.94, PGlite/R2 95.82, TiDB/R2 16.10 and FoundationDB/R2 34.97 IOPS with 1,200/1,200 successful lifecycle operations and zero timeout/cleanup failures, but every row failed the hard target and aggregate `106501216372` failed closed. Published `d1bc8fb9` at merged tip `6429c7ba` removes per-read/EOF metadata publication while preserving explicit sync/shutdown durability; no hosted result exists for this tip yet. Local live Ozone evidence outside hosted CI remains unavailable. |
...
| P14 — final W26 integration-readiness review | NO-GO review active; W26.15 remains open | 31% | Current run `35649202405` is audited: base Ozone passed, all four provider lifecycles completed with zero timeout/cleanup failures but missed the hard IOPS target, and aggregate `106501216372` failed closed on the missing SQLite/PGlite pass marker. Lazy-atime/EOF commit `d1bc8fb9` is locally tested, strict-Clippy clean, security-scanned with zero reportable findings and published at merged tip `6429c7ba`; no hosted result exists for that new tip. A new one-revision all-provider/performance/security/end-to-end audit is required before an integration-ready handoff. |
