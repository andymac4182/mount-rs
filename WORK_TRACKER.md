...
| W26 | Apache Ozone S3 backend | W26 qualification harness and local controls are implemented; current published `origin/main` tip is `7389be4d` and includes publication-barrier `c7f0e6d0`, bounded mutation-window `183660a4`, lease-renewal `f10dbf22`, lazy-atime/EOF reduction, R2 content-addressing/cache, single-flight upload coalescing `d05548e8`, metadata batching, queue cancellation safety, same-revision concurrent-create inode rebasing, the corrected Ozone content-addressed contract and deduplicated cleanup, plus Ozone test lockfile fix `0cef5d44` and FoundationDB test lockfile fix `8428a5ef`. The customer-deployed Ozone integration track remains **NO-GO**: terminal run `35678993571` on exact SHA `4b4fe43a` measured SQLite/R2 802.37, PGlite/R2 614.90, TiDB/R2 86.53 and FoundationDB/R2 360.14 IOPS against 1,000; all rows completed 1,200/1,200 lifecycle operations with zero timeout/cleanup failures, but aggregate `106595737360` failed closed. Retained artifacts are composition `10674341635`, TiDB `10674350992` and FoundationDB `10674436409`; base Ozone job `106591586312` passed. The prior FoundationDB preflight lockfile block is fixed by `8428a5ef`. No current packet is acceptance evidence; the next exact-SHA packet follows a correctness-preserving metadata/publication performance chunk. Security scans `e44de27d-96d8-4330-a831-b995d6458a6c`, `47641152-6539-48e1-96b6-bf2201033486` and `739f4f5b-8cc4-4154-93f7-9e486745eab1` have zero reportable findings with complete local coverage; secure customer topology, 99.99%/5-minute objective evidence, native/end-to-end coverage and Ozone-owned DR/release gates remain explicit | Main |
