...
| W26 | Apache Ozone S3 backend | W26 qualification harness and local controls are implemented; current published `origin/main` tip is `4b4fe43a` and includes publication-barrier `c7f0e6d0`, bounded mutation-window `183660a4`, lease-renewal `f10dbf22`, lazy-atime/EOF reduction, R2 content-addressing/cache, single-flight upload coalescing `d05548e8`, metadata batching, queue cancellation safety, same-revision concurrent-create inode rebasing, the corrected Ozone content-addressed contract and deduplicated cleanup, plus Ozone test lockfile fix `0cef5d44` and FoundationDB test lockfile fix `8428a5ef`. The customer-deployed Ozone integration track remains **NO-GO**: terminal run `35674425514` missed the hard 1,000-IOPS-per-drive target for SQLite/R2 637.01, PGlite/R2 457.65, TiDB/R2 147.31 and FoundationDB/R2 126.50; diagnostic run `35677828440` on exact SHA `91866234` had SQLite/R2 1,220.33 (pass), PGlite/R2 522.34 (fail), TiDB/R2 145.74 (fail), and FoundationDB blocked before benchmark by its stale lockfile. The fresh rows completed 1,200/1,200 lifecycle operations with zero timeout/cleanup failures wherever the benchmark ran; artifacts `10673983034` and `10673463926` are retained, and FoundationDB log artifact `10672999481` records the preflight block. New hosted run `35678993571` targets exact SHA `4b4fe43a` with producer jobs `106591586312`, `106591586421`, `106591586086` and `106591586325` queued and no aggregate yet. No current packet is acceptance evidence. Security scans `e44de27d-96d8-4330-a831-b995d6458a6c`, `47641152-6539-48e1-96b6-bf2201033486` and `739f4f5b-8cc4-4154-93f7-9e486745eab1` have zero reportable findings with complete local coverage; secure customer topology, 99.99%/5-minute objective evidence, native/end-to-end coverage and Ozone-owned DR/release gates remain explicit | Main |
...
- [ ] W07.7 **Production rollout readiness and go/no-go:** the demo and local
  ...
  - [ ] **Hosted and platform evidence:** the latest hosted FoundationDB/RustFS,
    ...
    `1670ceba81b24c1ed39b8ab396671324c7f28193` in run `35675987457` (job
    `106582524993`) on `ubuntu-24.04`/Linux `amd64`, with the retained
    `foundationdb-production-qualification-35675987457-1` artifact (ID
    `10673640911`, SHA-256
    `0cb38b97b121ef8b6b69a1c4ef76d112bb24ba716b19eb6153ee52b13a3cc694`).
    Complete the
...
- [x] W08.36 **Production evidence placeholder rejection:** the machine-
  ...
  Merged source base `f68106cfcc64f3c7e4c90b2c413fcc393824a526` was freshly
  reverified after concurrent WebDAV mounted-restart cleanup correction, S3
