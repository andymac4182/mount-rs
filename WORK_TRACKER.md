...
| W05 | Cloudflare R2 | Functional slice and exact local Rust/Node SDK+CLI/N-API/PGlite/oracle packet are green; candidate CI `35702348089` is terminal-failed on an actionable Rust 9P test race, Node 9P timeout, Windows Node timeout, a TiDB prepared-statement failure-injector gap, and hard Ozone IOPS below 1000, so production closure remains active with repair/rerun, AWS security/OIDC, R2 cap reset/rotation, native-FUSE, platform, package, scope, and W20.6 gates open | Main |
