...
| `2026-09-22 FUSE forced-unmount deadline packet` | Share the forced `umount`/lazy-detach deadline with final session-task draining so bounded teardown does not add a third full timeout | Host FUSE all-target tests, host/Linux-target strict Clippy, Linux-target test check, formatting and diff checks pass; a Linux-gated stuck-helper regression covers the timeout and closed/inactive state, while hosted `/dev/fuse` forced-unmount and broader lifecycle gates remain open |
