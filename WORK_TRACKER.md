...
drain deadline expires. The focused Rust target passes 18/18 with strict
...
- [ ] W07.7 **Production rollout readiness and go/no-go:** the demo and local
  ...
  - [ ] **Production-like workload and soak:** run multi-chunk reads/writes,
    ...
    latency and throughput; latest hosted run `35652638242` recorded base
    `operations=15 p50_us=7712 p95_us=228723 p99_us=228723 total_ms=353
    throughput_ops_per_sec=42.41` at revision `c6f0039`; its five soak-round
    p95/p99 values ranged from 27,598µs to 35,139µs and throughput ranged
    from 77.45 to 95.05 ops/s. This remains bounded
  ...
  - [ ] **Hosted and platform evidence:** the latest hosted FoundationDB/RustFS,
    Node, CLI/native Linux checkpoint is green for revision `c6f0039` in run
    `35652638242` on `ubuntu-24.04`, with the retained schema-2
...
- [ ] W26.15 Resolve the terminal per-drive IOPS qualification blocker. The
  ...
  across remote block I/O and full namespace publication, yielding
  61.97/63.56/14.14/23.07 IOPS in the fixed 4 KiB/400-iteration/concurrency-64
  profile. The first W26-owned remediation is implemented and published:
  commit `a2075af0` added optimistic immutable-block writes with revision/base
  checks and serialized conflict fallback; commit `c4378dc1` extends the same
  safe overlap to remote reads, adds orphan-atime handling and a lifecycle read
  barrier for shutdown, and adds blocked-read gate-release/shutdown plus
  stale-read-versus-concurrent-write regressions. The 8-test concurrency suite,
  14 chunked unit tests, full locked workspace tests and strict workspace
  Clippy pass. Replacement hosted run `35641941218` on exact code head
  `88b707ba` is terminal for W26: SQLite/R2 85.70, PGlite/R2 94.04, TiDB/R2
  14.50 and FoundationDB/R2 30.46 IOPS, with 1,200/1,200 lifecycle operations
  and zero timeout/cleanup failures, all failed `IOPS_TARGET_NOT_MET`; aggregate
  `106477164582` failed closed. This measured improvement is not acceptance.
  The next W26-owned chunk is published in commit `96a25f17` and reconciled
  into `origin/main` at `92f900be`: `FsDriver::write_file` now provides a safe
  generic fallback, N-API `writeFile` uses it, and `ChunkedFs` stages immutable
  bytes outside the metadata gate, flushes them, then performs one fenced
  namespace publication with serialized open/write/close fallback on an
  optimistic conflict. The new creation/reopen/content regression, 15-test
  ChunkedFs suite, full locked workspace tests, strict workspace Clippy, N-API
  check, storage benchmark units, formatting and diff checks pass. Security
  diff scan `11fa7e54-bb66-488d-a369-c1a7ac46da81` found zero reportable
  findings across the three changed files, with hosted/provider/customer
  controls deferred. Current-tip hosted run `35649202405` on exact revision
  `e1765b7b` measured SQLite/R2 91.94, PGlite/R2 95.82, TiDB/R2 16.10 and
  FoundationDB/R2 34.97 IOPS. Every row completed 1,200/1,200 lifecycle
  operations with zero timeout/cleanup failures but failed
  `IOPS_TARGET_NOT_MET`; aggregate job `106501216372` failed closed on the
  missing `OZONE_IOPS_PASS` marker. The atomic path is a measured improvement,
  not acceptance. Implement and locally verify the next safe
  metadata-publication reduction before dispatching the next one-revision
  packet. Preserve
  fencing, revision CAS, immutable-block ordering, POSIX semantics, cleanup
  and fail-closed artifact verification; do not lower the 1,000 target or
  convert failed rows into skips.
...
| P14 — final W26 integration-readiness review | NO-GO review active; W26.15 remains open | 29% | Current run `35649202405` is audited: base Ozone passed, all four provider lifecycles completed with zero timeout/cleanup failures but missed the hard IOPS target, and aggregate `106501216372` failed closed on the missing SQLite/PGlite pass marker. Atomic whole-file write commit `96a25f17` is locally tested, strict-Clippy clean and security-scanned with zero reportable findings; the hosted result is diagnostic and insufficient. A new substantive W26.15 result plus one-revision all-provider/performance/security/end-to-end audit is required before an integration-ready handoff. |
