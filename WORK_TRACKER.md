...
- [x] W26.7 Add a credential-free production configuration policy gate for the
  ...
  values. The `ozone` CI job runs four positive fixtures and requires an
  insecure HTTP fixture to fail. Local positive/negative execution passed;
  commit `b8d8fb1` was published in reconciled tip `4d8b5fd`. Focused diff scan
  `d74e3e86-2e0a-45cf-9819-e31f428eb5d4` found zero reportable findings with
...
| P3 — authentication, TLS, secrets and redaction | Local transport and credential-free production-config policy hardened; secure integration open | 58% | Static and runtime R2/Ozone validation rejects malformed, credential-bearing and remote plaintext-HTTP endpoints before client construction; HTTP config/runtime now reject non-loopback binds and require a TLS reverse proxy for remote clients; the new all-provider policy gate requires HTTPS R2 blocks, exact external secret references, durable metadata and TLS-required TiDB input without contacting a provider; secure endpoint/auth, least privilege, rotation and full negative-path evidence remain open |
