...
| W26 | Apache Ozone S3 backend | W26 qualification harness and local controls are implemented; current `origin/main` `412c422e` includes W26 publication-barrier implementation SHA `c7f0e6d0`, bounded mutation-window SHA `183660a4`, lease-renewal SHA `f10dbf22`, lazy-atime/EOF reduction, R2 content-addressing/cache, single-flight R2 upload coalescing `d05548e8`, metadata batching, queue cancellation safety, same-revision concurrent-create inode rebasing, the corrected Ozone content-addressed contract and deduplicated cleanup. The customer-deployed Ozone integration track remains **NO-GO** after terminal run `35674425514` missed the hard 1,000-IOPS-per-drive target for SQLite/R2 637.01, PGlite/R2 457.65, TiDB/R2 147.31 and FoundationDB/R2 126.50; every row completed 1,200/1,200 lifecycle operations with zero timeout/cleanup failures and aggregate `106579811902` failed closed. Fresh hosted run `35676336466` tests exact SHA `503f3f75` with producer jobs `106583542086`, `106583542156`, `106583542085` and `106583542280` initially queued and no aggregate yet. Security scans `e44de27d-96d8-4330-a831-b995d6458a6c`, `47641152-6539-48e1-96b6-bf2201033486` and `739f4f5b-8cc4-4154-93f7-9e486745eab1` have zero reportable findings with complete local coverage; secure customer topology, 99.99%/5-minute objective evidence, native/end-to-end coverage and Ozone-owned DR/release gates remain explicit | Main |
...
`origin/main` is `412c422e2485a5c7ce2caf55892ec6475faab8d8`, including the
...
hardening, same-revision concurrent-create inode rebasing, plus the corrected
...
manual run `35674425514` on exact revision
`a81a08272aac9085d6f2c2c31a7f1c9392a4bed6`: SQLite/R2 637.01, PGlite/R2
457.65, TiDB/R2 147.31 and FoundationDB/R2 126.50 IOPS, each with
...
`106579811902` failed closed. Fresh manual run `35676336466` selected exact SHA
`503f3f757c8b599e11dbd0c5f99f8cfcc67aee5` and initially had W26 producer jobs
`106583542086` (`ozone`, queued), `106583542156` (`ozone-compositions`,
queued), `106583542085` (`ozone-tidb`, queued) and `106583542280`
(`ozone-foundationdb`, queued); the aggregate job was not yet present.
...
- [ ] W26.15 Resolve the terminal per-drive IOPS qualification blocker. The
  ...
  `35674425514` on exact revision `a81a0827` remains diagnostic: SQLite/R2
  637.01, PGlite/R2 457.65, TiDB/R2 147.31 and FoundationDB/R2 126.50 IOPS;
  every row completed 1,200/1,200 operations with zero timeout/cleanup
  failures but failed `IOPS_TARGET_NOT_MET`, and aggregate `106579811902`
  ...
  `d05548e8` is included in current `origin/main` `412c422e`; fresh manual run
  `35676336466` selected exact revision `503f3f75`, and its four producers were
  queued at dispatch with no aggregate yet.
  Preserve fencing,
...
| P8 — 1,000 IOPS per-drive CI workload | Per-provider hard-threshold gates implemented; latest terminal packet failed hosted qualification; single-flight qualification pending | 62% | Benchmark measures successful write+read+delete lifecycle IOPS and fails below 1,000 for each configured Ozone-backed metadata provider. The generic Ozone composition requests SQLite/R2 and PGlite/R2 with 4 KiB payloads, 400 iterations, concurrency 64, `--min-iops 1000` and `--require-configured`; dedicated TiDB/FoundationDB Ozone jobs request their own rows with the same strict mode. Wrapper settings cannot lower the target below 1,000 or weaken the fixed profile. The verifier requires the exact provider set, zero skipped/configuration-failed rows, successful cleanup, per-size lifecycle success and a valid retained JSON artifact before a pass marker is emitted. Terminal run `35674425514` on exact revision `a81a0827` recorded SQLite/R2 637.01, PGlite/R2 457.65, TiDB/R2 147.31 and FoundationDB/R2 126.50 IOPS with 1,200/1,200 successful lifecycle operations and zero timeout/cleanup failures, but every row failed the hard target and aggregate `106579811902` failed closed. Bounded mutation-window implementation `183660a4` and single-flight R2 upload coalescing `d05548e8` are in current `origin/main` `412c422e`; fresh run `35676336466` targets exact SHA `503f3f75` with all four W26 producers initially queued and no aggregate job yet. Local live Ozone evidence outside hosted CI remains unavailable. |
...
| P14 — final W26 integration-readiness review | NO-GO review active; W26.15 remains open; single-flight qualification pending | 37% | Terminal run `35674425514` is audited: base Ozone passed, all four provider lifecycles completed with zero timeout/cleanup failures but missed the hard IOPS target, and aggregate `106579811902` failed closed. Publication-barrier implementation `c7f0e6d0`, bounded mutation window `183660a4`, single-flight R2 upload coalescing `d05548e8`, mutation batching/cache/queue hardening, same-revision concurrent-create rebasing, the corrected Ozone block-contract test and deduplicated cleanup are locally tested; security scans `e44de27d-96d8-4330-a831-b995d6458a6c`, `47641152-6539-48e1-96b6-bf2201033486` and `739f4f5b-8cc4-4154-93f7-9e486745eab1` report zero findings within complete local scopes. Fresh run `35676336466` targets exact SHA `503f3f75` with all four W26 producers initially queued and no aggregate yet. A terminal one-revision all-provider/performance/security/end-to-end audit is required before an integration-ready handoff. |
