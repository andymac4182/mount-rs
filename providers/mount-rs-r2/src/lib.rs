//! Cloudflare R2 and S3-compatible object-store provider.
//!
//! `R2Config` builds an S3-compatible client, `R2Store` stores versioned
//! snapshot state, and `R2BlockStore` exposes immutable blocks through the
//! shared object-store adapter. Filesystem behavior lives in
//! `mount-rs-persist` and the `mount-rs-r2-fs` crate. The snapshot key and
//! block prefix are configurable so volumes can share a bucket safely.

mod blocks;

pub use blocks::{
    R2BlockStore, R2BlockStoreErrorClass, R2BlockStoreStats, R2ConcurrentQualification,
};
...
use mount_rs_core::{
    FsError, LoadedSnapshot, Result, StateStore, backend_error, snapshot_conflict,
};
