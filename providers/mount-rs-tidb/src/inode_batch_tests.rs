use super::*;
#[test]
fn packet_bound_counts_unicode_and_checked_overflow() {
    let a = encoded_bound(64, 255, 1000).unwrap();
    let b = encoded_bound(64, 1020, 1000).unwrap();
    assert_eq!(b - a, 64 * (1020 - 255));
    assert!(encoded_bound(usize::MAX, 1, 1).is_none());
    assert!(encoded_bound(1, usize::MAX, 1).is_none());
    assert!(encoded_bound(64, 1020, 256 * 1024).unwrap() > MAX_BYTES);
}
...
async fn actual_guard_batch_boundaries_and_rollback_cancel_body() {
    ...
    for count in [1, 63, 64, 65, 131] {
        ...
        if count == 131 {
            ...
            let peer = TidbMetadataStore::connect_with_key(&url, &key)
                .await
                .unwrap();
            ...
            println!(
                "GUARD_SELECTED_RACE_PASS provider=tidb stale_selected_not_acknowledged=true generation=4"
            );
            println!(
                "GUARD_FAULT_PASS provider=tidb real_second_batch_duplicate_rollback=true cancel_after_first_batch=true reuse_publication=true"
            );
        }

        store.close().await.unwrap();
        cleanup(&url, &key).await;
        println!("GUARD_BOUNDARY_PASS provider=tidb nodes={count} rows={counts:?}");
    }
}
...
async fn actual_guard_byte_boundary_and_oversized_singletons_body() {
    ...
    for scenario in ["exact", "over", "oversized", "byte-split"] {
        ...
        cleanup(&url, &key).await;
        println!(
            "GUARD_SIZE_PASS provider=tidb scenario={scenario} rows={rows:?} exact_namespace=true"
        );
    }
}
...
async fn cleanup_public_sdk_bulk_owned_rows_body() {
    ...
    for line in manifest.lines() {
        ...
        let suffix = key.strip_prefix("sdk-bulk-tidb-").unwrap();
        ...
        println!("SDK_BULK_CLEANUP_PASS provider=tidb exact_owned_tables=4 residual_rows=0");
    }
}
...
async fn actual_guard_late_signed_overflow_rolls_back_enrollment_and_structure_body() {
    ...
    for enrollment in [true, false] {
        ...
        cleanup(&url, &key).await;
        println!(
            "GUARD_LATE_OVERFLOW_PASS provider=tidb enrollment={enrollment} prior_successful_batches=2 rollback=true"
        );
    }
}
